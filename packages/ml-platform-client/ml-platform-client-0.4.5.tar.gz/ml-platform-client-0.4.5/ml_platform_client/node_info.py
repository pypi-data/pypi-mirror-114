...
class NodeInfo:
    ...
    def get_io(self):
        ...
        self.net_io = str(round(net_io.bytes_sent / self.unit, 1)) + "/" + str(round(net_io.bytes_recv / self.unit, 1))

        block_io = psutil.disk_io_counters()
        self.block_io = str(round(block_io.read_bytes / self.unit, 1)) + "/" + str(
            round(block_io.write_bytes / self.unit, 1))

    def statistics(self):
        self.get_cpu()
        self.get_memory()
        self.get_io()
        result = {"totol_memory": self.total_memory,
