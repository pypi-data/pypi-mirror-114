...
class MinIOAccessor:
    ...
    def get_client(remote_host=None) -> Minio:
        if MinIOAccessor._client is None:
            ...
            MinIOAccessor.init_client(host, client_config.minio_access_key,
    ...
    def fget_object(filepath: str, save_path: str, remote_host=None):
        ...
        while retry < MinIOAccessor.max_retry:
            try:
                MinIOAccessor.get_client(remote_host).fget_object(bucket, filepath, save_path)
    ...
    def save_object(local_path: str, remote_path: str, remote_host=None):
        ...
        with open(local_path, 'rb') as file_data:
            ...
            MinIOAccessor.get_client(remote_host).put_object(bucket, filepath, file_data, file_stat.st_size)

    @staticmethod
    def delete(bucket, prefix: str, recursive=False, remote_host=None):
        objs = MinIOAccessor.get_client(remote_host).list_objects(bucket, prefix, recursive)
        for obj in objs:
            MinIOAccessor.get_client(remote_host).remove_object(bucket, obj.object_name)
