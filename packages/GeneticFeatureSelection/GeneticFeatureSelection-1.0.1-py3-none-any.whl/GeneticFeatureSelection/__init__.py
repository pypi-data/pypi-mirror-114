...
class GeneticFeatureSelection:
    ...
    def sequential_selection(self, X, y, pop_size, estimator, scoring, cv, select_method, offspring_size, c_pt, epsilon=.1, heuristic=None, tolerance=5, verbose=1):
        total_start = time.time()
        population = self.initialize(X, pop_size, heuristic)
        n_gen = 1
        self.trial_best = []
        self.track = {}
        self.mean_fitness = []
        ...
        while True:
            ...
            if verbose > 0:
                print(f'---------------------------------------------Gen {n_gen}---------------------------------------------')
            performance = self.trial(population, estimator, X, y, scoring, verbose, cv=cv)
