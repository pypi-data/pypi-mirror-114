...
def text_classification(pred_data, ref_data=None, k_nearest_neighbours=None, thread_ratio=None, lang='EN', index_name=None):
    ...
    if conn_context.is_cloud_version():
        ...
        conn_context.create_table(result_tab, table_structure={'ID': 'VARCHAR(255)',
                                                               'RANK': 'INT',
                                                               'CATEGORY_SCHEMA': 'NVARCHAR(256)',
                                                               'CATEGORY_TABLE': 'NVARCHAR(256)',
                                                               'CATEGORY_COLUMN': 'NVARCHAR(256)',
                                                               'CATEGORY_VALUE': 'NVARCHAR(256)',
                                                               'NEIGHBOR_COUNT': 'INT',
                                                               'SCORE': 'DECIMAL(34, 0)'})
        for row in pred_data.select(pred_data.columns[0]).collect().to_numpy().flatten():
            ...
            temp_df.save(result_tab, append=True)
...
class TFIDF(PALBase):  # pylint: disable=too-many-instance-attributes, too-few-public-methods
    ...
    >>> df_train.collect()
            ID	CONTENT
        0   0	doc1	term1 term2 term2 term3 term3 term3
        1   1	doc2	term2 term3 term3 term4 term4 term4
        2   2	doc3	term3 term4 term4 term5 term5 term5
        3   3	doc5	term3 term4 term4 term5 term5 term5 term5 term5 term5
        4   4	doc4	term4 term6
        5   5	doc6	term4 term6 term6 term6
