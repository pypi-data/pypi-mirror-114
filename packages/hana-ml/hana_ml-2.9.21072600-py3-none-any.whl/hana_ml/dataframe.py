...
class ConnectionContext(object):
    ...
    def create_table(self, table, table_structure, schema=None, table_type='COLUMN', prop='', data_lake=False):
        """
        ...
        if data_lake:
            query = "CALL SYSRDL#CG.REMOTE_EXECUTE ('{}')".format(query)
        try:
            with self.connection.cursor() as cur:
                cur.execute(query)
    ...
    def create_virtual_table(self, table, data_lake_table, schema=None):
        """
        ...
        if schema is None:
            ...
            table_reference = '.'.join(map(quotename, (schema, table)))
        sql = 'CREATE VIRTUAL TABLE {} AT "SYSRDL#CG_SOURCE"."NULL"."SYSRDL#CG".{}'.format(table_reference, quotename(data_lake_table))
        try:
            with self.connection.cursor() as cur:
                cur.execute(sql)
    ...
    def drop_table(self, table, schema=None, data_lake=False):
        """
    ...
    def copy_to_data_lake(self, data, virtual_table, data_lake_table, schema=None, append=False):
        """
...
class DataFrame(object):#pylint: disable=too-many-public-methods
    ...
    def save(self, where, table_type=None, force=False, save_source=True, append=False, data_lake=False):
        """
        ...
        if data_lake:
            if force:
                self.connection_context.drop_table(table=table, data_lake=True) #drop data lake table
    ...
    def geometries(self) -> list:
        ...
        """
        return [col[0] for col in self.dtypes() if col[1] in ["GEOMETRY", "POINT"]]
