...
class PCA(PALBase):
    ...
    def fit(self, data, key=None, features=None, label=None):#pylint: disable=too-many-locals, invalid-name
        """
    ...
    def fit_transform(self, data, key=None, features=None, n_components=None, label=None):#pylint:disable=invalid-name
        """
        ...
        if self.scores_ is None:
            return self.transform(data_, key, features, n_components, label)
        if label is None:
            return self.scores_
        return self.scores_.alias('L').join(data_.alias('R'), 'L.%s' % key + '= R.%s' % key, select=['L.*', label])

    def transform(self, data, key=None, features=None, n_components=None, label=None):#pylint:disable=invalid-name, too-many-locals
        """
