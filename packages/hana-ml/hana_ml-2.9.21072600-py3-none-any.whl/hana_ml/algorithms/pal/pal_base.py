...
class PALBase(hana_ml.ml_base.MLBase, ModelSavingServices):
    ...
    def __init__(self, conn_context=None):
        ...
        self.parallel_by_parameter_partitions = False

    def enable_parallel_by_parameter_partitions(self):
        """
        Enable parallel by parameter partitions.
        """
        self.parallel_by_parameter_partitions = True

    def disable_parallel_by_parameter_partitions(self):
        """
        Disable parallel by parameter partitions.
        """
        self.parallel_by_parameter_partitions = False
    ...
    def _call_pal_auto(self, funcname, *args):
        self.execute_statement = call_pal_auto_with_hint(self.conn_context,
                                                         self.parallel_by_parameter_partitions,
                                                         funcname,
                                                         *args)
...
def call_pal_auto(conn,
    ...
    """
    args = list(args)
    return call_pal_auto_with_hint(conn, False, funcname, *args)

def call_pal_auto_with_hint(conn,
                            parallel_by_parameter_partitions,
                            funcname,
                            *args):
    """
    Uses an anonymous block to call a PAL function.
    ...
    conn : ConnectionContext
        HANA connection to use.
    parallel_by_parameter_partitions : bool,
        If True, it will use parallel with hint PARALLEL_BY_PARAMETER_PARTITIONS.
    funcname : str
        Name of the PAL function to execute. Should not include the "_SYS_AFL."
        part.
    arg1, arg2, ..., argN : DataFrame, ParameterTable, or str
