...
class OnlineLinearRegression(PALBase):
    ...
    def partial_fit(self,
        ...
        if self.intermediate_result_ is None:
            unique_id = str(uuid.uuid1()).replace('-', '_').upper()
        ...
        try:
            with conn.connection.cursor() as cur:
                cur.execute('CREATE LOCAL TEMPORARY COLUMN TABLE {} ("SEQUENCE" INTEGER, "INTERMEDIATE_MODEL" NVARCHAR(5000))'.format(inter_tbl_name))
                cur.execute('INSERT INTO {} SELECT * FROM ({})'.format(inter_tbl_name, self.intermediate_result_.select_statement))
