...
class ModelSavingServices(object): #pylint: disable=useless-object-inheritance
    ...
    def _encode_and_save(self, schema, storage_type, force=False):
        """
    ...
    @staticmethod
    def _delete_model(connection_context, name, version, js_str, storage_type):
        """
            ...
            with connection_context.connection.cursor() as cursor:
                if lib == 'APL':
                    ...
                    if storage_type == 'HDL':
                        ...
                        connection_context.drop_table(table=data_lake_table_name, data_lake=True)
                    connection_context.connection.commit()
    ...
    def _save_model_tables(self, schema, model_table_names, storage_type, force=False):
        """
        ...
        if self._is_APL():
            ...
            if not isinstance(self.model_, (list, tuple)):
                if storage_type == 'HDL':
                    self.model_.save(where=model_table_names, data_lake=True, force=force)
                else:
                    self.model_.save(where=(schema, model_table_names), force=force)
            else:
                for i, table_name in enumerate(list(model_table_names)):
                    if storage_type == 'HDL':
                        self.model_[i].save(where=table_name, data_lake=True, force=force)
                    else:
