...
class TemporalNetwork :
    ...
    def has_tedge(self, tedge) :
        ...
        return ((self._tedges['i'] == u) & (self._tedges['j'] == v) & (self._tedges['t'] == t))
    ...
    def neighbors(self) :
        ...
        return {node : list(values.keys()) for node, values in dict(G_agg.adjacency()).items()}
        ...
        if not reverse : 
            ...
            for edge_to_modify in self.temporal_edges : 
                if edge_to_modify != edge : 
                    i, j = TN_modified._edge_to_indices(edge_to_modify) # one edge to modify
                    TN_modified.snapshots[:, i, j] = default_weight
                    TN_modified.snapshots[:, j, i] = default_weight
...
#                    TN_modified.snapshots[:, :, i] = default_weight
        ...
        if reverse :
            ...
            TN_modified.temporal_edges = [edge for edge in TN_modified.temporal_edges
    ...
    def from_tedges(cls, tedges, normalise=None) :
        ...
        if normalise :
            if 'weight' not in tedges.columns :
                raise ValueError('Cannot normalise weights because edges are unweighted')
    ...
    def from_static_network_and_tedges(cls, static_network, tedges,
        ...
        static_network_edges_set = set(static_network_edges[['static_i', 'static_j']].itertuples(index=False, name=None))
        ...
        if missing_edges == set() :
            ...
            print(f"WARNING: {len(missing_edges)}/{len(static_network_edges)} edges "
            ...
            return pk.PartiallyTemporalNetwork.from_static_network_and_tedges(static_network, tedges,
    ...
    @classmethod
    def from_static_network_and_edge_timeseries(cls,
                                                static_network,
                                                edge_timeseries,
                                                static_edge_default_weight=None,
                                                normalise=None,
        ...
        return cls.from_static_network_and_tedges(static_network, tedges, static_edge_default_weight, normalise, quiet)

    @classmethod
    def from_static_network_and_node_timeseries(cls,
                                                static_network,
                                                node_timeseries,
        ...
        node_series_in_network = node_timeseries[node_timeseries.index.isin(nodes_temporal)]

        # combine node timeseries to obtain edge timeseries, only edges that a present in the static network
        edge_series = convert_node_to_edge_timeseries(node_series_in_network, combine_node_weights,
                                                      static_edges=list(static_network.edges))

        return cls.from_static_network_and_edge_timeseries(static_network, edge_series,
                                                           static_edge_default_weight, normalise, quiet)
    ...
    if isinstance(tedges, list) :  # convert to DataFrame
        if all([isinstance(tedge, tuple) for tedge in tedges]) :
            if all([len(tedge) == 3 for tedge in tedges]) :
                ...
                raise ValueError('Tedges in list must have length 3 (i, j, t) or 4 (i, j, t, weight)')
        else :
            raise TypeError('All tedges in list should be tuples')
    elif isinstance(tedges, pd.DataFrame) :  # check columns
        if (list(tedges.columns) == ['i', 'j', 't']) or (list(tedges.columns) == ['i', 'j', 't', 'weight']) :
            pass
        else :
            print(tedges)
            raise ValueError('Tedge dataframe must have columns (i, j, t) or  (i, j, t, weight)')
    else :
        raise TypeError('Invalid type of input tedges: should be a list of tuples or a DataFrame')
...
def convert_edge_timeseries_to_tedges(edge_timeseries) :
    ...
    tedges[['i', 'j']] = tedges['level_0'].str.split('-', expand=True)  # split edge as columns 'i' and 'j'
    del tedges['level_0']
...
def convert_node_to_edge_timeseries(node_series, combine=lambda x, y : x * y, static_edges=None) :
    ...
    edges = [tuple(edge) for edge in set3]  # list of edges in static network with temporal information
