...
class AbstractGrapheneMutationCreator(abc.ABC):
    ...
    def login_mutation_body(self, mutation_instance: any, info, *args, **kwargs) -> any:
        ...
        return maybe_thenable((user, mutation_instance, info, result, args, kwargs), lambda values: self._on_login_mutation_output_computed(*values))
    ...
    def me_query_body(self, query_instance: any, info, *args, **kwargs) -> any:
        ...
        return query_instance(**{self.me_query_actual_output_name: user})
    ...
    def _generate_me(self, context) -> type:
        ...
        self.me_query_body.output_name = self.me_query_return_value_name(context)
...
class AbstractTokenGeneratorMutationCreator(AbstractGrapheneMutationCreator):
    ...
    @abc.abstractmethod
    def jwt_refresh_expires_in_name(self) -> str:
        """
    ...
    def _login_mutation_return_type(self, context) -> Dict[str, any]:
        ...
        result[self.jwt_refresh_expires_in_name()] = graphene.Int(required=True)
        return result
    ...
    def login_mutation_body(self, mutation_instance, info, *args, **kwargs):
        csrf_dec = get_csrf_rotation_decorator(self.jwt_csrf_rotation())
        refresh_dec = get_refresh_expiration_decorator(
            self.jwt_token_expiration_time(),
            self.jwt_refresh_expires_in_name()
        )
