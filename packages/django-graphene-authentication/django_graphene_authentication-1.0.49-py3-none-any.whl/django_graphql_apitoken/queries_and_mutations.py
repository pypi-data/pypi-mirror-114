...
class AbstractGrapheneMutationCreator(abc.ABC):
    ...
    def _perform_authentication(self, root, info, *args, **kwargs) -> any:
        """
        ...
        request._jwt_token_auth = True # We are trying to authenticate via a login mutation
        user = authenticate(root=root, info=info, *args, **kwargs) # Call django to authenticate the user
        if user is None:
            self.authentication_failed(root, info, *args, **kwargs)
    ...
    def login_generate_mutation_output(self, cls, root, info, *args, **kwargs):
        """
        ...
        :return:
        """
        return cls(root, info, *args, **kwargs)

    def login_mutation_body(self, cls, root, info, *args, **kwargs) -> any:
        """
        Actual body of hte mutation login. You should not directly override this method, but rely
        ...
        :return:
        """
        user = self._perform_authentication(root, info, *args, **kwargs)
        result = self.login_generate_mutation_output(self,  cls, root, info, *args, **kwargs)
        # now generate the payload
        return maybe_thenable((user, root, info, user, result, args, kwargs), lambda values: self._on_login_mutation_output_computed(*values))
...
class AbstractTokenGeneratorMutationCreator(AbstractGrapheneMutationCreator):
    ...
    def _create_refresh_token(self, user, refresh_token=None) -> any:
        ...
        return model_type.objects.create(**model_type.get_create_arguments(user))
    ...
    def _jwt_payload_handle(self, user, request):
        ...
        exp = datetime.utcnow() + self.token_expiration_time()
    ...
    def login_mutation_body(self, cls, root, info, *args, **kwargs):
        return csrf_rotation(self.jwt_csrf_rotation())(
            refresh_expiration(self.token_expiration_time())(
                super().login_mutation_body(cls, root, info, *args, **kwargs)
            )
        )
...
class AbstractAccessTokenGeneratorMutationCreator(AbstractTokenGeneratorMutationCreator):
    """
