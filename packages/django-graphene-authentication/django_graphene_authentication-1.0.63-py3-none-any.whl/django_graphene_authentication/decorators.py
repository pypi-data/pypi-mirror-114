...
def get_ensure_refresh_token_decorator(cookie_name: str):
    ...
    def decorator(f):
        @functools.wraps(f)
        def wrapper(info=None, refresh_token=None, **kwargs):
            if info is None:
                raise ValueError(f"graphql info is None!")
            if refresh_token is None:
                refresh_token = info.context.COOKIES.get(cookie_name)
                if refresh_token is None:
                    raise ValueError(_('Refresh token is required'))
            result = f(**{"info": info, "refresh_token": refresh_token, **kwargs})
        return wrapper
    return decorator
...
def get_csrf_rotation_decorator(enable: bool = True):
    ...
    def decorator(f):
        @functools.wraps(f)
        def wrapper(info=None, **kwargs):
            if info is None:
                raise ValueError(f"graphql info is None!")
            result = f(**{"info": info, **kwargs})
            if enable:
