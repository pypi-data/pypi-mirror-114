...
class SlappResponseObject:
    ...
    def get_teams_for_player(self, p: Player) -> List[Team]:
        from slapp_py.core_classes.builtins import NoTeam, UNKNOWN_TEAM
        return list(map(lambda t: self.known_teams.get(t, UNKNOWN_TEAM) if t != NoTeam.guid else NoTeam, p.teams))
    ...
    def get_low_ink_placements(self, p: Player) -> List[Tuple[int, str, str]]:
        ...
        result = []
        if p.guid.__str__() in self.placements_for_players:
            low_ink_sources = list(filter(lambda s: "low-ink-" in s, self.placements_for_players[p.guid.__str__()]))
            for source_id in low_ink_sources:
                ...
                brackets = list(filter(lambda b:
                                       (b.name.startswith("Alpha") or
                                        b.name.startswith("Beta") or
                                        b.name.startswith("Gamma") or
                                        b.name.startswith("Top Cut")) and
                                       1 in bracket.placements.players_by_placement, self.placements_for_players[p.guid.__str__()][source_id]))
                for bracket in brackets:
                    for placement in bracket.placements.players_by_placement:
                        if p.guid.__str__() in placement:
    ...
    def best_low_ink_placement(self, p: Player) -> Optional[Tuple[int, str, str]]:
        ...
        for placement in self.get_low_ink_placements(p):
            if current_best is None:
                current_best = (placement[0], placement[1], placement[2])
            else:
                is_same_comparison = (placement[1].startswith("Alpha") and current_best[1].startswith("Alpha")) or \
                                     (placement[1].startswith("Beta") and current_best[1].startswith("Beta")) or \
                                     (placement[1].startswith("Gamma") and current_best[1].startswith("Gamma")) or \
                                     (placement[1].startswith("Alpha") and current_best[1].startswith("Top Cut")) or \
                                     (placement[1].startswith("Top Cut") and current_best[1].startswith("Alpha"))
                if is_same_comparison:
                    # If lower place (i.e. did better)
                    if placement[0] < current_best[0]:
                        current_best = placement
                else:
                    if placement[1].startswith("Alpha") or placement[1].startswith("Top Cut"):
                        # Better than the current best's bracket
                        current_best = placement
                    elif placement[1].startswith("Beta"):
                        if not current_best[1].startswith("Alpha"):
    ...
    def placement_is_winning_low_ink(placement: Tuple[int, str, str]):
        return placement[0] == 1 and (placement[1].startswith("Top Cut") or placement[1].startswith("Alpha"))
