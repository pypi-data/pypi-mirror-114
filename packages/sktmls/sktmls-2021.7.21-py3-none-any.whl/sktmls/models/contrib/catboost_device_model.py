...
class CatboostDeviceModel(MLSCatBoostModel):
    ...
    def __init__(
        ...
        - context_meta: (`sktmls.meta_tables.MetaTable`) 컨텍스트 그룹 값
        ...
        context_meta = metatable_client.get_meta_table(name="test_context_table")
        ...
        assert isinstance(context_meta, MetaTable) and len(context_meta.items) > 0, "`context_meta`는 필수 입력 값 입니다."
        ...
        for context_meta_item in context_meta.items:
            context_meta_item_name = context_meta_item.get("name")
            context_meta_item_value = context_meta_item.get("values")

            assert (
                context_meta_item_value and context_meta_item_name
            ), "`context_meta`의 `item` 항목 중 `name`과 `values`는 필수입니다."
            assert isinstance(context_meta_item_name, str), "`context_meta.item`의 `name`은 str 타입이어야 합니다."
            assert isinstance(context_meta_item_value, dict), "`context_meta.item`의 `values`은 dict 타입이어야 합니다."
            assert {
                "context_id",
                "text_id",
            } <= context_meta_item_value.keys(), "`context_meta.item`의 `values`는 `context_id`, `text_id` 키를 포함하여야 합니다."
        ...
        self.contexts_meta = {meta["name"]: meta["values"] for meta in context_meta.items}
                ...
                if context_features:
                    for context_feature in context_features:
                        if re.match("^context_age[0-9]{2}", context_feature):
                            ...
                            context_id = context_feature
                            break
                text_id = self.contexts_meta.get(context_id, {}).get("text_id", "Text_069")
                text_value = self.contexts_meta.get(context_id, {}).get("t_world_text", "유행에 민감한")
    ...
    def _convert(self, features):
        feature_names = self.features[1 : len(self.features) - len(self.default_emb_feature_values)]
        converted_features = []
