...
class TestScmCube:
    ...
    @pytest.mark.parametrize("log_failure", (True, False, None))
    ...
    def test_get_scm_timeseries_weights(
        ...
        if log_failure is not None:
            extra_kwargs["log_failure"] = log_failure
        ...
        if log_failure is not None:
            mock_get_weights.assert_called_with(
                expected_regions, log_failure=log_failure
            )
        else:
            mock_get_weights.assert_called_with(expected_regions, log_failure=False)
