...
def upsert_timeseries_ds(df: pandas.DataFrame, api_inputs: ApiInputs, is_local_time: bool = True,
    ...
    # timeseries_intersect = list(set(required_columns) & set(proposed_columns))
    # if len(timeseries_intersect) != 4:
    #     logger.error("data_frame must contain the following columns: %s.", req_cols)
    ...
    start_date = data_frame['Timestamp'].dt.date.unique().min()
    end_date = data_frame['Timestamp'].dt.date.unique().max()

    timezones = _timezone_offsets(api_inputs=api_inputs, date_from=start_date, date_to=end_date,
...
def replace_data(data_frame, api_inputs: ApiInputs, table_name: str):
    ...
    # print("Switch: Replacing All Data for ... " + name)
...
def append_data(data_frame, api_inputs: ApiInputs, table_name: str):
    ...
    # print("Switch: Appending Data for ... " + name)
