...
class Timer:
    def __init__(
        self, mode: str, durations: Durations, increments: int, config: Config
    ):
    ...
    def _start_timer(self):
        timer_iterator = tqdm(
            ...
            unit=" seconds",
            bar_format="%s{l_bar}%s%s{bar}%s%s{r_bar}%s"
            % (
                display.color_map(self.config.theme_color),
                Fore.RESET,
                display.color_map(self.bar_color),
                Fore.RESET,
                display.color_map(self.config.theme_color),
                Fore.RESET,
            ),
...
class Sylo:
    ...
    def show_task(self, dates, with_id: bool = False):
        for date in dates:
            ...
            if with_id is False:
                display.print_incomplete_tasks(self.tasks)
            else:
