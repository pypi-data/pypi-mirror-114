...
class Timer:
    def __init__(
        ...
        self.tasks = database.select(
            ...
            f'"effort",'
            f'"complete"'
            f"FROM tasks WHERE \"created_at\" = '{get_today()}' "
            f";"
        )
...
class Insights:
    ...
    def print_busy_days(self):
        ...
        businest_day_completed = database.select(query.format("completed_at"))
        self.display.print_busy_days_header()
        self.display.print_busy_days(
            busiest_day_created[0][0], businest_day_completed[0][0]
        )
    ...
    def show_task(self, dates, with_id: bool = False):
        for date in dates:
            ...
            self.tasks = database.select(
                ...
                f'"effort",'
                f'"complete"'
                f"FROM tasks WHERE \"created_at\" = '{date}'"
                f";"
            )
            if with_id is False:
