...
class Display:
    ...
    def print_timer(self, mode: str, timer_val: int, tasks: List = None):
        if mode == "work":
            ...
            print(
                self.col(f"Tasks for {calendar.day_name[get_weekday()]} {get_today()}")
            )
            print(self.col("-----------------------------------------"))
            ...
            self.print_header_small()
            print(self.col(f"{calendar.day_name[get_weekday()]} {get_today()}"))
            print(
                f"\n{self.col('Press')} {self.col('Ctrl-C', 'yellow')} "
                f"{self.col('to stop timer early.')}"
                f"\n{self.col('REST', 'green')} "
    ...
    def print_insights(self, durations: Durations):
        print(
            f"\nTotal {self.col(self.add_plurality(self.pom_name), 'lmagenta')} today:          "
            f"{self.col(str(int(durations.poms)), 'yellow')} {self.add_plurality(self.pom_name)}"
    ...
    def print_splash(self, durations: Durations, mode: str, tasks: List):
        print(
            self.col(
                f"Tasks for today, {calendar.day_name[get_weekday()]} {get_today()}"
            )
        )
        print(self.col("-----------------------------------------"))
        self.print_tasks(tasks)
        print(
            f"\n{self.col('Upcoming timer: ')}"
    ...
    def print_splash_variants(
        ...
        self.print_header_small()
        self.print_splash(durations, mode, tasks)
        ...
        complete_tasks = "\n".join(
            "".join(
                map(
                    ...
                    f"{self.trim_string(self.col(strike(t[2]),'green'),65)}",
                )
            )
            for t in tasks
            if t[4] == True
    ...
    def print_incomplete_tasks(self, tasks: List = None):
        incomplete_tasks = "\n".join(
            "".join(
                map(
                    str,
    ...
    def print_tasks_with_id(self, tasks: List = None):
        complete_tasks = "\n".join(
            "".join(
                map(
                    str,
                    f"{self.col('--')} {self.col(t[0], 'yellow')}  "
                    f"{self.print_poms(t[3])}: "
                    f"{self.trim_string(self.col(strike(t[2]),'green'),65)}",
                )
            )
