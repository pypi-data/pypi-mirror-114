...
class Timer:
    def __init__(self, mode: str, durations: Durations, increments: int):
        ...
        self.tasks = database.select(
            ...
            f'"effort",'
            f'"complete"'
            f"FROM tasks WHERE \"task_date\" = '{get_today()}' "
            f";"
        )
...
class Sylo:
    ...
    def show_task(self, dates, with_id: bool = False):
        ...
        return [i[0] for i in self.tasks]

    def task_menu_new(self, dates: List, task: Task):
    ...
    def task_menu_move(self, dates: List):
        clear_screen()
        display.print_header_small()
        task_ids = self.show_task(dates, True)
        display.print_edit_task()
        ...
        while valid_input(comp_input, task_ids) is False:
            ...
            self.task_menu(dates)
        database.execute(
            f"UPDATE tasks SET \"task_date\" = '{get_today()}'"
            f' WHERE "id" = {comp_input};'
        )
        self.task_menu(dates)

    def task_menu_delete(self, dates: List):
        clear_screen()
        display.print_header_small()
        task_ids = self.show_task(dates, True)
    ...
    def task_menu(self, dates: List):
        ...
        while usr_input.lower() == "n":
            self.task_menu_new(dates, task)
        while usr_input.lower() == "c":
            self.task_menu_close(dates)
        while usr_input.lower() == "m":
            self.task_menu_move(dates)
        while usr_input.lower() == "d":
            self.task_menu_delete(dates)
