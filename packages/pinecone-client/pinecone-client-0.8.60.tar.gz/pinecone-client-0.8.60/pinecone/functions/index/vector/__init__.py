...
class VectorIndex(Index):
    ...
    def downcast_data(data: np.ndarray) -> np.ndarray:
        if data.dtype == np.float32:
            return data
        if not np.can_cast(data.dtype, np.float32, casting='same_kind'):
            raise TypeError(f"Data type must be float32 compatible (float64, int64, short, etc.). Given: {data.dtype}")
