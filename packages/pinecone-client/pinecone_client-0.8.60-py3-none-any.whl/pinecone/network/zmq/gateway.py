...
from pinecone.network.zmq import SocketWrapper
...
class GatewayServlet(ZMQServlet):
    ...
    async def poll_traceroute(self) -> AsyncIterable['core_pb2.TraceRoute']:
        ...
        while True:
            yield await self.recv_traceroute(control_sock.socket())

    async def recv_traceroute(self, sock: SocketWrapper) -> 'core_pb2.TraceRoute':
        msg = await sock.recv()
