...
class RedisConfig:
  ...
  def strict_redis(self):
    if not hasattr(self, "_strict_redis"):
      self._strict_redis = redis.StrictRedis(host=self._redis_host, port=self._redis_port)
    return self._strict_redis

  @property
  def redis_config(self):
    if not hasattr(self, "_redis_config"):
      self._redis_config = redis.Redis(host=self._redis_host, port=self._redis_port, db=self._config_db)
      self._redis_config.set('_is_config', 1)
      if self._redis_config.get(self._allocated_dbs_key) is None:
        self._redis_config.set(self._allocated_dbs_key, json.dumps({'self': self._config_db}))
  ...
  def get_redis_allocated_db(self, db):
    if db not in self._redis_dbs:
      self._redis_dbs[db] = redis.Redis(host=self._redis_host, port=self._redis_port, db=db)
    return self._redis_dbs[db]

  def get_unused_dbs(self):
    possible_dbs = range(0,15)
    return list(set(possible_dbs) - set([int(i) for i in self.redis_db_allocations.values()]))
