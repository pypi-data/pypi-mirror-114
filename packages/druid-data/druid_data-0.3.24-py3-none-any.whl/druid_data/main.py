...
from inspect import isclass, getmembers, ismethod
    ...
    def get_instance(
        ...
        if cls._instance is None or overwrite_existing_instance:
            ...
            if access_params is None:
                ...
                logging.warning(
                    "The provided access_params " + str(access_params) + " is missing required values, trying"
...
class DynamoCrudRepository(CrudRepository, metaclass=abc.ABCMeta):
    ...
    def find_collection(
        ...
        if key_condition_expression is not None:
            ...
            keys_tuple = self.keys2KeyConditionExpression(keys, ordered_key_names, sk_comparator)
            query_args["KeyConditionExpression"] = keys_tuple[0]
            query_args["ExpressionAttributeValues"] = keys_tuple[1]
        else:
            raise TypeError("missing argument, need to provide either 'key_condition_expression' or 'key' argument")
        ...
        try:
            ...
            logging.warning("Not able to query table " + str(self.table_name) +
                            " with parameters" + str(query_args))
            return None
    ...
    def _id2key_pair(self, unique_id):
        ...
        log_dict = {
            "Level": "[FINE]",
            "method": "DynamoCrudRepository._id2key_pair",
    ...
    def item_attr2_instance_attr(self, item_attr_val, instance_attr_name: str, instance_attr_type: type):
        ...
        elif issubclass(instance_attr_type, (bytes, bool, UUID)):  # Perform a direct conversion
    ...
    def keys2KeyConditionExpression(
        ...
        if len(ordered_key_names) > 1:
            ...
            if sk_comparator == SortKeyComparator.EQUALS:
                ...
                buffer += "begins_with ( " + str(key_name) + ", :" + str(key_name) + "val )"
    ...
    def instance_attr2_item_attr(self, instance_attr_val, instance_attr_name: str):
        ...
        # switch self.map_dict[<attribute_name>]
        ...
        elif self.map_dict[instance_attr_name][ConstantString.ITEM_ATTR_TYPE.value] == "BS":
        ...
        elif self.map_dict[instance_attr_name][ConstantString.ITEM_ATTR_TYPE.value] == "NS":
            for old_value in instance_attr_val:
                if not isinstance(old_value, Decimal):
                    new_value = DynamoConversions.get_decimal_context().create_decimal(old_value)
                    instance_attr_val.remove(old_value)
                    instance_attr_val.add(new_value)
            item_attr_val = set(instance_attr_val)

        elif self.map_dict[instance_attr_name][ConstantString.ITEM_ATTR_TYPE.value] == "SS":
            for old_value in instance_attr_val:
            ...
            # sets and tuples (cast to list and converted to json)
                ...
                item_attr_val = json.dumps(list(instance_attr_val), cls=DynamoConversions.get_custom_encoder())

            # maps and lists (converted to json)
            elif isinstance(instance_attr_val, (dict, list)):
                item_attr_val = json.dumps(instance_attr_val, cls=DynamoConversions.get_custom_encoder())
    ...
    def instance2item_params_inject_attributes(self, obj: T, item_params: dict = None):
        ...
        for instance_attr in getmembers(obj):
            if (
                ...
                if (instance_attr_val is not None and
                    not (hasattr(instance_attr_val, '__len__') and len(instance_attr_val) < 1)):
    ...
    @classmethod
    def dynamo_type_from_type(cls, python_type: type, custom_conversions: list=None):
        """
        ...
        if not isclass(python_type):  # Try to get the class type
            if hasattr(python_type, "__supertype__"):  # Try to get the class type from typings.NewType
                ...
                if (hasattr(python_type, "__args__") and issubclass(python_type.__origin__, (set, frozenset))
                        and issubclass(python_type.__args__[0], (bytes, bytearray, str, int, float, Decimal))):
                    ...
                    if issubclass(python_type.__args__[0], bytes):
                        if custom_conversions is not None:
                            custom_conversions.insert(0, DynamoConversions.bytes_set_to_dynamo)
                            custom_conversions.insert(1, DynamoConversions.bytes_set_from_dynamo)
                        return "BS"
                    elif issubclass(python_type.__args__[0], bytearray):
                        if custom_conversions is not None:
                            custom_conversions.insert(0, DynamoConversions.bytes_set_to_dynamo)
                            custom_conversions.insert(1, DynamoConversions.bytearray_set_from_dynamo)
                        return "BS"
                    elif issubclass(python_type.__args__[0], str):
                        if custom_conversions is not None:
                            custom_conversions.insert(0, DynamoConversions.string_set_to_dynamo)
                            custom_conversions.insert(1, DynamoConversions.identity)
                        return "SS"
                    elif issubclass(python_type.__args__[0], int):
                        if custom_conversions is not None:
                            custom_conversions.insert(0, DynamoConversions.number_set_to_dynamo)
                            custom_conversions.insert(1, DynamoConversions.int_set_from_dynamo)
                        return "NS"
                    elif issubclass(python_type.__args__[0], float):
                        if custom_conversions is not None:
                            custom_conversions.insert(0, DynamoConversions.number_set_to_dynamo)
                            custom_conversions.insert(1, DynamoConversions.float_set_from_dynamo)
                        return "NS"
        ...
        if issubclass(python_type, Enum):
            if custom_conversions is not None:
                ...
                custom_conversions.insert(1, DynamoConversions.gen_enum_from_dynamo_for_enum_type(python_type))
            ...
            for fl in fls:
                ...
                if fl.name not in self.entity_type._dynamo_ignore():
                    if ConstantString.ITEM_ATTR_TYPE.value not in self.map_dict[fl.name]:
                        ...
                        custom_conversions = []
                        attrib_type = self.dynamo_type_from_type(fl.type, custom_conversions=custom_conversions)
                        self.map_dict[fl.name][ConstantString.ITEM_ATTR_TYPE.value] = attrib_type
                        if len(custom_conversions) >= 2:
                            self.map_dict[fl.name][ConstantString.OBJ2ITEM_CONV.value] = custom_conversions[0]
                            self.map_dict[fl.name][ConstantString.ITEM2OBJ_CONV.value] = custom_conversions[1]
