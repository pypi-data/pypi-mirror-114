...
def readyaml(filename):
	...
	def yaml_include(loader, node):
		...
		if os.path.isfile(node.value):
			f=node.value
			with open(node.value, 'r') as f: subdata=ruamel.yaml.load(f, Loader=ruamel.yaml.Loader)
		else:
	...
	if data.get("yaml-include"):
		for f in data["yaml-include"]:
			if os.path.isfile(f):
				with open(f, 'r') as f: subdata=ruamel.yaml.load(f, Loader=ruamel.yaml.Loader)
				data.update(subdata)
			else:
				path=os.path.dirname(filename)
