...
class CarbonBoxModel:
    ...
    # def production_function_generator(self, data, time):
    #     mse = np.inf
    #     troposphere_index = None
    #     for index, node in self._nodes.items():
    #         if node.get_name() == 'Troposphere':
    ...
    #         raise ValueError('there is currently no Troposphere node to equilibrate!')
    #
    #     equilibrium = self.equilibrate(production_rate=self.equilibrate(target_C_14=data[0]))
    #     production_terms = self._reverse_pass(data, time, troposphere_index, equilibrium=equilibrium)
    #
    #     while mse > 0.001:
    #         forwards = self._forward_pass(time, equilibrium, production_terms)
    #         mse = np.sum((forwards[troposphere_index, :] - data) ** 2)
    #         production_terms = self._reverse_pass(data, time, troposphere_index, first_pass=False,
    #                                               box_contents=forwards)
    #         print(mse)
    #
    #     return production_terms
    #
    # def _forward_pass(self, time, equilibrium, production_rates):
    #     production_fun = (lambda x: (jnp.interp(x, time, production_rates)))
    #     binned_data = self.run_bin(time, 1000, production_fun, y0=equilibrium)
    #     return binned_data
    #
    # def _reverse_pass(self, troposphere_content, time, troposphere_index, first_pass=True, box_contents=None,
    #                   equilibrium=None):
    #     production_terms = []
    #     time_step = time[1] - time[0]
    #     if first_pass:
    #         for i in range(len(time) - 1):
    #             new_y = np.concatenate([equilibrium[:troposphere_index], np.array([troposphere_content[i]]),
    #                                     equilibrium[troposphere_index + 1:]])
    #             production_term = (troposphere_content[i + 1] - troposphere_content[i]) / time_step \
    #                               - self._matrix[troposphere_index, :] * new_y
    #             production_terms.append(production_term / 0.3)
    #     else:
    #         for i in range(len(time) - 1):
    #             new_box_contents = box_contents.copy()
    #             new_box_contents[troposphere_index, :] = troposphere_content
    #             # new_y = box_contents[i,:troposphere_index] + troposphere_content[i] + \
    #             #         box_contents[i,troposphere_index+1:]
    #             # print(new_y)
    #             production_term = (troposphere_content[i + 1] - troposphere_content[i]) / time_step \
    #                               - self._matrix[troposphere_index, :] * new_box_contents[:, i]
    #             production_terms.append(production_term / 0.3)
    #     production_terms.append(0)
    #     return production_terms
