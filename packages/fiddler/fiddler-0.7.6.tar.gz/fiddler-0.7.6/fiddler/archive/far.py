...
from fiddler.archive.build_container import build_cmd, run_cmd
...
def parse_args(argv=None):
    ...
    common_args.add_argument(
        '--api-endpoint', default='http://api.fiddler.ai', help=argparse.SUPPRESS
    )
        ...
        try:
            ...
            if hasattr(args, 'auth_key') and not args.auth_key:
                ...
                if not args.auth_key:
                    logging.warning(
                        'Auth key is not available. ' 'The requests will likely fail.'
                    )
