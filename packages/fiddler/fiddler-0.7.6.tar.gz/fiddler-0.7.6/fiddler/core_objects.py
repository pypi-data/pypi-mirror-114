            ...
            if not target_column.data_type.is_valid_target():
                ...
                output_columns = [
                    Column(
                        ...
                        data_type=DataType.FLOAT,
                        is_nullable=False,
                        # by default, we assume that the output min/max will match the target min/max
                        value_range_min=target_column.value_range_min,
                        # by default, we assume that the output min/max will match the target min/max
                        value_range_max=target_column.value_range_max,
                    )
                ]
                if model_task is None:
                    model_task = ModelTask.REGRESSION
