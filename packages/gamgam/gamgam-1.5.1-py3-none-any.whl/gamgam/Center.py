...
class SceneManager:
    ...
    def init(self):
        if scene is not None:
            self.scene_now = scenes[now]
            self.object_now = GameObjects[now]
            display.init()
            ...
            for i in self.ObjectWaiting:
                GameObject[now].append(i)
            for i in self.ComponentWaiting:
                ComponentWaiting[now].append(i)
            self.ObjectWaiting = []
            self.ComponentWaiting = []
            self.scene_now = scenes[now]
            self.object_now = GameObjects[now]
    ...
    def Update(self):
        self.scene_now = scenes[now]
        self.object_now = GameObjects[now]
        self.Screen.fill(screen.color)
        if self.scene_now.background is not None:
            self.Screen.blit(self.scene_now.background.img, (0, 0))
        for i in self.ObjectWaiting:
            i.Start()
            self.GameObjects[now].append(i)
        for i in self.ComponentWaiting:
            i.set()
            self.GameComponents[i.name] = i
        self.ObjectWaiting = []
        self.ComponentWaiting = []
        self.GameObjects[now].sort(key = lambda x : x.order)
        self.scene_now = scenes[now]
        self.object_now = GameObjects[now]
        for i in self.GameComponents.values():
            i.reflect()
        for i in self.GameObjects[now]:
    ...
    def add(self,screen: PlayScreen, name:str = f"screen{len(self.scenes) + 1}"):
    ...
    def new_object(self, objects, scene : str = self.now):
        if self.now == scene:
    ...
    def del_object(self, objects, scene : str = self.now):
...
global Input, default_icon, SoundManager, Time, fps_limit, Frame
Input = InputManager()
