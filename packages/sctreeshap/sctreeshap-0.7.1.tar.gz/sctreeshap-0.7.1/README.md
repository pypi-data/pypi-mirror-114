...
In v0.7.0 and v0.7.1, we mainly work for supporting probability output for shap values. This is not stable and could not deal with a large feature set. Therefore, use geneFiltering() before setting output as 'probability'.
...
sample.explainBinary(sample_dataset, 'Micro L1-3 TYROBP', shap_params = {
    "output": 'probability', # set output mode as 'probability'
    "bar_plot" : True,
    "beeswarm" : True,
    "decision_plot" : True
})
...
We will add the new feature to documentations in the later updates.
