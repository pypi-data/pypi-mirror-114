...
class sctreeshap:
    ...
    def explainBinary(self, data=None, cluster_name=None, use_SMOTE=False, nthread=32, shap_params=None):        
        ...
        if shap_params["output"] == 'raw':
            ...
            print("\033[1;33;40mWarning:\033[0m The core may be dumped if the number of features is too large.")
    ...
    def explainMulti(self, data=None, use_SMOTE=False, nthread=32, shap_params=None):
        ...
        if shap_params["output"] == 'raw':
            ...
            y_test = [np.array(y_test) for i in range(self.numOfClusters)]
            y_train[0][y_train[0] == 0] = -1
            y_train[0][y_train[0] > 0] = False
            y_train[0][y_train[0] < 0] = True
            y_test[0][y_test[0] == 0] = -1
            y_test[0][y_test[0] > 0] = False
            y_test[0][y_test[0] < 0] = True
            for i in range(1, self.numOfClusters):
                y_train[i][y_train[i] != i] = False
                y_train[i][y_train[i] == i] = True
                y_test[i][y_test[i] != i] = False
                y_test[i][y_test[i] == i] = True
            self.__isFinished = True
            thread_preprocessData.join()
            time.sleep(0.2)
            for key in self.clusterDict.keys():
            ...
            # Cross validation
            y_pred = [self.__XGBClassifer[i].predict(x_test) for i in range(self.numOfClusters)]
            accuracy = np.mean(np.array([np.sum(y_pred[i] == y_test[i]) / (len(y_pred[i])) for i in range(self.numOfClusters)])) * 100
            print("Accuracy: %.4g%%" % accuracy)
            time.sleep(0.2)
