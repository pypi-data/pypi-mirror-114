...
class SQLCursor(Thread):
    ...
    def execute(self, query_str, execute=False, handler=None, buffer=1000, csv_path=None, csv_replace=False,
                delimiter=',', quotechar='"', quoting=QUOTE_ALL, **kwargs):
        """
    ...
    def __stream_dataset(self, dataset, handler, buffer, csv_path, delimiter, quotechar, quoting, **kwargs):
        try:
            if dataset:
                if hasattr(dataset, 'description') and dataset.description:
                    cols = [column[0] for column in dataset.description]
    ...
    @staticmethod
    def __handle_buffer(row_num, cols, buffer_list, handler, csv_path, delimiter, quotechar, quoting, **kwargs):
        if buffer_list:
