...
class IntrinsicCalibSession(CalibrationSession, ABC):
    def __init__(self, camera, intrinsic_calib, path='CalibrationImages', no=5):
        # Set camera
        self.camera = camera
        self.path = path
        self.no = no
    ...
    def capture(self,path='Intrinsic'):
        ...
        for i in range(self.no):
            ...
            self.camera.getImage(name= path + '/'+ filename, calibration=True)
            print("Alternate the viewing direction of the camera..." + str(i))
