...
class Internal(Camera, ABC):
    ...
    def getImage(self, path="", saveImage=True, saveNumpy=True, calibration=False, timeout=5000):
        if not os.path.exists('CalibrationImages/' + path):
            os.mkdir('CalibrationImages/' + path)
        if not os.path.exists('CapturedImages/' + path):
            os.mkdir('CapturedImages/' + path)
        if not os.path.exists('CalibrationNumpyData/' + path):
            os.mkdir('CalibrationNumpyData/' + path)
        if not os.path.exists('CapturedNumpyData/' + path):
            os.mkdir('CapturedNumpyData/' + path)
        ...
        if saveImage:
            if calibration:
                cv2.imwrite('CalibrationImages/' + path + '.PNG', cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            else:
                cv2.imwrite('CapturedImages/' + path + '.PNG', cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
        if saveNumpy:
            if calibration:
                np.save('CalibrationNumpyData/' + path, cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            else:
                np.save('CapturedNumpyData/' + path, cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
        return frame
