...
def ginds2syminds(gsim, inds):
    """Convert global simulation indexes to indexes into the sub simulation
    after application of symmetries.
    """

    sym_inds = np.copy(inds)
    keep_inds = np.ones((sym_inds.shape[0],), dtype=bool)
    for d, sym in enumerate(gsim.symmetries):
        Nd = gsim.grid.Nxyz[d]
        if sym != 0:
            sym_inds[:, d] -= Nd // 2
            keep_inds *= sym_inds[:, d] >= 0
        if sym == 1:
            sym_inds[:, d] += 1

    return sym_inds[keep_inds, :], keep_inds


def syminds2ginds(gsim, inds):
    """Convert indexes of sub simulation after application of symmetries to
    global simulation indexes.
    """
