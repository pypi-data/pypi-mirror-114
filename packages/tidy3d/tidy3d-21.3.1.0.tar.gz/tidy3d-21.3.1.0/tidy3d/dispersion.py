...
class DispersionModel(object):
    ...
    def __init__(self, eps_inf = 1.0, poles = None):
        ...
        poles : None, optional
            A list of the (a, c) coefficients, specified here in rad/s. Added
            isotropically to all three components.
        """
        self._eps_inf = eps_input(eps_inf, "eps_inf")
    ...
    def epsilon(self, freqs=None):
        ...
        -------
        array_like
            The permittivity values, shape ``Nfreqs, 3``.
        """
        if freqs is None:
            return self._eps_inf[None, :]

        w = 2*np.pi*np.array(freqs)
        w = w.reshape((w.size, 1))
        eps = np.tile(self._eps_inf.astype(complex_), (w.size, 1))
        for (a,c) in self._poles:
            pole_term = c/(1j*w-a) + c.conjugate()/(1j*w-a.conjugate())
            eps += pole_term
        return eps
...
class Lorentz(DispersionModel):
    ...
    def __init__(self, eps_inf, coeffs, name=None):
        ...
        """
        eps_inf = eps_input(eps_inf, "eps_inf")
        poles = []
        self.name = name
        for c in coeffs:
            w = 2*math.pi*c[1]
