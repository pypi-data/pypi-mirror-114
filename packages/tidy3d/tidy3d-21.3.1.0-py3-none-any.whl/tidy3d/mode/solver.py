...
def compute_modes(eps_cross, freq, mesh_step, pml_layers, num_modes=1,
                    target_neff=None, symmetries=(0, 0)):
    ...
    # Derivative matrices with Dirichlet ("hard") boundaries
    Dmats = D_mats((Nx, Ny), mesh_step[:2], dmin_pmc)

    # PML matrices; do not impose PML on the bottom when symmetry present
    dmin_pml = np.array(symmetries) == 0
    Smats = S_mats(omega, (Nx, Ny), pml_layers, mesh_step[:2], dmin_pml)
    ...
    # plt.imshow(np.abs(eps_zz).T)
    # print(np.amax(np.abs(eps_zz)))

    # print(eps_yy.flatten()[:10])
