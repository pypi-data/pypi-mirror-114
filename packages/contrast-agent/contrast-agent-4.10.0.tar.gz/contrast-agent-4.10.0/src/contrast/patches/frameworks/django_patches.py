...
def new_file_patch(orig_func, patch_policy, *args, **kwargs):
    """
    Patch for TemporaryFileUploadHandler.new_file

    Uploaded files that exceed the FILE_UPLOAD_MAX_MEMORY_SIZE threshold are streamed
    from a temporary file on disk rather than loaded into memory.
    ...
    if args and hasattr(args[0], "file"):
        try:
            # This also marks the stream as a source
            # While ContrastFileProxy was originally intended for file objects in Py27,
            # it also meets our needs for proxying the TemporaryUploadedFile stream.
            # Eventually we may decide to use a different proxy class, or at least a
            # different name (or change the docstring in ContrastFileProxy).
...
def patch_django_uploadhandler(uploadhandler_module):
    patch_cls_or_instance(
        uploadhandler_module.TemporaryFileUploadHandler, "new_file", new_file_patch
    )
