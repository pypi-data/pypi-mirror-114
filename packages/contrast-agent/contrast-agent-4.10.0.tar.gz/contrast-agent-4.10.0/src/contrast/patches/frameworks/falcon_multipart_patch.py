...
def parse_patch(orig_method, patch_policy, *args, **kwargs):
    """
    Deadzone for falcon_multipart.middleware.MultipartMiddleware.parse

    This method can cause an enormous amount of propagation to occur, which can cause
    requests to be extremely slow. This is because this middleware parses all of the
    multipart form data into memory on the request object. We deadzone this propagation
    in order to greatly improve performance. The parse_field method patch below ensures
    that we still track the relevant data.
...
def parse_field_patch(orig_method, patch_policy, self, field, **kwargs):
    """
    Deadzone and source tracker for falcon_multipart.middleware.MultipartMiddleware

    First we deadzone the call to the original method in order to prevent unnecessary
    propagation and improve performance. Next, we create sources for the result so that
    we don't miss any input data.
...
def patch_falcon_multipart(module):
    ...
    patch_cls_or_instance(middleware_cls, "parse", parse_patch)
    patch_cls_or_instance(middleware_cls, "parse_field", parse_field_patch)
