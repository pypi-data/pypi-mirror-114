...
class ContrastEvent(_TriggerTimeMixin):
    ...
    def __init__(
        ...
        source_type=None,
        source_name=None,
        all_other_events=None,  # only needed when creating a dataflow trigger event
    ):
        self.node = node
        self.tagged = tagged
        ...
        self.request = None
        self._set_request(all_other_events)
    ...
    def is_source_event(self):
        """
        A ContrastEvent is a source event if its node is a SourceNode
        :return: bool
        """
        return isinstance(self.node, SourceNode)

    @property
    def is_trigger_event(self):
        """
        A ContrastEvent is a trigger event if its node is a TriggerNode
        :return: bool
        """
        return isinstance(self.node, TriggerNode)
    ...
    def _set_request(self, all_other_events):
        """
        Set request for source and trigger events according to business logic.
        Propagation events don't need a reference to the request.
        """
        context = contrast.CS__CONTEXT_TRACKER.current()

        if context and (self.is_source_event or self.is_trigger_event):
            # Source events must always created within a request
            # If a trigger is hit during a request, use this request
            self.request = context.request
            return

        if self.is_trigger_event and all_other_events:
            # 2. If a trigger is hit outside a request, use the request
            # for the most recent source.
            source = [event for event in all_other_events if event.is_source_event][-1]
