...
class ConfigManager:
    ...
    def project(self):
        if 'project' in self.overriden_config:
            return self.overriden_config['project'].lower()
        
        if 'project' in self.local_config:
            return self.local_config['project'].lower()

        if 'DSO_PROJECT' in os.environ:
            return os.getenv('DSO_PROJECT').lower()

        if 'project' in self.merged_config:
            return self.merged_config['project'].lower()
    ...
    def application(self):
        ...
        if not result == 'default' and self.project == 'default':
            Logger.debug(f"Application context '{result}' was ignored because the global context is being used.")
            result = 'default'
