...
class Pyrat(object):
  ...
  def radiative_equilibrium(self, nsamples=None):
      """
      ...
      temp = np.zeros((nsamples+1, atm.nlayers))
      fscale0 = 1.0e5  # Initial temperature scale factor
      maxf = 3.0e6  # Maximum temperature scale factor
      if not hasattr(atm, '_fscale'):
          atm._fscale = fscale = np.tile(fscale0, atm.nlayers)
      else:
          fscale = atm._fscale
      Fsign = np.tile(2.0, atm.nlayers)
      sigma = 1.0
      # Starting point:
      temp[0] = np.copy(atm.temp)
      dpress = np.ediff1d(np.log(atm.press), to_begin=1.0)

      Fint = np.tile(pc.sigma*phy.tint**4, atm.nlayers)
      f_int = ps.blackbody_wn(spec.wn, phy.tint)
      spec.f_int = pc.sigma * phy.tint**4 * f_int / np.trapz(f_int,spec.wn)

      new_fscale = fscale
      sigma = 1.0
      no_conv = True

      rad_temp = np.zeros((nsamples,nlayers))
      rad_temp[0] = np.copy(temp[0])
      ...
      for k in range(nsamples):
          ...
          Qup = np.trapz(spec.Fup, spec.wn, axis=1)
          Qdown = np.trapz(spec.Fdown, spec.wn, axis=1)
          dQ = Qup - Qdown - Fint
          ...
          rad_temp[k+1] = atm.temp + dT
          rad_temp[k+1,0] = rad_temp[k+1,1]  # Isothermal top

          # Smooth out kinks and wiggles:
          dtm = np.mean(np.abs(np.diff(rad_temp[:k+1],n=1,axis=0)),axis=1)
          if np.size(dtm) > 0:
              sigma = np.clip(0.5*np.mean(dtm[-5:]), 0.5, 1.0)
          rad_temp[k+1,:-1] = gaussf(rad_temp[k+1], sigma)[:-1]
          rad_temp[k+1] = np.clip(rad_temp[k+1], tmin, tmax)
          if no_conv:
              Fsign = np.sign(dF)
              fscale = new_fscale
              temp[k+1] = np.copy(rad_temp[k+1])
              continue

          # Convection:
          # TBD: convective_flux
          F_conv = convective_flux(pyrat, atm.chem_model, rad_temp[k+1])
          dF = np.ediff1d(dQ+F_conv, to_begin=0)
          # Update scaling factor:
          idiff = np.sign(dF) != Fsign
          fscale[ idiff] *= 0.90
          fscale[~idiff] *= 1.05
          fscale = np.clip(fscale, 0, maxf)
          Fsign = np.sign(dF)
          # Adaptive temperature update:
      ...
      #atmfile = os.path.basename(self.atm.atmfile)
      #atmfile = 'radeq_' + atmfile.strip('radeq_')
      atmfile = spec.specfile.replace('.dat', '.atm')
      io.write_atm(
          atmfile, self.atm.press, self.atm.temp,
          self.mol.name, q, punits="bar",
          header="# Radiative-thermochemical equilibrium profile.\n\n")

      self.log.verb = self.verb
      self.atm.radeq_temp = temp
