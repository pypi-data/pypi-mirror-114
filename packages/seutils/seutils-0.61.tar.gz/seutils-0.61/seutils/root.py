...
def get_command(cmd_name, implementation=None, needs_xrd=False):
    """
    ...
    if implementation is None:
        for module in [_uproot, pyroot]:
            if module.is_installed() and hasattr(module, cmd_name):
                if module is _uproot and needs_xrd and not(_uproot.pyxrd_installed()):
                    # Can only use uproot for remote paths if the xrootd python bindings are there
                    continue
                break
        else:
            raise Exception(
                'No implementation for {} found. '
                'Do you have uproot or PyRoot installed? If you are using uproot for'
                ' a remote path, you will need the python bindings for XRootD as well'
