...
def get_my_logger(file_path, file_name, logger_name=__name__, **kwargs):
    ...
    # 设置日志文件输出，INFO级别
    info_rt_handler = RotatingFileHandler(filename=file_path + '.info', mode='a', maxBytes=100 * 1024 * 1024,
                                          backupCount=3, encoding='utf-8')
    info_rt_handler.setLevel(level=logging.INFO)

    # 设置日志文件输出，WARNING级别
    warnig_rt_handler = RotatingFileHandler(filename=file_path + '.warning', mode='a',
                                            maxBytes=100 * 1024 * 1024,
                                            backupCount=3, encoding='utf-8')
    warnig_rt_handler.setLevel(level=logging.WARNING)

    debug_rt_handler = RotatingFileHandler(filename=file_path + '.debug', mode='a', maxBytes=100 * 1024 * 1024,
                                           backupCount=3, encoding='utf-8')
    debug_rt_handler.setLevel(level=logging.DEBUG)

    error_rt_handler = RotatingFileHandler(filename=file_path + '.error', mode='a', maxBytes=100 * 1024 * 1024,
                                           backupCount=3, encoding='utf-8')
    error_rt_handler.setLevel(level=logging.ERROR)

    critical_rt_handler = RotatingFileHandler(filename=file_path + '.critical', mode='a', maxBytes=100 * 1024 * 1024,
                                              backupCount=3, encoding='utf-8')
    ...
    handler = CMRESHandler(hosts=[{'host': '192.168.111.112', 'port': 9200}], auth_type=CMRESHandler.AuthType.NO_AUTH,
