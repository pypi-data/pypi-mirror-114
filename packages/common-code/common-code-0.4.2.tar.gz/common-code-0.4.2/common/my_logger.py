...
def get_my_logger(file_path, file_name, logger_name=__name__, **kwargs):
    ...
    # 设置日志文件输出，INFO级别
    file_rt_info_handler = RotatingFileHandler(filename=file_path + '.info', mode='a', maxBytes=100 * 1024 * 1024,
                                               backupCount=3, encoding='utf-8')
    file_rt_info_handler.setLevel(level=logging.INFO)

    # 设置日志文件输出，WARNING级别
    file_rt_warning_handler = RotatingFileHandler(filename=file_path + '.warning', mode='a',
                                                  maxBytes=100 * 1024 * 1024,
                                                  backupCount=3, encoding='utf-8')
    file_rt_warning_handler.setLevel(level=logging.WARNING)

    file_rt_debug_handler = RotatingFileHandler(filename=file_path + '.debug', mode='a', maxBytes=100 * 1024 * 1024,
                                                backupCount=3, encoding='utf-8')
    file_rt_debug_handler.setLevel(level=logging.DEBUG)

    file_rt_error_handler = RotatingFileHandler(filename=file_path + '.error', mode='a', maxBytes=100 * 1024 * 1024,
                                                backupCount=3, encoding='utf-8')
