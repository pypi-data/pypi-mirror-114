...
import numpy as np
from math import *
from ExtractCard.ctpn_predict import get_det_boxes
from ExtractCard.ocr import img_to_text
...
def char_rec(img, text_recs, adjust=False):
    ...
    for index, rec in enumerate(text_recs):
        ...
        # xoay ảnh vào cắc từng trường
        part_img = rotate_image(img, degree, pt1, pt2, pt3, pt4)

        if (
            part_img.shape[0] < 1
            or part_img.shape[1] < 1
            or part_img.shape[0] > part_img.shape[1]
        ):
            continue

        # results.append(img_to_text(part_img))
...
def proc_img(image):
    ...
    # nhận diện , đầu vào là file tọa độ và hình ảnh ban đầu
    result = char_rec(image, text_recs)
