...
import ExtractCard.config as config
from ExtractCard.ctpn_utils import resize
from ExtractCard.ctpn_model import CTPN_Model
...
def get_det_boxes(image, display=True, expand=True):
    ...
    # quy định mảng hình ảnh float32
    image = image.astype(np.float32) - config.IMAGE_MEAN
    image = (
        torch.from_numpy(image.transpose(2, 0, 1)).unsqueeze(0).float()
    )  # tạo ra tensor từ ma trận chuyển vị
