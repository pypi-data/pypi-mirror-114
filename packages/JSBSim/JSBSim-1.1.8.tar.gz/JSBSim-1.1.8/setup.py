...
if 'sdist' not in dist.commands and compiler.find_library_file([library_path],
    ...
    # the sources.
    if compiler.compiler_type == 'msvc':
        compile_flags = ['/D'+flag for flag in ['_USE_MATH_DEFINES', 'NOMINMAX', 'JSBSIM_VERSION="1.1.8 [GitHub build 588/commit c943f83deeb3e14bed7939ac65dfac789a7a0181]"','HAVE_EXPAT_CONFIG_H',]]
        if sys.version_info < (3,9):
            compile_flags = [flag.replace('"', '\\"') for flag in compile_flags]
    else:
        compile_flags = ['-D'+flag for flag in ['JSBSIM_VERSION="1.1.8 [GitHub build 588/commit c943f83deeb3e14bed7939ac65dfac789a7a0181]"','HAVE_EXPAT_CONFIG_H',]]
