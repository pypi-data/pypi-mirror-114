...
from sdmn import display
from sdmn import __version__
from sdmn.configuration import Configuration

logger = logging.getLogger('sdmn')
docker_url = None


@click.group(help=('SmartDex MasterNode (sdmn) is a cli tool to help you run a SmartDex Masternode'
                   'chain masternode'))
...
@click.command(help='Display SmartDex documentation link')
def docs() -> None:
    "Link to the documentation"
    display.link_docs()


@click.command(help='Start your SmartDex masternode')
...
@click.command(help='Stop your SmartDex masternode')
def stop() -> None:
...
@click.command(help='Show the status of your SmartDex masternode')
def status() -> None:
...
@click.command(help='Show details about your SmartDex masternode')
def inspect() -> None:
    "Show details about the smartdex masternode"
    configuration = Configuration(docker_url=docker_url)
    if configuration.force_recreate:
        display.error_breaking_change()
        sys.exit('\n')
    display.title_inspect_masternode(configuration.name)
    identity = configuration.services['smartdex'].execute(
        'echo $IDENTITY'
    ) or 'container not running'
    display.detail_identity(identity)
    display.newline()
    coinbase = configuration.services['smartdex'].execute(
        'sdx account list --keystore keystore 2> /dev/null | head -n 1 | cut '
        '-d"{" -f 2 | cut -d"}" -f 1'
    )
    if coinbase:
...
def update() -> None:
    "Update the smartdex masternode with the lastest images"
    configuration = Configuration(docker_url=docker_url)
    if configuration.force_recreate:
        display.error_breaking_change()
        sys.exit('\n')
...
@click.command(help='Remove your SmartDex masternode')
