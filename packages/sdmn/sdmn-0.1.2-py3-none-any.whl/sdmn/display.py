...
help_url = 'https://docs.swapdex.net/masternode/sdmn/'
...
def warning_ignoring_start_options(name: str) -> None:
    "Custom warning when sdmn is ignoring the start options"
    warning(
...
def warning_remove_masternode(name: str) -> None:
    "Custom warning when sdmn is removing masternode but no confirmation"
    warning(
...
def error_start_not_initialized() -> None:
    "Custom error when `sdmn start` has never been used with `--name` option"
    error(
        'sdmn doesn\'t manage any masternode yet\n'
        '         please use '
        '<hy>sdmn start --name</hy> to get started'
    )


def error_start_option_required(option: str) -> None:
    "Custom error when `sdmn start` is used with name but not the other options"
    error(
