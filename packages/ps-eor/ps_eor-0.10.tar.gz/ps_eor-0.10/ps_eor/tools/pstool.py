...
def do_ml_gpr(i_cube, noise_cube, ml_gpr_config, save_dir='.', save_name=None, rnd_seed=1):
    from ps_eor import ml_gpr

    # Saving PS stacker results for the residual, the noie
    print('Running ML-GPR for Stokes I ...\n')
    t = time.time()

    fitter = ml_gpr.MLGPRForegroundFitter(ml_gpr_config)

    ml_gpr_res = fitter.run(i_cube, noise_cube, save_dir=save_dir, save_name=save_name, verbose=True)
...
def run_ml_gpr(file_i, file_v, ml_gpr_config_file, flag_config, eor_bins_list, ps_conf,
    ...
    for eor_name in ps_builder.eor_bin_list.get_all_names():
        ...
        ml_gpr_res = do_ml_gpr(i_cube_slice, n_cube_slice, ml_gpr_config, save_dir=out_dir,
                               save_name='ml_gpr_res_I', rnd_seed=rnd_seed)
        ...
        ml_gpr_res.sampler.plot_corner(ml_gpr_config.mcmc.n_burn).savefig(
            os.path.join(plot_out_dir, 'mcmc_corner.pdf'))
        ml_gpr_res.sampler.plot_samples().savefig(os.path.join(plot_out_dir, 'mcmc_samples.pdf'))
        ml_gpr_res.sampler.plot_samples_likelihood(n_burn=ml_gpr_config.mcmc.n_burn
                                                   ).savefig(os.path.join(plot_out_dir, 'mcmc_samples_likelihood.pdf'))
...
def simu_uv(instru, z, out_filename, umin, umax, dec_deg, fov, bandwidth, df, total_time, int_time, res):
    ...
    telescop = obssimu.TelescopeSimu.from_name(instru, freqs, dec_deg, 0, total_time, umin, umax, int_time)
    ...
    uv_simu = telescop.simu_gridded_uv(uu_meter, vv_meter, fov, osamp_factor, min_weight=0)
...
@click.option('--min_weight', help='Minimum number of visibilities in a uv-cell', default=10, type=int, show_default=True)
def simu_noise_ps(uv_simu_h5, total_time_hour, out_filename, kmax, nks, filter_kpar_min, filter_wedge_theta, sefd, min_weight):
...
def simu_noise_ps_zrange(uv_simu_h5, total_time_hour, k, out_filename, bandwidth, delta_z, dk_over_k,
    ...
    ax.set_ylabel(r'$\Delta^2 (k)\,[\mathrm{mK}]$')
    ax.legend()
    fig.tight_layout()
    fig.savefig(f'{name}.pdf')
