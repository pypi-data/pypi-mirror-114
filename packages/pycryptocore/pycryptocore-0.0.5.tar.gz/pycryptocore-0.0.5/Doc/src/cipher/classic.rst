...
in the relevant module under ``CryptoCore.Cipher``:
...
Their name starts with ``MODE_``, for instance :const:`CryptoCore.Cipher.AES.MODE_CBC`.
...
.. function:: CryptoCore.Cipher.<algorithm>.new(key, mode)

  Create a new ECB object, using <algorithm> as the base block cipher.
  
  :param bytes key: the cryptographic key
  :param mode: the constant ``CryptoCore.Cipher.<algorithm>.MODE_ECB``
...
You might need to use :mod:`CryptoCore.Util.Padding` to align the plaintext to the right boundary.
...
.. function:: CryptoCore.Cipher.<algorithm>.new(key, mode, *, iv=None)

  Create a new CBC object, using <algorithm> as the base block cipher.
  
  :param bytes key: the cryptographic key
  :param mode: the constant ``CryptoCore.Cipher.<algorithm>.MODE_CBC``
...
Example (encryption)::
    ...
    >>> from CryptoCore.Cipher import AES
    >>> from CryptoCore.Util.Padding import pad
    >>> from CryptoCore.Random import get_random_bytes
    >>>
    >>> data = b"secret"
    >>> key = get_random_bytes(16)
...
Example (decryption)::
    ...
    >>> from CryptoCore.Cipher import AES
    >>> from CryptoCore.Util.Padding import unpad
    >>>
...
.. function:: CryptoCore.Cipher.<algorithm>.new(key, mode, *, nonce=None, initial_value=None, counter=None)

  Create a new CTR object, using <algorithm> as the base block cipher.
  
  :param bytes key: the cryptographic key
  :param mode: the constant ``CryptoCore.Cipher.<algorithm>.MODE_CTR``
  ...
  :param counter: a custom counter object created with :func:`CryptoCore.Util.Counter.new`.
...
.. function:: CryptoCore.Cipher.<algorithm>.new(key, mode, *, iv=None, segment_size=8)

  Create a new CFB object, using <algorithm> as the base block cipher.
  
  :param bytes key: the cryptographic key
  :param mode: the constant ``CryptoCore.Cipher.<algorithm>.MODE_CFB``
...
.. function:: CryptoCore.Cipher.<algorithm>.new(key, mode, *, iv=None)
  ...
  :param bytes key: the cryptographic key
  :param mode: the constant ``CryptoCore.Cipher.<algorithm>.MODE_OFB``
  :param bytes iv: the *Initialization Vector*.
         It must be unique for the combination message/key.
         It is as long as the block size (e.g. 16 bytes for AES).
         If not present, the library creates a random IV.
...
OpenPGP mode
------------
Constant: ``CryptoCore.Cipher.<cipher>.MODE_OPENPGP``.
