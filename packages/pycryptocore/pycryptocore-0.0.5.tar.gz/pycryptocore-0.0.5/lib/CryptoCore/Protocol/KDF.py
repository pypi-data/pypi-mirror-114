...
from CryptoCore.Util.py3compat import (tobytes, bord, _copy_bytes, iter_range,
                                  tostr, bchr, bstr)

from CryptoCore.Hash import SHA1, SHA256, HMAC, CMAC, BLAKE2s
from CryptoCore.Util.strxor import strxor
from CryptoCore.Random import get_random_bytes
from CryptoCore.Util.number import size as bit_size, long_to_bytes, bytes_to_long

from CryptoCore.Util._raw_api import (load_pycryptocore_raw_lib,
                                  create_string_buffer,
                                  get_raw_buffer, c_size_t)

_raw_salsa20_lib = load_pycryptocore_raw_lib("CryptoCore.Cipher._Salsa20",
                    """
                    int Salsa20_8_core(const uint8_t *x, const uint8_t *y,
                                       uint8_t *out);
                    """)

_raw_scrypt_lib = load_pycryptocore_raw_lib("CryptoCore.Protocol._scrypt",
                    """
...
def PBKDF1(password, salt, dkLen, count=1000, hashAlgo=None):
    ...
    Args:
     ...
     dkLen (integer):
        ...
        instance for :mod:`CryptoCore.Cipher.AES`.
     count (integer):
        The number of iterations to carry out. The recommendation is 1000 or
        more.
     hashAlgo (module):
        The hash algorithm to use, as a module or an object from the :mod:`CryptoCore.Hash` package.
        The digest length must be no shorter than ``dkLen``.
        The default algorithm is :mod:`CryptoCore.Hash.SHA1`.
...
def PBKDF2(password, salt, dkLen=16, count=1000, prf=None, hmac_hash_module=None):
    ...
    Args:
     ...
     hmac_hash_module (module):
        A module from ``CryptoCore.Hash`` implementing a Merkle-Damgard cryptographic
...
class _S2V(object):
    ...
    def __init__(self, key, ciphermod, cipher_params=None):
        ...
        :Parameters:
          ...
          ciphermod : module
            A block cipher module from `CryptoCore.Cipher`.
...
def HKDF(master, key_len, salt, hashmod, num_keys=1, context=None):
    ...
    Args:
     ...
     hashmod (module):
        A cryptographic hash algorithm from :mod:`CryptoCore.Hash`.
        :mod:`CryptoCore.Hash.SHA512` is a good choice.
     num_keys (integer):
...
def _bcrypt_hash(password, cost, salt, constant, invert):
    from CryptoCore.Cipher import _EKSBlowfish
