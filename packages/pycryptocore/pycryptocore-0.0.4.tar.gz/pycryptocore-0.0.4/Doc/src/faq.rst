...
You can replicate the same keystream in PyCryptoCore with::
...
In PyCryptoCore, you maximize the salt length with::
...
Why do I get the error ``No module named CryptoCore`` on Windows?
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
You might find a directory named ``crypto``, with all the PyCryptoCore files in it.
...
The Windows filesystem is **case-insensitive** so ``crypto`` and ``CryptoCore`` are effectively considered the same thing.
When you subsequently install ``pycryptocore``, ``pip`` finds that a directory named with the target namespace already exists (under the rules of the underlying filesystem),
and therefore installs all the sub-packages of ``pycryptocore`` in it.
...
Most probably you have installed both the ``pycryptocore`` and the old ``pycrypto`` packages.

Run ``pip uninstall pycrypto`` and try again.

The old PyCryptoCore shipped with a ``strxor`` module written as a native library (``.so`` or ``.dll`` file).
If you install ``pycryptocore``, the old native module will still take priority over the new Python extension that comes in the latter.
