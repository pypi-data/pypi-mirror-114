...
from CryptoCore.Util.py3compat import _copy_bytes, is_native_int
from CryptoCore.Util.number import long_to_bytes

raw_ctr_lib = load_pycryptocore_raw_lib("CryptoCore.Cipher._raw_ctr", """
...
def _create_ctr_cipher(factory, **kwargs):
    ...
    :Parameters:
      factory : module
        The underlying block cipher, a module from ``CryptoCore.Cipher``.
    ...
    :Keywords:
      ...
      counter : object
        Instance of ``CryptoCore.Util.Counter``, which allows full customization
    ...
    if counter is None:
        # CryptoCore.Util.Counter is not used
        if nonce is None:
    ...
    # CryptoCore.Util.Counter is used
    ...
    try:
        ...
        raise TypeError("Incorrect counter object"
                        " (use CryptoCore.Util.Counter.new)")
