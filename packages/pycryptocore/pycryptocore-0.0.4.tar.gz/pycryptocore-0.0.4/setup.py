...
project_name = "pycryptocore"
package_root = "CryptoCore"
other_project = "pycryptocorex"
other_root = "CryptoCore"
...
PyCryptoCore is a fork of PyCryptoCore. It brings several enhancements
...
.. _`homepage`: http://www.pycryptocore.org
...
class TestCommand(Command):
    ...
    def run(self):
        ...
        try:
            ...
            if use_separate_namespace:
                from CryptoCore import SelfTest
                from CryptoCore.Math import Numbers
            else:
                from CryptoCore import SelfTest
                from CryptoCore.Math import Numbers
...
def create_cryptocore_lib():
    assert os.path.isdir("lib/CryptoCore")

    try:
        shutil.rmtree("lib/CryptoCore")
    except OSError:
        pass
    for root_src, dirs, files in os.walk("lib/CryptoCore"):

        root_dst, nr_repl = re.subn('CryptoCore', 'CryptoCore', root_src)
        assert nr_repl == 1
...
packages =  [
    ...
    "CryptoCore.Hash",
    "CryptoCore.IO",
    "CryptoCore.PublicKey",
    "CryptoCore.Protocol",
    "CryptoCore.Random",
    "CryptoCore.Signature",
    "CryptoCore.Util",
    "CryptoCore.Math",
    "CryptoCore.SelfTest",
    "CryptoCore.SelfTest.Cipher",
    "CryptoCore.SelfTest.Hash",
    "CryptoCore.SelfTest.IO",
    "CryptoCore.SelfTest.Protocol",
    "CryptoCore.SelfTest.PublicKey",
    "CryptoCore.SelfTest.Random",
    "CryptoCore.SelfTest.Signature",
    "CryptoCore.SelfTest.Util",
    "CryptoCore.SelfTest.Math",
]
package_data = {
    "CryptoCore" : [ "py.typed", "*.pyi" ],
    "CryptoCore.Cipher" : [ "*.pyi" ],
    "CryptoCore.Hash" : [ "*.pyi" ],
    "CryptoCore.Math" : [ "*.pyi" ],
    "CryptoCore.Protocol" : [ "*.pyi" ],
    "CryptoCore.PublicKey" : [ "*.pyi" ],
    "CryptoCore.Random" : [ "*.pyi" ],
    "CryptoCore.Signature" : [ "*.pyi" ],
    "CryptoCore.IO" : [ "*.pyi" ],
    "CryptoCore.Util" : [ "*.pyi" ],
}
...
ext_modules = [
    # Hash functions
    Extension("CryptoCore.Hash._MD2",
        include_dirs=['src/'],
        sources=["src/MD2.c"],
        py_limited_api=True),
    Extension("CryptoCore.Hash._MD4",
        include_dirs=['src/'],
        sources=["src/MD4.c"],
        py_limited_api=True),
    Extension("CryptoCore.Hash._MD5",
        include_dirs=['src/'],
        sources=["src/MD5.c"],
        py_limited_api=True),
    Extension("CryptoCore.Hash._SHA1",
        include_dirs=['src/'],
        sources=["src/SHA1.c"],
        py_limited_api=True),
    Extension("CryptoCore.Hash._SHA256",
        include_dirs=['src/'],
        sources=["src/SHA256.c"],
        py_limited_api=True),
    Extension("CryptoCore.Hash._SHA224",
        include_dirs=['src/'],
        sources=["src/SHA224.c"],
        py_limited_api=True),
    Extension("CryptoCore.Hash._SHA384",
        include_dirs=['src/'],
        sources=["src/SHA384.c"],
        py_limited_api=True),
    Extension("CryptoCore.Hash._SHA512",
        include_dirs=['src/'],
        sources=["src/SHA512.c"],
        py_limited_api=True),
    Extension("CryptoCore.Hash._RIPEMD160",
        include_dirs=['src/'],
        sources=["src/RIPEMD160.c"],
        py_limited_api=True),
    Extension("CryptoCore.Hash._keccak",
        include_dirs=['src/'],
        sources=["src/keccak.c"],
        py_limited_api=True),
    Extension("CryptoCore.Hash._BLAKE2b",
        include_dirs=['src/'],
        sources=["src/blake2b.c"],
        py_limited_api=True),
    Extension("CryptoCore.Hash._BLAKE2s",
        include_dirs=['src/'],
        sources=["src/blake2s.c"],
        py_limited_api=True),
    Extension("CryptoCore.Hash._ghash_portable",
        include_dirs=['src/'],
        sources=['src/ghash_portable.c'],
        py_limited_api=True),
    Extension("CryptoCore.Hash._ghash_clmul",
        include_dirs=['src/'],
        sources=['src/ghash_clmul.c'],
        py_limited_api=True),

    # MACs
    Extension("CryptoCore.Hash._poly1305",
        include_dirs=['src/'],
        sources=["src/poly1305.c"],
        py_limited_api=True),

    # Block encryption algorithms
    Extension("CryptoCore.Cipher._raw_aes",
        include_dirs=['src/'],
        sources=["src/AES.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._raw_aesni",
        include_dirs=['src/'],
        sources=["src/AESNI.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._raw_arc2",
        include_dirs=['src/'],
        sources=["src/ARC2.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._raw_blowfish",
        include_dirs=['src/'],
        sources=["src/blowfish.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._raw_eksblowfish",
        include_dirs=['src/'],
        sources=["src/blowfish_eks.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._raw_cast",
        include_dirs=['src/'],
        sources=["src/CAST.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._raw_des",
        include_dirs=['src/', 'src/libtom/'],
        sources=["src/DES.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._raw_des3",
        include_dirs=['src/', 'src/libtom/'],
        sources=["src/DES3.c"],
        py_limited_api=True),
    Extension("CryptoCore.Util._cpuid_c",
        include_dirs=['src/'],
        sources=['src/cpuid.c'],
        py_limited_api=True),

    Extension("CryptoCore.Cipher._pkcs1_decode",
        include_dirs=['src/'],
        sources=['src/pkcs1_decode.c'],
        py_limited_api=True),

    # Chaining modes
    Extension("CryptoCore.Cipher._raw_ecb",
        include_dirs=['src/'],
        sources=["src/raw_ecb.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._raw_cbc",
        include_dirs=['src/'],
        sources=["src/raw_cbc.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._raw_cfb",
        include_dirs=['src/'],
        sources=["src/raw_cfb.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._raw_ofb",
        include_dirs=['src/'],
        sources=["src/raw_ofb.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._raw_ctr",
        include_dirs=['src/'],
        sources=["src/raw_ctr.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._raw_ocb",
        sources=["src/raw_ocb.c"],
        py_limited_api=True),

    # Stream ciphers
    Extension("CryptoCore.Cipher._ARC4",
        include_dirs=['src/'],
        sources=["src/ARC4.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._Salsa20",
        include_dirs=['src/', 'src/libtom/'],
        sources=["src/Salsa20.c"],
        py_limited_api=True),
    Extension("CryptoCore.Cipher._chacha20",
        include_dirs=['src/'],
        sources=["src/chacha20.c"],
        py_limited_api=True),

    # Others
    Extension("CryptoCore.Protocol._scrypt",
        include_dirs=['src/'],
        sources=["src/scrypt.c"],
        py_limited_api=True),

    # Utility modules
    Extension("CryptoCore.Util._strxor",
        include_dirs=['src/'],
        sources=['src/strxor.c'],
        py_limited_api=True),

    # ECC
    Extension("CryptoCore.PublicKey._ec_ws",
        include_dirs=['src/'],
    ...
    # Math
    Extension("CryptoCore.Math._modexp",
        include_dirs=['src/'],
...
if use_separate_namespace:
    ...
    for i in range(len(packages)):
        packages[i] = packages[i].replace("CryptoCore", "CryptoCore")
    new_package_data = {}
    for k, v in package_data.items():
        new_package_data[k.replace("CryptoCore", "CryptoCore")] = v
    package_data = new_package_data
    for ext in ext_modules:
        ext.name = ext.name.replace("CryptoCore", "CryptoCore")

    # Recreate lib/CryptoCore from scratch, unless it is the only
    # directory available
    if os.path.isdir("lib/CryptoCore"):
        create_cryptocore_lib()
...
setup(
    ...
    description="CryptoCoregraphic library for Python",
    long_description=longdesc,
    author="Helder Eijs",
    author_email="helderijs@gmail.com",
    url="https://www.pycryptocore.org",
