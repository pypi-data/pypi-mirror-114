...
PyCryptoCore can be used as:
 ...
 #. **an almost drop-in replacement for the old PyCryptoCore library**.
    ...
    You can test everything is right with::
        ...
        pip install pycryptocore-test-vectors
        python -m CryptoCore.SelfTest
...
.. note::
    If you want to install under the ``CryptoCore`` package, replace
    below ``pycryptocorex`` with ``pycryptocore``.
...
The simplest way to compile the *Pycryptocore* extensions from
...
#. Compile and install PyCryptoCore::

        > pip install pycryptocorex --no-use-wheel
...
#. Compile and install PyCryptoCore::
        ...
        > pip install pycryptocorex --no-binary :all:
...
.. _pypi: https://pypi.python.org/pypi/pycryptocore
