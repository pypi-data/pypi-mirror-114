...
* GH#466: ``CryptoCore.Math.miller_rabin_test()`` was still using the system random
...
* GH#476: ``CryptoCore.Util.Padding.unpad()`` was raising an incorrect exception
...
* The bulk of the test vectors have been moved to the separate
  package ``pycryptocore-test-vectors``. As result, packages ``pycryptocore`` and
  ``pycryptocorex`` become significantly smaller (from 14MB to 3MB).
...
Resolved issues
---------------
* GH#435: Fixed ``CryptoCore.Util.number.size`` for negative numbers.
...
* Add support for bcrypt key derivation function (``CryptoCore.Protocol.KDF.bcrypt``).
...
* ``point_at_infinity()`` becomes an instance method for ``CryptoCore.PublicKey.ECC.EccKey``, from a static one.
...
* Include type stub files for ``CryptoCore.IO`` and ``CryptoCore.Util``.
...
* New parameter ``output`` for ``CryptoCore.Util.strxor.strxor``, ``CryptoCore.Util.strxor.strxor_c``,
  ``encrypt`` and ``decrypt`` methods in symmetric ciphers (``CryptoCore.Cipher`` package).
...
* Remove ``CryptoCore.Util.py3compat.unhexlify`` and ``CryptoCore.Util.py3compat.hexlify``.
...
* In ``CryptoCore.Util.number``, functions ``floor_div`` and ``exact_div``
...
* Reintroduced ``CryptoCore.__version__`` variable as in PyCryptoCore.
...
* GH#129. pycryptocorex could only be installed via wheels.
...
* GH#121. The record list (--record) was not always correctly filled for the
  pycryptocorex package. Thanks to John W. O'Brien.
...
* Added option to install the library under the ``CryptoCore`` package
  (instead of ``CryptoCore``).
...
* Added ``CryptoCore.PublicKey.ECC`` module (NIST P-256 curve only), including export/import of ECC keys.
...
* Added ``CryptoCore.Cipher.DES3.adjust_key_parity()`` function.
...
* ``CryptoCore.IO.PEM`` and ``CryptoCore.IO.PKCS8`` used to accept empty passphrases.
...
* Removed method ``getRandomNumber`` in ``CryptoCore.Util.number``.
* Removed module ``CryptoCore.pct_warnings``.
...
* Added ``CryptoCore.Signature.pss`` and ``CryptoCore.Signature.pkcs1_15``
...
* Reverted to original PyCryptoCore behavior of method ``verify`` in ``PKCS1_v1_5``
  and ``PKCS1_PSS``.
...
* GMP headers and static libraries are not required anymore at the time
  PyCryptoCore is built. Instead, the code will automatically use the
...
* Reduced the amount of C code by almost 40% (4700 lines).
  ...
  Pycryptocore is now free of CPython extensions.
...
* The ``counter`` parameter of a CTR mode cipher must be generated via
  ``CryptoCore.Util.Counter``. It cannot be a generic callable anymore.
* Removed the ``CryptoCore.Random.Fortuna`` package (due to lack of test vectors).
* Removed the ``CryptoCore.Hash.new`` function.
* The ``allow_wraparound`` parameter of ``CryptoCore.Util.Counter`` is ignored.
...
* Removed ``CryptoCore.PublicKey.RSA.RSAImplementation``.
* Removed ``CryptoCore.PublicKey.DSA.DSAImplementation``.
...
* SHA-3 hash family based on the April 2014 draft of FIPS 202.
  See modules ``CryptoCore.Hash.SHA3_224/256/384/512``.
  Initial Keccak patch by Fabrizio Tarizzo.
* Salsa20 stream cipher. See module ``CryptoCore.Cipher.Salsa20``.
  Patch by Fabrizio Tarizzo.
* Colin Percival's ``scrypt`` key derivation function (``CryptoCore.Protocol.KDF.scrypt``).
* Proper interface to FIPS 186-3 DSA. See module ``CryptoCore.Signature.DSS``.
* Deterministic DSA (RFC6979). Again, see ``CryptoCore.Signature.DSS``.
* HMAC-based Extract-and-Expand key derivation function
  (``CryptoCore.Protocol.KDF.HKDF``, RFC5869).
* Shamir's Secret Sharing protocol, compatible with *ssss* (128 bits only).
  See module ``CryptoCore.Protocol.SecretSharing``.
* Ability to generate a DSA key given the domain parameters.
* Ability to test installation with a simple ``python -m CryptoCore.SelfTest``.
...
* Removed the following methods from all 3 public key object types (RSA, DSA, ElGamal):
  ...
  use the algorithms available in ``CryptoCore.Signature`` and ``CryptoCore.Cipher``.
...
* Removed the ``CryptoCore.Random.OSRNG`` package.
* Removed the ``CryptoCore.Util.winrandom`` module.
* Removed the ``CryptoCore.Random.randpool`` module.
* Removed the ``CryptoCore.Cipher.XOR`` module.
* Removed the ``CryptoCore.Protocol.AllOrNothing`` module.
* Removed the ``CryptoCore.Protocol.Chaffing`` module.
* Removed the parameters ``disabled_shortcut`` and ``overflow`` from ``CryptoCore.Util.Counter.new``.

Other changes
-------------

* ``CryptoCore.Random`` stops being a userspace CSPRNG. It is now a pure wrapper over ``os.urandom``.
