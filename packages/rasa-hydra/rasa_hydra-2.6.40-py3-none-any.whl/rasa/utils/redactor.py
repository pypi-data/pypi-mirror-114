...
class Redactor:
    ...
    def redact_string(self, message: str):
        return re.sub(r"(?<!\d)(\d{12,})(?!\d)", '****************', message)

    # used when message is marked as pci
    def redact_string_strict(self, record: str):
        return re.sub(r"(?<!\d)(\d{3}|\d{4}|\d{6}|\d{12,})(?!\d)", '****************', record)
    ...
    def redact_entity_list(self, entities: list):
        ...
        if number_count > 11:
            for entity in result:
                ...
                additional_value = entity.get('additional_info').get('value')
                if additional_value and isinstance(additional_value, int):
                    entity['additional_info']['value'] = ''.ljust(get_count_digits(additional_value), '*')
    ...
    def redact_metadata_complete(self, metadata: dict):
        ...
        for key in metadata.keys():
            if key == 'partials':
                ...
                result[key] = self.redact_generic(metadata[key])
        return result
        ...
        for key in event.keys():
            if key == self.redact_generic(key):
                if key == 'entities_redacted':
                    ...
                    result[key] = self.redact_generic_strict(event[key])
        return result
