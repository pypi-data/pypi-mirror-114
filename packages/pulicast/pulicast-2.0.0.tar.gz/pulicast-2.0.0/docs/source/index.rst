...
.. note:: You will have to `set up access to the pip server <https://kiteswarms.gitlab-pages.kiteswarms.com/wiki/local-pip-setup/>`_ in advance.

Install the pulicast library only:

.. code-block:: bash

   $ pip install kiteswarms-pulicast

Install the library including tools such as puliconf and puliscope:

.. code-block:: bash

   $ pip install kiteswarms-pulicast[tools]
...
Click [here for the C++ API reference](https://kiteswarms.gitlab-pages.kiteswarms.com/pulicast/).
