...
class BaseKTDatatableView(JSONResponseView):
    ...
    def filter_queryset(self, qs):
        ...
        for column in self._columns:
            column_search = self._query_dict.get(f'query[{camelize(column, False)}]')
            try:
    ...
    def prepare_results(self, qs):
        data = []
        for item in qs:
            data.append({camelize(column, False): self.render_column(item, column) for column in self._columns})
