...
def output(code,input,end="\n",flush=False):
    ...
    """
    try:
        data = f"\033[{code}{input}\033[m"+end
        if len(sys.argv) > 1:
            data = f"\033[{sys.argv[1]}{sys.argv[2]}\033[m"+"\n"
        if flush:
            sys.stdout.flush()
        return str(sys.stdout.write(data))
    except TypeError as err:
        return None


#output("1;31m","ok",flush=True)