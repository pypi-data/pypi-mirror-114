...
class ExtendedBasisLattice(CheckerboardLattice.CheckerboardLattice):
    def __init__(
        self, Vtot, Ratio, n_plane_waves, Frequency, K0, nFloquetBasis, Ellipse_Factor=1
    ):
    ...
    def DiagonalizeExtendedBasis_qx(self, q_points, n_to_keep=25, qy=0):
        ...
        self.ExtendedEnergies = np.zeros(
            (q_points, n_to_keep)
        )  # Full size = self.n_freq * self.plane_wave_dim_2d))
        ...
        for loop, kx in enumerate(self.qx):
            ...
            self.ExtendedEnergies[loop, :] = np.sort(la.eigvalsh(self.H_extended))[
                :n_to_keep
            ]

    def DiagonalizeExtendedBasis_GMKG(self, n_per_line, n_to_keep=25):
        """
        ...
        for loop, (kx, ky) in enumerate(zip(self.qx, self.qy)):
            self.ExtendedEnergies[loop, :] = self.DiagonalizePoint(kx, ky, n_to_keep)

    def DiagonalizePoint(self, qx, qy, n_to_keep=25):
        self.Hd = (
            -2 * self.n_plane_waves
            + np.mod(self.idx - 1, self.plane_wave_dim_1d)
            + np.floor((self.idx - 1) / self.plane_wave_dim_1d)
    ...
    def SetUpExtendedBasis(self, qx, qy):
        ...
        self.H0_nomega = self.Frequency * np.kron(
            np.diag(self.floquet_freq), np.eye(self.H0.shape[0])
        )
        ...
        self.CouplingMatrix = np.diag(
            self.CouplingStrength
            * (
                (
                    -2 * self.n_plane_waves
                    + np.mod(self.idx - 1, self.plane_wave_dim_1d)
        ...
        self.H1 = np.kron(
            np.diag(np.ones(2 * self.nFloquetBasis), 1), self.CouplingMatrix
        ) + np.conj(
