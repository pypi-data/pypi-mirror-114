...
class droptrackingdata:
    def __init__(self):
        ...
        self.noofbp = Entry_Box(dtLabelframe,'# of Bp to remove','13')
    ...
    def confirm(self):
        ...
        self.table = LabelFrame(self.frame, text='Current Order:')

        #orgarnize
        self.frame.grid(row=1,sticky=W,pady=10)
        self.table.grid(row=0,sticky=N,pady=5)

        idx, animallist, bplist = ([0]*int(self.noofbp.entry_get) for i in range(3))

        if self.poseTool == 'DLC':
            for i in range(int(self.noofbp.entry_get)):
                idx[i] = Label()
