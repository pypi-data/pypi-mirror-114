...
class Setup:
    def __init__(
            self, pipe=None, log_pipe: queue.Queue = None, url=None
    ):
        self.pipe = pipe
...
class DownloadModule(Setup):
    def __init__(self, pipe, log_pipe, url, path):
        super().__init__(pipe, log_pipe, (url, path))
...
class READMESetup(Setup):
    def start(self, url):
        ...
        future = crawler.crawl(
            READMESpider, url=url, pipe=self.cache
        )
...
class RawModuleManager(
    ...
    def download(self, module):
        ...
        result = SettingsParser(result) if status else result
...
class ModuleManager(
    ...
    def investigate(self, module, *_):
        return super().download(module)

    def fetch_readme(self, url, *_):
        return super().fetch_readme(url)
    ...
    def Start(cls):
        ...
        for module in temp.sql_code(6):
            module = module[0]

            if temp.check_module(module):
                continue

            try:
                logging.info("Downloading %s", module)
                ...
                logging.exception("Failed to Download %s", module, exc_info=True)
#
        skip_code = """
