...
class AssetManagerClient(AssetManagerClientCache):
    ...
    def get_bytes_in_batch(self,
        ...
        with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
            for id in ids:
                executor.submit(threaded_get_bytes_and_append, id)
    ...
    def get_url_in_batch(self,
        ...
        with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
            for id in ids:
                executor.submit(threaded_get_url_and_append, id)
    ...
    def put_asset(self,
        ...
        if self.asset_type in AssetsConstants.METADATA_ONLY_ASSETS:
            ...
            if not any([file_object, url, file_path, uri]):
                raise AssetError(
    ...
    def download_data_with_query(self,
        ...
        with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
            for id in ids:
                executor.submit(threaded_download_asset_and_metadata, id)
    ...
    def add_auxiliary_batch(self, id_auxiliary_map: Dict[str, dict]):
        ...
        with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
            for id, auxiliary in id_auxiliary_map:
                executor.submit(self.add_auxiliary,
                                id=id,
                                auxiliary=auxiliary)
