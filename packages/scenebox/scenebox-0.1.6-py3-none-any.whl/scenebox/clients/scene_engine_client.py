...
from .asset_manager_client import (DEFAULT_SEARCH_SIZE, MAX_THREADS,
                                   AssetManagerClient)
...
class SceneEngineClient(object):
    ...
    def add_annotation(self,
        ...
        raw_annotation = raw_annotation or "see metadata for {}".format(
            annotation.id)
    ...
    def add_scalar_measurements_dataframe(self,
        ...
        import pandas as pd
        def add_measurements(measurement_df: pd.DataFrame,
                             column: str,
            ...
            entity_id = self.add_scalar_measurements(
        ...
        if TIMESTAMP_FIELD not in measurement_df.columns and not timestamps:
            raise KeyError("dataframe must include a timestamp column or timestamps must be passed separately")
        if not timestamps:
            timestamps = [str_or_datetime_to_datetime(_) for _ in measurement_df.timestamp]

        with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
            for column in measurement_df.columns[1:]:
                executor.submit(add_measurements,
                                measurement_df,
                                column,
                                timestamps,
                                entity_ids)
    ...
    def add_embeddings(
        ...
        def add_embedding_threaded(media_asset_id, embedding_bytes):
            ...
            embedding_meta = {
                ...
                'model': model,
                'version': version,
                'index_name': get_similarity_index_name(media_type=asset_type,
        ...
        if threading:
            with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
                for _media_asset_id, _embedding_bytes in media_asset_id_embedding_bytes_map.items():
                    executor.submit(
                        add_embedding_threaded,
                        media_asset_id=_media_asset_id,
                        embedding_bytes=_embedding_bytes)
        else:
            for _media_asset_id, _embedding_bytes in media_asset_id_embedding_bytes_map.items():
                add_embedding_threaded(
                    media_asset_id=_media_asset_id,
