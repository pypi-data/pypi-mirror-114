...
def findFigureConf(dir: str, tree: dict, unit=1.0):
...
def convertAssets(sourceDir, destDir, platform, unit=1.0, rootDir=None):
    ...
    confTree = createFigureConfTree(sourceDir)
    allTextures = dict()
    ...
    for files in sceneList.values():
        tmpfig.clear()
        conf = findFigureConf(files[0], confTree, unit)
