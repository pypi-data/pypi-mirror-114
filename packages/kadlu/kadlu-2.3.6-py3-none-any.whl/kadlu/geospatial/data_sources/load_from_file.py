...
def load_netcdf(filename, var=None, gridXY=None, plot=False, cmap=matplotlib.pyplot.cm.bone.reversed(), **kwargs):
    ...
    if not gridXY:
        ...
        rng_lon = index_arr(kwargs['west'],  ncfile[axes['x']]), index_arr(kwargs['east'],  ncfile[axes['x']])
        rng_lat = index_arr(kwargs['south'], ncfile[axes['y']]), index_arr(kwargs['north'], ncfile[axes['y']])
    else:
        assert False
    '''
    ...
    out.update(dict(lat=ncfile[axes['y']][rng_lat[0]:rng_lat[1]], lon=ncfile[axes['x']][rng_lon[0]:rng_lon[1]]))
    ...
    if 't' in axes.keys(): 
        if ncfile.variables[axes['t']].units == 'days since 1990-01-01T00:00:00Z':
            ...
            rng_t = (index_arr(dt_2_epoch(kwargs['start'], t0), ncfile[axes['t']] * 24), 
                     index_arr(dt_2_epoch(kwargs['end'], t0), ncfile[axes['t']] * 24))
            out['time'] = epoch_2_dt(ncfile[axes['t']][rng_t[0]:rng_t[1]] * 24, t0)
        if ncfile.variables[axes['t']].units == 'seconds since 1970-01-01T00:00:00Z':
            ...
            rng_t = (index_arr(dt_2_epoch(kwargs['start'], t0), ncfile[axes['t']] / 60 / 60 ), 
                     index_arr(dt_2_epoch(kwargs['end'], t0), ncfile[axes['t']] / 60 / 60))
            out['time'] = epoch_2_dt(ncfile[axes['t']][rng_t[0]:rng_t[1]] , t0, unit='seconds')
        else:
    ...
    if 'z' in axes.keys() and 'v' in axes.keys(): 
        ...
        out = dict(val=ncfile[axes['z']][rng_lat[0]:rng_lat[1], rng_lon[0]:rng_lon[1]], lat=out['lat'].copy(), lon=out['lon'].copy())
