...
class Constants:
    ...
    def google_workaround(self,visit_url: str) -> Union[bool, str]:
        """
        ...
        returned_html =  AsyncFetcher.post_fetch
        ...
        correct_html = ''.join(
            ch.strip().replace('&lt;', '<').replace('&gt;', '>')
            for ch in correct_html
        )
