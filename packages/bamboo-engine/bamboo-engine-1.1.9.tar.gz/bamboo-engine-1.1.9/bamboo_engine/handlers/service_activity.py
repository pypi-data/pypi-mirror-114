    ...
    def execute(
        ...
        if self.LOOP_KEY in data.outputs:
            ...
            context_values.append(
                ContextValue(
                    key=loop_output_key, type=ContextValueType.PLAIN, value=loop_value
                )
            )
        ...
        service = self.runtime.get_service(
            code=self.node.code, version=self.node.version
        )
        ...
        try:
            service.pre_execute(
                data=service_data, root_pipeline_data=root_pipeline_data
            )
            execute_success = service.execute(
                data=service_data, root_pipeline_data=root_pipeline_data
            )
        except Exception:
            ...
            logger.warning(
                "[%s]service execute fail: %s", process_info.root_pipeline_id, ex_data
            )
        logger.debug(
            "[%s] service data after execute: %s", root_pipeline_id, service_data
        )
