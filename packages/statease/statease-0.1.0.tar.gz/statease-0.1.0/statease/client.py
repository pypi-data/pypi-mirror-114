...
class SEClient:
    ...
    def __init__(self):
        ...
        self.socket.connect("tcp://localhost:4900")

    def status_ok(self, reply):
        return reply.get('status', 500) < 400

    def send_payload(self, payload):
        self.socket.send_json(payload, zmq.DONTWAIT)
        reply = self.socket.recv_json()

        if reply.get('payload', None):
            payload_bytes = base64.b64decode(reply['payload'])
            reply['payload'] = json.loads(zlib.decompress(payload_bytes))

        if not self.status_ok(reply):
            if reply.get('payload', None):
                raise HTTPException("Received error from Stat-Ease 360: {}".format(reply['payload']), status=reply.get('status', 500))
