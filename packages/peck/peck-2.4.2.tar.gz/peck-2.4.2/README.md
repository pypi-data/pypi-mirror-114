...
config attributes allow various changes to how peck formats your notes. this file is generated with ```peck -config```, or when ```peck -s [new file]``` is run. for example, if you want a note's date header to be underlined differently, this attribute can be modified. if you want a different string to mark the end of an entry, this can be modified. it should be noted that changing things integral to how the program functions, such as an entry end marker, will outdate the directory you are working in. specifications on what is safe to update in an existing directory are included in the config file itself (```peck.ini```).
