...
from seeq.spy.assets._model import _AssetBase, DependenciesNotBuilt, BuildContext
...
def _build(model, metadata):
    ...
    if isinstance(model, ModuleType):
        ...
        if 'Build Template' in metadata:
            raise SPyValueError('"Build Template" not allowed when "model" parameter is Asset/Mixin class '
                                'declaration')
        ...
        raise SPyTypeError('"model" parameter must be a Python module (with Assets/Mixins) or an Asset/Mixin class '
                           'declaration')
    ...
    for index, row in unique_assets.iterrows():
        ...
        try:
            ...
            raise SPyRuntimeError('The following references/dependencies could not be resolved. Check to see if you '
                                  'have circular references in your classes.\n%s' % '\n'.join(e.descriptors))
