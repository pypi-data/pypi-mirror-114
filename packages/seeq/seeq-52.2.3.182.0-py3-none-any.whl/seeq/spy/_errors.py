...
__all__ = ['DependencyNotFound', 'SPyException', 'SPyRuntimeError', 'SPyValueError', 'SPyTypeError',
...
def spy_exception_handler(self, etype, value, tb, tb_offset=None):
    ...
    try:
        ...
        # the traceback head which calls SPy, if it exists
        spy_call_head = None
        runner = tb
        while runner:
            try:
                if 'seeq.spy' in getmodule(runner.tb_next.tb_frame.f_code).__name__:
        ...
        if spy_call_head is not None:
            ...
            last_cell = tb
            while last_cell.tb_next and get_ipython_cell_no(last_cell.tb_next.tb_frame) is not None:
                last_cell = last_cell.tb_next

            cell_no = get_ipython_cell_no(last_cell.tb_frame)
            line_no = last_cell.tb_lineno
            header = format_tb_header(self.InteractiveTB.Colors, etype.__name__, str(value), line_no, cell_no)
            button = True
