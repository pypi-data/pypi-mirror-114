...
public:
	...
	void setState(vector<int> myState) {
		state = myState;
	}

	void setGoalState(vector<int> myGoalState) {
...
PYBIND11_MODULE(probotest1, m) {
    ...
    pybind11::class_<ProtossMacroEnv>(m, "ProtossMacroEnv")
		...
		.def("stepAuto", &ProtossMacroEnv::stepAuto)
		...
		.def("setGoalState", &ProtossMacroEnv::setGoalState)
		.def("setState", &ProtossMacroEnv::setState)
		...
		.def_readwrite("ACTION_SPACE", &ProtossMacroEnv::ACTION_SPACE)
        ;
