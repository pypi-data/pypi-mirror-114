...
public:
	...
	ProtossMacroEnv(const Names& myNames, const vector<AssetStats>& myAssetList) {
		REGEN_RATE = 1;
		n = myNames;
...
public:
	...
	//create an environment from here that will have references to this object
	ProtossMacroEnv createFreshEnv() {
		ProtossMacroEnv env = ProtossMacroEnv(n, assetList);
		return env;
	}

	ProtossMacroEnv deepCopyEnv(ProtossMacroEnv& origEnv) {
		ProtossMacroEnv env = ProtossMacroEnv(origEnv);
		return env;
	}
...
PYBIND11_MODULE(probotest1, m) {
	...
	pybind11::class_<GeneralLoader>(m, "GeneralLoader")
		.def(pybind11::init<>())
		.def("createFreshEnv", &GeneralLoader::createFreshEnv)
		.def("deepCopyEnv", &GeneralLoader::deepCopyEnv)
		;
