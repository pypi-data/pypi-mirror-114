...
class SmartbearTunnel:
    ...
    def run(self):
        ...
        if self.tunnel_type == 'local-html-files':
            tunnel_command.extend(('--dir', self.requested_options['directory']))

        if self.tunnel_type == 'proxy':
            tunnel_command.extend(('--proxyIp', self.requested_options['proxyIp']))
            tunnel_command.extend(('--proxyPort', self.requested_options['proxyPort']))

        if self.tunnel_name is not None:
            tunnel_command.extend(('--name', self.tunnel_name))
