...
async def handle(reader, writer, socks_proxy=None, feed=b''):
    ...
    try:
        if method == b'CONNECT':
            len_local, len_remote = await handle_connect(
                reader, writer, method, path, protocol, headers, socks_proxy)
        else:
            len_local, len_remote = await handle_proxy(reader, writer, method,
    ...
    logging.info('%s %s%s %.3fs <%d >%d', method.decode(), path, proxy_log,
...
async def handle_connect(reader, writer, method, path, protocol, headers,
                         socks_proxy):
    proxy_log = ' X' + socks_proxy if socks_proxy else ''
    hostinfo = Host(path)
...
async def handle_proxy(reader, writer, method, path, protocol, headers,
                       socks_proxy):
    proxy_log = ' X' + socks_proxy if socks_proxy else ''
    ...
    remote_reader, remote_writer = await open_connection(
        hostinfo.hostname, hostinfo.port, socks_proxy)
    headers = [
        header for header in headers if not header[0].startswith(b'proxy-')
    ]
...
if __name__ == '__main__':
    ...
    run_forever(
        start_server_asyncio(
            partial(handle, socks_proxy='socks5://127.0.0.1:2020'), ':2100'))
