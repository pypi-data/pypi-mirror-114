...
from cloudrail.cli.commands_utils import echo_error, validate_origin, exit_with_code, validate_cloud_account_input
from cloudrail.cli.error_messages import generate_failure_message, generate_simple_message
...
class EvaluationRunCommandService(CommandService):
    ...
    def _upload_iac_file(self, customer_id: str, account_config: AccountConfigDTO, job_id: str,
                         custom_rules: dict, drift_track: bool):
        pass
    ...
    def run(self):
        ...
        self._upload_iac_file(customer_id=customer_id, account_config=account_config,
                              job_id=job_id, custom_rules=custom_rules, drift_track=drift_track)
    ...
    def _return_assessment_results(self, assessment_job: AssessmentJobDTO, account_policies: List[PolicyDTO]):
        ...
        formatter = self._get_formatter(output_format=self.command_parameters.output_format, api_key=censored_api_key,
                                        ...
                                        stylize=stylize, verbose=self.command_parameters.verbose)
        ...
        if self.command_parameters.stdout:
            if self.command_parameters.output_format != OutputFormat.TEXT:
                formatter = TextFormatter(True, self.command_parameters.verbose).format
                format_result, notices = formatter(rule_results, assessment_job, account_policies)
