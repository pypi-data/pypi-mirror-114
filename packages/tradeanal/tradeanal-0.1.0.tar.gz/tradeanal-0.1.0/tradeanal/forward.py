...
def forward_test(
    ...
    with Pool(processes=12) as pool:
        for epoch in range(epochs):
            ...
            train_df = df[epoch_start_learn:epoch_end_learn]
            tasks[epoch] = pool.apply_async(find_best_params, (
                calc_strategy,
                train_df.copy(),
                param_ranges
            ))
        df_result = pd.DataFrame(columns=['epoch', 'coef_eff', 'train_profit', 'train_rel_profit', 'test_profit'] + list(param_ranges.keys()))
        total_epochs = len(tasks)
        while len(tasks):
            print("\r", end="")
            print(f'leave epochs: {len(tasks)}/{total_epochs}', end="")
            for epoch, task in tasks.copy().items():
                if task.ready():
                    ...
                    test_df = df[epoch_start_test:epoch_end_test]
            ...
            sleep(1)
    print("\r ")
    df_result = df_result.sort_values('epoch')
    return df_result
