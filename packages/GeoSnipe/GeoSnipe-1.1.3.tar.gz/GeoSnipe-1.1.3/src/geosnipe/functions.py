...
from geosnipe.constants import SKIN
...
async def send_request(conn: tuple[asyncio.StreamReader, asyncio.StreamWriter], username: str,
                       bearer: str, delay: float) -> tuple[str, float]:
    await asyncio.sleep(delay)
    conn[1].write(bytes(
        f"PUT /minecraft/profile/name/{username} HTTP/1.1\r\n"
    ...
    await conn[1].drain()
    return (await conn[0].read(12)).decode()[9:12], time()


async def change_skin(access_token: str) -> str:
    conn = await asyncio.open_connection("api.minecraftservices.com", 443, ssl=True)
    conn[1].write(SKIN % bytes(access_token, "utf-8"))
    await conn[1].drain()
    return (await conn[0].read(12)).decode()[9:12]
...
async def snipe(username: str, drop_time: int,
    ...
    try:
        results = await asyncio.gather(
            *[send_request(connections[i], username, bearer, 0.001 * i) for i in range(len(connections))]
        )
    except ConnectionError:
...
async def check_offset(tests: int = 5) -> Optional[float]:
    ...
    data = bytes(
        f"PUT /minecraft/profile/name/Minecraft HTTP/1.1\r\n"
        f"Host: api.minecraftservices.com\r\n"
        f"Content-Type: application/json\r\n"
