...
def adhoc_fetch(title,page,limit=10,tgtdir=os.getcwd(),debug=False,force=0,root=None) :
...
"""
    ...
    if os.path.isfile(os.path.join(tgtdir,title, "downloaded.txt")) :
        with open(os.path.join(tgtdir,title, "downloaded.txt"),"r",encoding="UTF-8") as f :
            ...
            if force and len(downloaded) > 1 :
                last = sorted(list(downloaded))[-(force+1)]
            elif downloaded :
                last = max(downloaded)
            else :
                pass
    ...
    if debug :
        print(rtext)
    chapters = list()
    for m in re.findall(r"(<td|<dd|<li)[^<>]*?>\s*<a\s+href\s*=\s*\"(\S*?\d+\.(htm|html))\".*?>\s*(.*?)\s*</a>",rtext,re.DOTALL) :
        if debug :
            print(m)
        url=m[1].split("/")[-1]
...
def batch_fetch(cfgfile,num=10,debug=False,force=0) :
    report = "\n"
    with open(cfgfile,"r",encoding="UTF-8") as f :
        ...
        for ln in f.readlines() :
            ...
            if m :
                ...
                try :
                    rpt = adhoc_fetch(title,page,num,tgtdir,debug,force,root) + "\n\n"
                    report += rpt
...
def novels_main():
    ...
    if args.title :
        adhoc_fetch(args.title,args.page,args.number,args.dir,args.debug,args.force,args.root)
        return
    args.cfgfile = os.path.expanduser(args.cfgfile)
    if os.path.isfile(args.cfgfile) :
        report = batch_fetch(args.cfgfile,num=args.number,debug=args.debug,force=args.force)
        print("# Summary :\n" + report)
    else :
