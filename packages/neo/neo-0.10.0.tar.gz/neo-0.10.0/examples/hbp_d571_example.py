...
# loading data
data = TiffIO(
    data_path, units=dimensionless, sampling_rate=25 * Hz, spatial_scale=0.05 * mm
).read()
...
freqs, psd = el.spectral.welch_psd(
    central_signal, fs=central_signal.sampling_rate, freq_res=0.1 * Hz, overlap=0.8
)
plt.subplot(2, 2, 4)
