...
def _relabel_numba(ai_cell_labels, ai_cellid_relabel, o_dtype=np.uint32):
    ...
    input:
        ...
        o_dtype: data type isndie the image array. stanadrd us np.unit32,
...
def match_nuccell_labels(
    ...
    input:
        ...
        i_bit_tiff: bit format that the resulting tiff basin files should have. stanadrd is 32, as most tiff compatible softwares can acces 32bit tiffs.
    ...
    if (ai_nuc_labels.max() >= 2**i_bit_tiff):
        sys.exit(f'Error @ jinxif.extract.segment.match_nuccell_labels : cell labels >= 2**{i_bit_tiff} found in nucleus cell label file.\nthis exeeds the limit of the set {i_bit_tiff}[bit] data type for the resulting matched tiff image.\nplease just the i_bit_tiff setting. knowen uint bit values are: 64, 32, 16, and 8. default is 32.')
    ...
    ai_cell_relabeled = _relabel_numba(ai_cell_labels=ai_cell_labels, ai_cellid_relabel=ai_cellid_relabel, o_dtype=np.uint32)
...
def segment_spawn(
    ...
    input:
        ...
        i_bit_tiff: bit format that the resulting matched tiff basin files should have. stanadrd is 32, as most tiff compatible softwares can acces 32bit tiffs.
        ...
        if b_found:
            ...
            for s_slide_pxscene in sorted(set(df_img.slide_scene)):
                ...
                if s_task == 'match':
                    ...
                    s_stream = s_stream.replace('peek_i_bit_tiff', str(i_bit_tiff))
                    # file system
