...
def renderSymbol(symbol, size, pen, brush, device=None):
    ...
    if device is None:
        device = QtGui.QImage(
            int(size + penPxWidth), int(size + penPxWidth), QtGui.QImage.Format_ARGB32
        )
        device.fill(0)
...
class SymbolAtlas(object):
    ...
    def __getitem__(self, styles):
        ...
        new = {
            key: style for key, style in zip(keys, styles) if key not in self._coords
        }
    ...
    def _keys(self, styles):
        return [
            (id(symbol), size, id(pen), id(brush))
...
class ScatterPlotItem(GraphicsObject):
    ...
    **Signals:**
    ...
    def __init__(self, *args, **kargs):
        ...
        self._maxSpotWidth = (
            0  ## maximum size of the scale-variant portion of all spots
        )
        self._maxSpotPxWidth = (
            0  ## maximum size of the scale-invariant portion of all spots
        )
        self.opts = {
            "pxMode": True,
    ...
    def setData(self, *args, **kargs):
        ...
        *name*                 The name of this item. Names are used for automatically
        ...
        oldData = (
            self.data
        )  ## this causes cached pixmaps to be preserved while new data is registered.
    ...
    def setPoints(self, *args, **kargs):
        warnings.warn("Use setData instead.", DeprecationWarning, stacklevel=2)
    ...
    def setPen(self, *args, **kargs):
        ...
        if len(args) == 1 and (
            isinstance(args[0], np.ndarray) or isinstance(args[0], list)
        ):
            pens = args[0]
            ...
            if len(pens) != len(dataSet):
                raise Exception(
                    "Number of pens does not match number of points (%d != %d)"
                    % (len(pens), len(dataSet))
                )
    ...
    def setBrush(self, *args, **kargs):
        ...
        if len(args) == 1 and (
            ...
            if len(brushes) != len(dataSet):
                raise Exception(
                    "Number of brushes does not match number of points (%d != %d)"
                    % (len(brushes), len(dataSet))
                )
    ...
    def setSymbol(self, symbol, update=True, dataSet=None, mask=None):
        ...
        if isinstance(symbol, np.ndarray) or isinstance(symbol, list):
            ...
            if len(symbols) != len(dataSet):
                raise Exception(
                    "Number of symbols does not match number of points (%d != %d)"
                    % (len(symbols), len(dataSet))
                )
    ...
    def setPointsVisible(self, visible, update=True, dataSet=None, mask=None):
        ...
        if isinstance(visible, np.ndarray) or isinstance(visible, list):
            ...
            if len(visibilities) != len(dataSet):
                raise Exception(
                    "Number of visibilities does not match number of points (%d != %d)"
                    % (len(visibilities), len(dataSet))
                )
        ...
        if self.opts["pxMode"] and self.opts["useCache"]:
            ...
            if np.any(mask):
                ...
                if _USE_QRECT:
                    ...
                    list(
                        imap(
                            self._sourceQRect.__setitem__,
                            imap(tuple, sru),
                            imap(QtCore.QRectF, *zip(*sru)),
                        )
                    )
                    dataSet["sourceQRect"][mask] = list(
                        imap(self._sourceQRect.__getitem__, imap(tuple, sr))
                    )
    ...
    def _updateMaxSpotSizes(self, **kwargs):
        if self.opts["pxMode"] and self.opts["useCache"]:
            ...
            w, pw = max(
                chain(
                    [(self._maxSpotWidth, self._maxSpotPxWidth)],
    ...
    def getSpotOpts(self, recs, scale=1.0):
        ...
        if recs.ndim == 0:
            ...
            recs["brush"][np.equal(recs["brush"], None)] = fn.mkBrush(
                self.opts["brush"]
            )
            return recs
    ...
    def dataBounds(self, ax, frac=1.0, orthoRange=None):
        ...
        if frac >= 1.0:
            self.bounds[ax] = (
                np.nanmin(d) - self._maxSpotWidth * 0.7072,
                np.nanmax(d) + self._maxSpotWidth * 0.7072,
            )
            return self.bounds[ax]
        elif frac <= 0.0:
            raise Exception(
    ...
    def paint(self, p, *args):
        ...
        if self._exportOpts is not False:
            ...
            scale = self._exportOpts.get(
                "resolutionScale", 1.0
            )  ## exporting to image; pixel resolution may have changed
        else:
    ...
    def hoverEvent(self, ev):
        if self.opts["hoverable"]:
            ...
            if vb is not None and self.opts["tip"] is not None:
                cutoff = 3
                tip = [
                    self.opts["tip"](x=pt.pos().x(), y=pt.pos().y(), data=pt.data())
    ...
    def _hasHoverStyle(self):
        return any(
            self.opts["hover" + opt.title()] != _DEFAULT_STYLE[opt]
            for opt in ["symbol", "size", "pen", "brush"]
        )
...
class SpotItem(object):
    ...
    def resetBrush(self):
        ...
        self._data[
            "brush"
        ] = None  ## Note this is NOT the same as calling setBrush(None)
        self.updateItem()
    ...
    def updateItem(self):
        self._data["sourceRect"] = (
            0,
            0,
            0,
            0,
        )  # numpy <=1.13.1 won't let us set this with a single zero
