...
from savvihub.cli.inputs.dataset import dataset_full_name_callback, get_path_callback, dest_path_callback
...
def ls(
    ...
    path: str = typer.Argument(None, callback=get_path_callback(inquiry_message='Path', default='/')),
...
def rm(
    ...
    dataset_full_name: str = typer.Argument(None, metavar='DATASET',
                                            help='{organization}/{dataset} format where organization is optional.',
                                            callback=dataset_full_name_callback),
    path: str = typer.Argument(None, callback=get_path_callback(inquiry_message='Path to delete', default='')),
...
def upload(
    ...
    """
    organization_name_override, dataset_name, _ = parse_dataset(dataset_full_name)
...
def download(
    ...
    dataset_full_name: str = typer.Argument(None, metavar='DATASET',
                                            help='{organization}/{dataset} format where organization is optional.',
                                            callback=dataset_full_name_callback),
    source_path: str = typer.Argument(None, callback=get_path_callback(inquiry_message='Source file or directory path',
                                                                       default='/')),
    dest_path: str = typer.Argument(None, callback=dest_path_callback),
):
    """
