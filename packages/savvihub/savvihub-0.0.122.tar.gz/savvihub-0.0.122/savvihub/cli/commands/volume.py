...
def parse_remote_volume_path(remote_path: str) -> (ResponseVolume, str, str):
    # sv://1:dataset_version/path/to/files
    u = urlparse(remote_path)
    volume_dataset_version = u.netloc
    split_dataset_version = volume_dataset_version.split(':', 1)
    volume_id = int(split_dataset_version[0])
    file_path = u.path.lstrip('/')

    dataset_version = split_dataset_version[1] if len(split_dataset_version) == 2 else 'latest'

    return volume_id, dataset_version, file_path
...
def ls(
    ...
    volume_id, dataset_version, path = parse_remote_volume_path(source_path)
    volume_file_list(ctx, volume_id, dataset_version, path, recursive, directory)
...
def cp(
    ...
    source_dataset_version = dest_dataset_version = None
    if is_source_remote:
        source_volume_id, source_dataset_version, source_path = parse_remote_volume_path(source_path)
    if is_dest_remote:
        dest_volume_id, dest_dataset_version, dest_path = parse_remote_volume_path(dest_path)

    volume_file_copy(ctx, source_volume_id, source_dataset_version, source_path,
                     dest_volume_id, dest_dataset_version, dest_path,
                     recursive, watch)


def volume_file_list(ctx: Context, volume_id: int, dataset_version: str, path: str, recursive: bool, directory: bool):
    ...
    if recursive and directory:
        ...
        files = client.volume_file_list(volume_id, dataset_version=dataset_version, path=path, recursive=True).results
    elif directory:
        if not path:
            raise ExitException('[Error] path must be specified to run -d option')

        file = client.volume_file_read(volume_id, path)
        files = [file] if file is not None else None
    else:
        files = client.volume_file_list(volume_id, dataset_version=dataset_version, path=path, recursive=False).results
...
def volume_file_copy(ctx: Context, source_volume_id, source_dataset_version, source_path, dest_volume_id, dest_dataset_version, dest_path, recursive, watch):
    client = ctx.authenticated_client
    while True:
        if source_volume_id is not None and dest_volume_id is not None:
            volume_file_copy_remote_to_remote(
                client,
                source_volume_id, source_path, source_dataset_version,
                dest_volume_id, dest_path, dest_dataset_version,
                recursive
            )
...
def volume_file_copy_remote_to_remote(
    ...
    if dest_dataset_version != 'latest':
        raise ExitException(f'Cannot write to dataset_versions: {dest_path}')

    source_file = client.volume_file_read(source_volume_id, source_path)
    ...
    try:
        dest_file = client.volume_file_read(dest_volume_id, dest_path)
    except NoSuchFileOrDirectoryAPIException:
        pass
    else:
