...
'''
    ...
    def prepare_username(self):
        if self.get_secure_cookie("username") is None:
            self.set_cookie("username", "", -1)

    def prepare_xsrf(self):
        if self.get_secure_cookie("_x") is None:
            self.set_secure_cookie("_x", omnitools.randb(16))

    def prepare_session_key(self):
        if self.get_secure_cookie("_k") is None:
            self.set_secure_cookie("_k", omnitools.randb(16))
    ...
    def decrpyt_params(self, k, v):
        return v

    def encrpyt_params(self, k, v):
        return v
...
class AJAX(SuccessHandler):
    ...
    def post(self, *args):
        ...
        if self.decrypted_params:
            self.write(self.encrpyt_params(self.get_cookie("_k").encode(), raw_params))
        else:
