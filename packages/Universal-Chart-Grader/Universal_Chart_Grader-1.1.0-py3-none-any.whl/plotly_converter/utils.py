...
def cast_float(na):
    ...
    elif isinstance(na, list) and isinstance(na[0], datetime):
...
def pprint(d, width=150, truncate=False, file=sys.stdout):
    prnt(d, width=width, truncate=truncate, file=file)
...
def EQ(a, b):
    ...
    if t == type(np.array([])):
        ...
        return all(EQ(elem_a, elem_b) for elem_b in b for elem_a in a)
    elif t == dict:
        if a.keys() != b.keys(): return False
        keys = a.keys()
        for key in keys:
            if not EQ(a[key], b[key]):
                return False
    elif t.__module__ == 'builtins' or np.issctype(t) or t == MySet:
        return a == b
    else:
