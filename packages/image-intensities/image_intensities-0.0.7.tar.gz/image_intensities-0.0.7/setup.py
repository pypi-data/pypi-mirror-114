...
with open('image_intensities/VERSION.txt') as f:
    version = f.read().strip()
# end with

with open('image_intensities/build_cffi.c-files.txt') as f:
    compile_c_files = f.read().strip().splitlines()
# end with

with open('image_intensities/build_cffi.h-files.txt') as f:
    compile_h_files = f.read().strip().splitlines()
# end with
...
setup(
    ...
    cffi_modules=["image_intensities/build_cffi.py:ffibuilder"],
    ...
    setup_requires=["cffi>=1.0.0", "Cython>=0.29.24"],
    ...
    package_data={
        ...
        'image_intensities': [
            'VERSION.txt',
            '*.pyx',
            'build_cffi.c-files.txt',
            'build_cffi.h-files.txt',
        ] + [
            f'_native_code/{file_name}' for file_name in compile_c_files if file_name
        ] + [
            f'_native_code/{file_name}' for file_name in compile_h_files if file_name
        ]
    },
