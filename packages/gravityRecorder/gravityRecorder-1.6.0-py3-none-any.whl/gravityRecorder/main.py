...
class Recorder(Wsqluse):
    ...
    def add_carrier(self, name, inn=None, kpp=None, ex_id=None, status=None):
        ...
        command = "INSERT INTO clients (name, inn, kpp, ex_id, status) " \
                  "VALUES (%s, %s, %s, %s, %s) ON CONFLICT (name, inn, kpp)" \
                  "DO UPDATE SET ex_id=%s, status=%s"
        values = (name, inn, kpp, ex_id, status, ex_id, status)
