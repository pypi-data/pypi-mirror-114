...
class Recorder(Wsqluse):
    ...
    def update_opened_record(self, record_id, auto_id, car_number, carrier, trash_cat_id, trash_type_id,
                             comment, polygon=None, *args, **kwargs):
        ...
        if auto_id is None:                 # Если auto_id = None (гос. номер вбит вручную, такой машины нет в таблице auto)
            self.register_car(car_number)    # Зарегистрировать ее (завести запсиь в таблице auto)
