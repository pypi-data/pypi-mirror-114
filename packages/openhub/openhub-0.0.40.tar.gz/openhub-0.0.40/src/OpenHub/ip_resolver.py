...
def pinger(job_q, results_q):
    ...
    while True:
        ...
        try:
            ...
            subprocess.check_call(['/usr/bin/ping', '-c', '1', '-W', '0.5', '-w', '0.5', str(ip)],shell=True,stdout=subprocess.STDOUT, timeout=0.3)
...
def non_multiprocessing():
    ...
    for i in range(1, 255):
        ...
        try:
            subprocess.check_call(['/usr/bin/ping', '-c', '1', '-W', '0.3', '-w', '0.3', str(ip)],
                                  stdout=subprocess.STDOUT,shell=True, timeout=0.3)
