...
class Property:
    def __init__(self, macho: MachO, objc_class, property: objc2_prop, vmaddr: int):
        self.macho = macho
        self.property = property

        name_location = macho.vm.get_file_address(property.name, "__objc_methname")
        self.name = getCStrAt(macho.fd, name_location)

        attr_location = self.macho.vm.get_file_address(property.attr, "__objc_methtype")
        self.attr = self.decode_property_attributes(getCStrAt(macho.fd, attr_location))
    ...
    def decode_property_attributes(self, type_str: str):
        ...
        for attribute in attribute_strings:
            ...
            if indicator == "T":
                ptype = self.macho.tp.process(attribute[1:])[0]
                if ptype == "{":
                    print(attribute)
                is_id = attribute[1] == "@"
                continue
