# AUTOGENERATED! DO NOT EDIT! File to edit: binding.ipynb (unless otherwise specified).]
...
class BindingProcessor:
    def __init__(self, lib):
        self.lib= lib
        self.import_stack = self._load_binding_info(lib)
    ...
    def _load_binding_info(self, lib):
        fd = lib.fd
        ...
        while True:
            ...
            while True:
                ...
                OP = getAt(fd, ea, 1) & 0xF0
                VALUE = getAt(fd, ea, 1) & 0x0F
                ea += 1
                if OP == OPCODE.BIND_OPCODE_DONE:
                    ...
                    lib_ordinal, bump = decodeUleb128(fd, ea)
                    ea = bump
                    ...
                    seg_index = VALUE
                    number, head = decodeUleb128(fd, ea)
                    seg_offset = number
                    ea = head
                elif OP == OPCODE.BIND_OPCODE_ADD_ADDR_ULEB:
                    o, bump = decodeUleb128(fd, ea)
                    seg_offset += o
                    ea = bump
                    ...
                    t, bump = decodeUleb128(fd, ea)
                    count = t
                    ea = bump
                    s, bump = decodeUleb128(fd, ea)
                    skip = s
                    ea = bump
