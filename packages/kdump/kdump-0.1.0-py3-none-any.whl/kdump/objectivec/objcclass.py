...
class Class:
    ...
    def __init__(self, macho: MachO, ptr: int, meta=False, objc2class=None):
        self.macho = macho
        self.ptr = ptr
        self.meta = meta
        self.metaclass = None
    ...
    def _load_objc2_class(self, macho, ptr):
        ...
        if objc2_class_item.isa != 0 and objc2_class_item.isa <= 0xFFFFFFFFFF and not self.meta:
            try:
                metaclass_location = macho.vm.get_file_address(objc2_class_item.isa)
                metaclass_item: objc2_class = load_struct(macho.fd, metaclass_location, objc2_class_t)
                self.metaclass = Class(macho, metaclass_location, True, metaclass_item)
            except ValueError:
                pass
        if bad_addr:
            # Linked Superclass
