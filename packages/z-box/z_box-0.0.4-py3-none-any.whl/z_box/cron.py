...
def execute_at(
	...
	# prompt
	z.out(f"job wil be excuted at {at}, please wait...")
...
def execute_after(
	...
	# prompt
	z.out(f"job wil be excuted after {after}, please wait...")
...
def execute_circularly(
	...
	if begin_at and end_at:
		z.out(f"job wil be excuted during {begin_at} - {end_at}, at every {interval}, please wait...")
	else:
		z.out(f"job wil be excuted at every {interval}, please wait...")
...
if __name__ == '__main__':
	...
	# execute_circularly(job_example, interval="1s", scheduler="background")
