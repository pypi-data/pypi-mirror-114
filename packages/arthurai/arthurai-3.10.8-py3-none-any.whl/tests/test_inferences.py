...
from tests.helpers import mock_post, MockDatetime, MockShortUUID, mock_patch
...
class TestInferences:
    ...
    def test_format_inference_no_gt(self, mock_client, tabular_model):
        inference = tabular_model._format_inference_request(
    ...
    @responses.activate
    def test_format_inference_request(self, tabular_model):
        inf = tabular_model._format_inference_request(
    ...
    send_inferences_cases = [
        pytest.param(InferencesCase(
            ...
            expected_response=EXPECTED_RESPONSE), id="orig-api-format"),
        pytest.param(InferencesCase(
            pos_args=(API_FORMAT_DATA,), kwargs={}, expected_request=API_FORMAT_DATA,
            expected_response=EXPECTED_RESPONSE), id="orig-api-format-positional"),
        pytest.param(InferencesCase(
            ...
            expected_request=API_FORMAT_DATA, expected_response=EXPECTED_RESPONSE),
        ...
        pytest.param(InferencesCase(
            ...
            id="api-format-no-ground-truth"),
        pytest.param(InferencesCase(
            pos_args=(), kwargs={'inferences': FLAT_DATA_FULL_INPUTS}, expected_request=API_FORMAT_DATA,
            expected_response=EXPECTED_RESPONSE), id="flat-dict-format"),
        pytest.param(InferencesCase(
        ...
        pytest.param(InferencesCase(
            pos_args=(), kwargs={'inferences': FLAT_DATA_INFERENCES_ONLY,
                                 'predictions': PREDS_RAW_LIST,
                                 'ground_truths': GROUND_TRUTH_RAW_LIST},
            expected_request=API_FORMAT_DATA_NO_BATCH, expected_response=EXPECTED_RESPONSE),
            id="flat-dict-format-separate-preds-gt-no-batch"),
        pytest.param(InferencesCase(
            pos_args=(), kwargs={'inferences': FLAT_DATA_PARTIAL_GT,
                                 'batch_id': BATCH_ID},
            expected_request=API_FORMAT_DATA_PARTIAL_GT, expected_response=EXPECTED_RESPONSE),
        ...
        pytest.param(InferencesCase(
            pos_args=(), kwargs={'inferences': pd.DataFrame(FLAT_DATA_INFERENCES_ONLY),
                                 ...
                                 'batch_id': BATCH_ID},
            expected_request=API_FORMAT_DATA, expected_response=EXPECTED_RESPONSE), id="mixed-types"),
        pytest.param(InferencesCase(
            pos_args=(), kwargs={'inferences': FULL_DATA_FRAME},
            expected_request=API_FORMAT_DATA, expected_response=EXPECTED_RESPONSE), id="single-data-frame"),
        pytest.param(InferencesCase(
            pos_args=(), kwargs={'inferences': pd.DataFrame(FLAT_DATA_INFERENCES_ONLY),
                                 'predictions': pd.DataFrame({PRED: PREDS_RAW_LIST}),
                                 'ground_truths': pd.DataFrame({GROUND_TRUTH: GROUND_TRUTH_RAW_LIST}),
                                 'batch_id': BATCH_ID},
            expected_request=API_FORMAT_DATA, expected_response=EXPECTED_RESPONSE), id="three-dataframes"),
        pytest.param(InferencesCase(
            ...
            expected_request=API_FORMAT_DATA, expected_response=EXPECTED_RESPONSE), id="timestamps-and-ids-separate"),
        pytest.param(InferencesCase(
            pos_args=(), kwargs={'inferences': FLAT_DATA_NO_TIMESTAMPS_OR_IDS,
                                 'batch_id': BATCH_ID,
    ...
    def test_send_inferences(self, tabular_model, case: InferencesCase):
        ...
        with patch("arthurai.core.models.shortuuid.uuid") as uuid_mock:
            with patch("arthurai.core.models.datetime") as datetime_mock:
                ...
                actual_response = tabular_model.send_inferences(*case.pos_args, **case.kwargs)
        ...
        assert case.kwargs.keys() == kwargs_copy.keys()
        for key in case.kwargs.keys():
            if isinstance(kwargs_copy[key], (pd.Series, pd.DataFrame)):
                assert kwargs_copy[key].equals(case.kwargs[key])
            elif isinstance(kwargs_copy[key], np.ndarray):
                assert np.equal(kwargs_copy[key], case.kwargs[key]).all()
            else:
                assert case.kwargs[key] == kwargs_copy[key]
    ...
    def test_update_ground_truth(self, tabular_model, case: InferencesCase):
        pos_args_copy = deepcopy(case.pos_args)
        kwargs_copy = deepcopy(case.kwargs)
        ...
        with patch("arthurai.core.models.datetime") as datetime_mock:
            datetime_mock.now.side_effect = datetime_generator.next
            actual_response = tabular_model.update_inference_ground_truths(*case.pos_args, **case.kwargs)
