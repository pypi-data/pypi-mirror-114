...
def enrich(df: pd.DataFrame,
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. df shape: {str(df.shape)}, weekly_dataset shape: {str(weekly_dataset.to_pandas_dataframe().shape)}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. df final shape : {str(df.shape)}')
...
def ingest_data(  # NOSONAR
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'ingest_data {str(debugging_count)}. input df shape: {str(df.shape)}'
        )
    ...
    if deployment:
        ...
        if logger:
            debugging_count += 1
            logger.debug(
                f'ingest_data {str(debugging_count)}. df after resetting index and forward filling: {str(df.shape)}'
            )
        ...
        if not last_day or not first_day:
            raise ValueError('missing time span parameters')
        ...
        if logger:
            debugging_count += 1
            logger.debug(
                f'ingest_data {str(debugging_count)}. df after selecting trafo with more than 90% of data: {str(df.head().to_string())}'
            )
    ...
    if logger:
        ...
        debugging_count += 1
        logger.debug(
            f'ingest_data {str(debugging_count)}. df final shape: {str(df.shape)}'
        )
