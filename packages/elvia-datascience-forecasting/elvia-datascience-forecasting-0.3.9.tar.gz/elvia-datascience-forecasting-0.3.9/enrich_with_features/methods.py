...
def enrich( # NOSONAR
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. Incoming df.shape: {str(df.shape)}, incoming weekly_dataset.shape: {str(weekly_dataset.to_pandas_dataframe().shape)}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. df.head() after setting index: {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. df.head() after applying dayofweek: {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. df.head() after applying hourofday: {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. df.head() after applying hourofweek: {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. after applying monthyear: {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. df.head() after resetting the index: {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. df.head() after merging with df_average_weekly: {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. df.head() after setting index and adding trafo: {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. df.head() after school_holiday : {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. df.head() after changing school_holiday dtype : {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'enrich {str(debugging_count)}. Final df.shape : {str(df.shape)}')
...
def ingest_data(  # NOSONAR
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'ingest_data {str(debugging_count)}. Incoming df.shape: {str(df.shape)}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'ingest_data {str(debugging_count)}. changed trafo dtype: {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'ingest_data {str(debugging_count)}. modified index: {str(df.head().to_string())}'
        )
        debugging_count += 1
        logger.debug(
            f'ingest_data {str(debugging_count)}. number of unique trafo: {str(df["trafo"].nunique())}'
        )
    ...
    if deployment:
        ...
        if logger:
            debugging_count += 1
            logger.debug(
                f'ingest_data {str(debugging_count)}. df.shape after filtering by trafo: {str(df.shape)}'
            )
        ...
        if logger:
            debugging_count += 1
            logger.debug(
                f'ingest_data {str(debugging_count)}. df after setting index: {str(df.head().to_string())}'
            )
        ...
        if logger:
            debugging_count += 1
            logger.debug(
                f'ingest_data {str(debugging_count)}. df after shifting the index: {str(df.head().to_string())}'
            )
        ...
        if logger:
            debugging_count += 1
            logger.debug(
                f'ingest_data {str(debugging_count)}. df.shape after resetting index and forward filling: {str(df.shape)}'
            )
        ...
        if logger:
            debugging_count += 1
            logger.debug(
                f'ingest_data {str(debugging_count)}. df after calling _check_input_data: {str(df.head().to_string())}'
            )
        ...
        if logger:
            debugging_count += 1
            logger.debug(
                f'ingest_data {str(debugging_count)}. df.head() after selecting trafo with more than 90% of data: {str(df.head().to_string())}'
            )
        ...
        if df.empty:
            if logger:
                debugging_count += 1
                logger.debug(
                    f'ingest_data {str(debugging_count)}. NotEnoughDataError')
        ...
        if logger:
            debugging_count += 1
            logger.debug(
                f'ingest_data {str(debugging_count)}. df.shape after removing outliers: {str(df.shape)}'
            )
            logger.debug(
                f'ingest_data {str(debugging_count)}. df.head() after removing outliers: {str(df.head().to_string())}'
            )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'ingest_data {str(debugging_count)}. df.head() after applying metadata: {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'ingest_data {str(debugging_count)}. df.head() after merging metadata: {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'ingest_data {str(debugging_count)}. df.head() after setting dtypes: {str(df.head().to_string())}'
        )
    ...
    if logger:
        debugging_count += 1
        logger.debug(
            f'ingest_data {str(debugging_count)}. df.head() after timezone manipulation: {str(df.head().to_string())}'
        )
        debugging_count += 1
        logger.debug(
            f'ingest_data {str(debugging_count)}. Final df.shape: {str(df.shape)}'
        )
