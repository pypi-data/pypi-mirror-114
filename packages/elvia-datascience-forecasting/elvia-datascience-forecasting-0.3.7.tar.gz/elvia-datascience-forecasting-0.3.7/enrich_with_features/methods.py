...
def enrich(df: pd.DataFrame,
    ...
    if logger:
        logger.debug(
            f'0. df shape: {str(df.shape)}, weekly_dataset shape: {str(weekly_dataset.to_pandas_dataframe().shape)}'
        )

    df = df.set_index('date_tz')
    if logger:
        logger.debug('1. after setting index: ' + df.head().to_string())
    ...
    df = dayofweek(df)
    if logger:
        logger.debug('2. after applying dayofweek: ' + df.head().to_string())

    # Adding hours of a day to dataframe
    df = hourofday(df)
    if logger:
        logger.debug('3. after applying hourofday: ' + df.head().to_string())

    # Adding hour of week to dataframe (0 - 167)
    df = hourofweek(df)
    if logger:
        logger.debug('4. after applying hourofweek: ' + df.head().to_string())

    # Adding month of a year to dataframe
    df = monthyear(df)
    if logger:
        logger.debug('5. after applying monthyear: ' + df.head().to_string())
    ...
    if logger:
        logger.debug('6. after resetting the index: ' + df.head().to_string())
