...
async def process_file(filename, tb_client, tag='', resource_versions=None, verbose=False):  # noqa: C901
...
async def build_graph(filenames, tb_client, tag='', resource_versions=None, process_dependencies=False, verbose=False):  # noqa: C901
    """process files"""
    ...
    async def process(filename, deps, dep_map, to_run):
        ...
        try:
            res = await process_file(filename, tb_client, tag, resource_versions=resource_versions, verbose=verbose)
...
async def folder_push(tb_client: TinyB, tag='', filenames=None, dry_run=False, check=False, push_deps=False,  # noqa: C901
    ...
    resources, dep_map = await build_graph(filenames, tb_client, tag=tag, process_dependencies=True)
