...
"""This module provides utility classes and functions for graphs."""
    ...
    if not reload:
        if config.graphml:
            if config.graphml.endswith(".graphml"):
                ...
                _path = os.path.join(filename + ext + suffix)
        else:
        ...
        try:
            ...
            raise FileNotFoundError(f"GraphML file at {_path} can not be found, do you need to set "
                                    "config.reload to True?")
    else:
...
def read_file(config, encoding=_ENCODING):
    ...
    for df in pd.read_csv(config.i, chunksize=config.chunksize, sep=config.csv_sep, index_col=0, low_memory=False,
        ...
        for i in [tuple(itemgetter(0, 1, config.sim_functions.index(config.sim) + 3 - 1)(x)) for x in df.values]:
            if i[2] > config.t:
                try:
