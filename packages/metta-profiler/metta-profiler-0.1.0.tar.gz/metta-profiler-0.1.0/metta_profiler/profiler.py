...
import metta_profiler.iter_utils as iter_utils
import metta_profiler.time_utils as time_utils

from metta_profiler.trace import Trace, TraceTouch
...
def touch_msg(msg: Dict[str, Any], name: str) -> Dict[str, Any]:
    if "trace" not in msg:
        logging.debug(
            "message tracing requires Dict[str, Any] to set field 'trace' of type Trace",
        )
        return msg
...
def pass_trace(src: Dict[str, Any], dest: Dict[str, Any]) -> None:
    if "trace" not in src:
        logging.debug(
            "message tracing requires Dict[str, Any] src to set field 'trace' of type Trace",
        )
        return
    dest["trace"] = src["trace"]


def copy_trace(trace: Trace, dest: Dict[str, Any]) -> None:
    dest["trace"] = trace
...
class ProtoTimer:
    def __init__(
        ...
        self._paths: Dict[str, List[Trace]] = defaultdict(list)
    ...
    def register(self, *msg: Dict[str, Any]):
        if not self.disable:
            for touch in msg:
                if "trace" not in touch:
                    raise ProfilerError(
                        "message tracing requires Dict[str, Any] to set field 'trace' of type Trace"
                    )

                self._traces[touch["trace"].id] = touch["trace"]
                self._paths[self.path_from_trace(touch["trace"])].append(
                    touch["trace"].id
                )
