...
def opensio(func):
    ...
    def wrapper(self, *args, **kwargs):
        ok = self.sio.IsOpen()
        if not ok:
            ports = self.sio.GetNumPorts(pidvids=PIDVIDS)
...
class TestLib(TestBase):
    ...
    def test_Constructor(self):
        ...
        with LIBUSBSIO(autoload=False) as sio:
            with self.assertRaises(Exception):
                sio.GetNumPorts(pidvids=PIDVIDS)
            with self.assertRaises(Exception):
                sio.Open()
            with self.assertRaises(Exception):
                sio.GetVersion()
    ...
    def test_GetNumPorts(self):
        v = self.sio.GetNumPorts(pidvids = [LIBUSBSIO.PIDVID_LPCLINK2])
        self.logger.info("GetNumPorts(LPCLINK2) returns %s" % v)
        v = self.sio.GetNumPorts(pidvids = [LIBUSBSIO.PIDVID_MCULINK])
