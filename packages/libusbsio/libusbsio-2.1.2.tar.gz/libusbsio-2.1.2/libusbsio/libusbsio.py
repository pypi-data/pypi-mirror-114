...
class LIBUSBSIO(object):
    ...
    # platforms
    (P_WIN32, P_WIN64, P_LINUX64, P_LINUX32, P_MACOS64) = (0,1,2,3,4)
    ...
    PIDVID_REDLINK  = (0x21BD, 0x0006)
    PIDVID_LPCLINK2 = (0x1FC9, 0x0090)
    PIDVID_MCULINK  = (0x1FC9, 0x0143)

    # all NXP USB devices
    PIDVID_NXP = (0x1FC9, 0)

    @need_dll_loaded
    def GetNumPorts(self, pidvids:'list[tuple[int,int]]' = None) -> int:
        ...
        ## Args
        - `pidvids` - List of [PID,VID] tuples identifying the device types to probe. Use None to probe default devices (PIDVID_LPCLINK2 and PIDVID_MCULINK)

        ## Returns
        The number of ports available on the USBSIO controller.
        '''
        if not pidvids:
            pidvids = [ LIBUSBSIO.PIDVID_LPCLINK2, LIBUSBSIO.PIDVID_MCULINK ]

        ret:int = 0
        for pv in pidvids:
            ret = self._GetNumPorts(pv[0], pv[1])
            self.logger.debug("Probing PID=0x%04x VID=0x%04x returned %d" % (pv[0], pv[1], ret))
            if(ret):
                self._pidvid = pv
                break
    ...
    @need_dll_loaded
    def HIDAPI_Enumerate(self, pidvid:'tuple[int,int]' = None, read_ex_info:bool = False) -> Generator[HIDAPI_DEVICE_INFO_T, None, None]:
        '''# USB HID enumeration generator

        ## Args:
        - `pidvid`   Tuple of [PID,VID] identifying the device types to enumerate.
        ...
        if not pidvid:
            pidvid = (0,0)    # enumerate all devices

        info = LIBUSBSIO.HIDAPI_DEVICE_INFO_T()
        h = self._HIDAPI_Enumerate(pidvid[0], pidvid[1], read_ex_info)
        if h:
...
def usbsio(*args, **kwargs) -> LIBUSBSIO:
    ...
    if not _LIBUSBSIO_SINGLETON:
        _LIBUSBSIO_SINGLETON = LIBUSBSIO(*args, **kwargs)
