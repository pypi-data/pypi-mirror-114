...
dataset: {self.dsId}         #用于指定需要上传至哪一个数据集
        ...
        if self.format not in ["voc", "coco"]:
            scriptText = '''from testindata.data.correspondenceAbstract import CorrespondenceAbstract #don't edit this line
from testindata.TDA import TDA #don't edit this line

#import packages as your wish
import json
import yaml

class Correspondence(CorrespondenceAbstract):
    def __init__(self, upConfig, tda:TDA):#don't edit this line
        self.upConfig = upConfig #the config data you apply
        self.tda = tda #the testindata SDK gloabal object that used to upload your data to dataset system

        print(self.upConfig.conf)
        print(self.upConfig.filePathList[0:10])
        print(self.upConfig.annPathList[0:10])
        
        # print(self.upConfig.format)
        # print(self.upConfig.path)
        # print(self.upConfig.fe)
        # print(self.upConfig.apath)
        # print(self.upConfig.at)
        # print(self.upConfig.afe)
        # print(self.upConfig.dsId)
        

    def AddFiles(self):
        """
        edit following code to upload your data, more about: https://testindata.gitbook.io/dataset/python-sdk/untitled
        """
        for path in self.upConfig.filePathList:
            fileMetaData = {
                "key1":"val1",
                "key2":"val2"
            }
            
            objectName = path.replace(self.upConfig.path, "").strip("/")

            f = self.tda.AddFile(path, objectName=objectName, metaData=fileMetaData)
    ...
    def appliy(self, cfPath, corrPath="", tda:TDA=None):
        ...
        if self.format == "customize":
            ...
            try:
                ...
                print(e)
                raise Exception(f"can't import Correspondence")
