...
from testindata.data.uploadData import UploadData
...
@upload.command()
@click.option("-format", "--format", 'format', default="voc", help="your uploading-data format, default:voc")
@click.option("-f", "--file-path", 'path', default="path/to/your/files", help="your uploading-data path")
@click.option("-fe", "--file-extension", 'fe', default=[".jpg"], help="your uploading-data file extension name")
@click.option("-af", "--annotation-path", 'apath', default="path/to/your/annotations", help="your annotation file path")
@click.option("-at", "--annotation-type", 'at', default="xml", help="your annotation file content type: xml、json etc.")
@click.option("-afe", "--annotation-extension", 'afe', default=[".xml", ".json"], help="your annotation file extension name")
@click.option("-ds", "--datasetId", 'ds_id', default="ds_*************", help="the dataset you wolud like to upload")
@click.pass_context
def init(ctx, format, path, fe, apath, at, afe, ds_id):
    """
    Initial uploading-configuration
    """
    ud = UploadData(format=format, path=path, fe=fe, apath=apath, at=at, afe=afe, ds_id=ds_id)
    ud.init()


@upload.command()
@click.option("-cf", "--config", 'config', default="", help="upload configuration file path")
@click.option("-script", "--script", 'script', default="", help="upload correspond script file path")
@click.pass_context
def apply(ctx, config, script):
    """
    Initial uploading-configuration
    """
    ud = UploadData()
    if config == "":
        click.echo('upload configuration file must be set')

    ud.appliy(config, script, ctx.obj)
