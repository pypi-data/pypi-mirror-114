...
def spawn(name, digitalocean_token):
    ...
    do.get_ssh("root", ip_address, process_out)(
        "wget https://raw.githubusercontent.com/canopy/gaea/main/spawn.sh -qO- | bash",
    )
    print("spawning the understory..", flush=True)
    do.get_ssh("gaea", ip_address, process_out)(f"./gaea setup {digitalocean_token}")
...
def get_app():
    ...
    namebox = QLineEdit(window)
    namebox.move(10, 40)

    tokenlabel = QLabel(window)
    tokenlabel.setText("DigitalOcean API Token")
    tokenlabel.move(10, 80)

    tokenbox = QLineEdit(window)
    tokenbox.move(10, 110)

    @Slot()
    def deploy():
        spawn(namebox.text(), tokenbox.text())
        label = QLabel(window)
