...
class RecordPDF(PDF):
    ...
    def print_extras(self):
        ...
        if self.record.extras:
            ...
            self._print_extras(self.record.extras)
            self.set_draw_color(r=0, g=0, b=0)
        else:
    ...
    def print_record_links(self):
        ...
        record_links = get_permitted_record_links(self.record, user=self.user).order_by(
            RecordLink.created_at.desc()
        )
...
def _get_json_data(record, user):
    # Unnecessary meta attributes to exclude.
    ...
    # Include all record links as "links". Only the basic metadata is included for each
    # record, while the current record is always excluded completely, regardless of link
    # direction.
    record_links = get_permitted_record_links(record, user=user).order_by(
        RecordLink.created_at.desc()
    )
    data["links"] = []
    ...
    for link in record_links:
        ...
        if link.record_to_id == record.id:
        ...
        data["links"].append(schema.dump(link))

    # Include all collections the record is part of as "collections". Only the basic
    # metadata is included for each collection.
    collections = get_linked_resources(
...
def get_export_data(record, export_type, user=None):
    ...
    if export_type == "json":
        return _get_json_data(record, user)

    if export_type == "pdf":
        return _get_pdf_data(record, user)
