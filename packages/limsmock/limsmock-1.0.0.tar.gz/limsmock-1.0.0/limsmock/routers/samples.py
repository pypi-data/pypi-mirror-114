...
def get_sample(entity_id, request: Request):
    db = request.app.db

    return Response(content=db['samples'].get(entity_id))


@router.put("/{entity_id}")
async def put_sample(entity_id, request: Request):
    body = await request.body()

    request.app.db['samples'][entity_id] = body
