...
class Corps:
    ...
    def __init__(self, code: str, table: str):
        if utils.is_6digit(code) and table in self.TABLE:
            self.code = code
            self.table = table
            self.engine = self.make_engine(code=code)
    ...
    @staticmethod
    def make_engine(code: str, echo=False):
        # make folder - /db/corp_db
        path = os.path.join(load_setting()['sqlite3'], 'corps')
        ...
        if table in self.TABLE:
            with self.engine.connect() as conn:
                ...
                if table == 'c101' or table == 'stamp':
                    # c101 과 stamp 테이블은 Base.metadata.create_all(self.engine) 명령어로 재생성 될 것임..
