...
class BBBDLC:
    def run(args):
        ...
        try:
            ...
            occCommand = "sudo -u adminroot /opt/plesk/php/7.3/bin/php " + serverDir + "/occ "
            ...
            if ncPath == "":
                raise NameError('No nextcloud path specified!')
            elif not os.path.isdir(ncPath):
                raise NameError('Invalid nextcloud path specified!\n' + ncPathFile.read())
            else:
                ncPathFile.close()
            ...
            for arg in vars(args):
                if arg == "recordId" or arg == "server" or arg == "secret" or arg == "serverDir": continue
            ...
            os.system("sudo -u adminroot ianes-bbb-dl " + passthroughArgs)
