...
SENT_EMAILS=[]
EMAIL_BLACKLIST=NroEx()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",NroEc(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=NroEm,from_name=NroEm,smtp_host=NroEm,smtp_user=NroEm,smtp_pass=NroEm,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if NroEp(recipients,NroEM)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=NroEm):
 ...
 result["From"]=formataddr((NroEc(Header(from_name,"utf-8")),from_email))
 return result
