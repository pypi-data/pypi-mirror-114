...
SENT_EMAILS=[]
EMAIL_BLACKLIST=FNAWK()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",FNAWU(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=FNAWG,from_name=FNAWG,smtp_host=FNAWG,smtp_user=FNAWG,smtp_pass=FNAWG,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if FNAWq(recipients,FNAWH)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=FNAWG):
 ...
 result["From"]=formataddr((FNAWU(Header(from_name,"utf-8")),from_email))
 return result
