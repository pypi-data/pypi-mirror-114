...
SENT_EMAILS=[]
EMAIL_BLACKLIST=qhtnl()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",qhtnz(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=qhtnM,from_name=qhtnM,smtp_host=qhtnM,smtp_user=qhtnM,smtp_pass=qhtnM,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if qhtnY(recipients,qhtnc)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=qhtnM):
 ...
 result["From"]=formataddr((qhtnz(Header(from_name,"utf-8")),from_email))
 return result
