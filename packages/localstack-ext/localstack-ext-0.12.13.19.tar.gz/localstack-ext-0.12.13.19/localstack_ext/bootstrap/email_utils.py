...
SENT_EMAILS=[]
EMAIL_BLACKLIST=cAJbX()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",cAJbe(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=cAJbI,from_name=cAJbI,smtp_host=cAJbI,smtp_user=cAJbI,smtp_pass=cAJbI,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if cAJbB(recipients,cAJbG)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=cAJbI):
 ...
 result["From"]=formataddr((cAJbe(Header(from_name,"utf-8")),from_email))
 return result
