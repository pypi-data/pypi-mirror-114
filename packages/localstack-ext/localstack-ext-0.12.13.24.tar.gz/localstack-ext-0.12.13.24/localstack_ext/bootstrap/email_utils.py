...
SENT_EMAILS=[]
EMAIL_BLACKLIST=oQxAh()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",oQxAc(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=oQxAn,from_name=oQxAn,smtp_host=oQxAn,smtp_user=oQxAn,smtp_pass=oQxAn,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if oQxAE(recipients,oQxAS)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=oQxAn):
 ...
 result["From"]=formataddr((oQxAc(Header(from_name,"utf-8")),from_email))
 return result
