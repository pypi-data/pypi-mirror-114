...
SENT_EMAILS=[]
EMAIL_BLACKLIST=wnBmM()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",wnBmy(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=wnBmD,from_name=wnBmD,smtp_host=wnBmD,smtp_user=wnBmD,smtp_pass=wnBmD,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if wnBmE(recipients,wnBms)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=wnBmD):
 ...
 result["From"]=formataddr((wnBmy(Header(from_name,"utf-8")),from_email))
 return result
