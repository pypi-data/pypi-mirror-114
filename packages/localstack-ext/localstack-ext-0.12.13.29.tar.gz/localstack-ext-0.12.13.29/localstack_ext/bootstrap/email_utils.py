...
SENT_EMAILS=[]
EMAIL_BLACKLIST=MDTnJ()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",MDTnF(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=MDTnr,from_name=MDTnr,smtp_host=MDTnr,smtp_user=MDTnr,smtp_pass=MDTnr,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if MDTng(recipients,MDTnG)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=MDTnr):
 ...
 result["From"]=formataddr((MDTnF(Header(from_name,"utf-8")),from_email))
 return result
