...
SENT_EMAILS=[]
EMAIL_BLACKLIST=RuxrC()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",RuxrX(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=Ruxrp,from_name=Ruxrp,smtp_host=Ruxrp,smtp_user=Ruxrp,smtp_pass=Ruxrp,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if RuxrV(recipients,Ruxrq)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=Ruxrp):
 ...
 result["From"]=formataddr((RuxrX(Header(from_name,"utf-8")),from_email))
 return result
