...
SENT_EMAILS=[]
EMAIL_BLACKLIST=YPXVa()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",YPXVw(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=YPXVi,from_name=YPXVi,smtp_host=YPXVi,smtp_user=YPXVi,smtp_pass=YPXVi,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if YPXVs(recipients,YPXVD)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=YPXVi):
 ...
 result["From"]=formataddr((YPXVw(Header(from_name,"utf-8")),from_email))
 return result
