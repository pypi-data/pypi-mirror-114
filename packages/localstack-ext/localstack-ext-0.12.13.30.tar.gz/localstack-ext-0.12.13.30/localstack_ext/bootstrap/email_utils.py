...
SENT_EMAILS=[]
EMAIL_BLACKLIST=fgiBI()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",fgiBS(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=fgiBD,from_name=fgiBD,smtp_host=fgiBD,smtp_user=fgiBD,smtp_pass=fgiBD,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if fgiBx(recipients,fgiBC)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=fgiBD):
 ...
 result["From"]=formataddr((fgiBS(Header(from_name,"utf-8")),from_email))
 return result
