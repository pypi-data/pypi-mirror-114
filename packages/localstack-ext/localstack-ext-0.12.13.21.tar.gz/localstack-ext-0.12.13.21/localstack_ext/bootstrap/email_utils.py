...
SENT_EMAILS=[]
EMAIL_BLACKLIST=mDjdH()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",mDjdN(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=mDjdC,from_name=mDjdC,smtp_host=mDjdC,smtp_user=mDjdC,smtp_pass=mDjdC,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if mDjdf(recipients,mDjdE)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=mDjdC):
 ...
 result["From"]=formataddr((mDjdN(Header(from_name,"utf-8")),from_email))
 return result
