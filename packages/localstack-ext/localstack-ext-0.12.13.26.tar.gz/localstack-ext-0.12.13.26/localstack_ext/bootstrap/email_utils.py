...
SENT_EMAILS=[]
EMAIL_BLACKLIST=ENzJG()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",ENzJS(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=ENzJH,from_name=ENzJH,smtp_host=ENzJH,smtp_user=ENzJH,smtp_pass=ENzJH,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if ENzJT(recipients,ENzJD)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=ENzJH):
 ...
 result["From"]=formataddr((ENzJS(Header(from_name,"utf-8")),from_email))
 return result
