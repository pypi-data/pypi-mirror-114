...
class NumberRecognizer:
    def __init__(self) -> None:
        # rectList = []
        # rectList.append([0,0,0,0])
        # rectList.append([1,1,1,1])
        # rectList.append([2,2,2,2])
        ...
        #     print( x,y,w,h,sep='   ' )
        self.model_path = pkg_resources.resource_filename(__package__,"res/model/mnist.tflite")
    ...
    def __call__(self, image) -> np.array:
        ...
        canny = cv2.Canny(processedImg,0,100)
        processedImg = cv2.dilate(canny, self.kernel, anchor=(-1,1), iterations=1)
        ...
        if numberImage is not None:
            num = self.__findNumeric(gray, canny)
    ...
    def __findNumeric(self, grayImg, cannyImg) -> str:
        ...
        numberStr = ''
        for x,y,w,h in rectList:
            numRoi = grayImg[x:x+w, y:y+h]
            numRoi = self.__makeSquare(numRoi)
            self.__recognizeNumericImage(numRoi)
    ...
    def __recognizeNumericImage(self, image):
        ...
        numberImg = cv2.bitwise_not(image)
        numberImg = cv2.erode(numberImg,self.kernel)
        numberImg = cv2.threshold(numberImg, 96, 255, cv2.THRESH_BINARY)
        
        numberImg = np.expand_dims(numberImg, axis=0)
        numberImg = numberImg.astype(np.float32)

        try:
            self.model.set_tensor(self.input_details[0]['index'],numberImg)
            self.model.invoke()
