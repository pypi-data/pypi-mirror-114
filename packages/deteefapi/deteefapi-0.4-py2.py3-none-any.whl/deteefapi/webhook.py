...
class WebhookComment:
    ...
    @cached_property
    def reply_to(self) -> WebhookComment:
        return WebhookComment(self._data.get("reply_to", {}))
...
class WebhookData:
    def __init__(self, data: list):
        ...
        """
        self._data = data
        self._urls_events = dict(((item['url'], item['event']) for item in self))

    def __iter__(self) -> Generator[dict, None, None]:
        return (item for item in self._data)

    def is_webhook(self, webhook_url, event_type='new_comment'):
        return webhook_url in self._urls_events and self._urls_events[webhook_url] == event_type
