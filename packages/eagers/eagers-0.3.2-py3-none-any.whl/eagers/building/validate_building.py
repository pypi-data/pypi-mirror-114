from eagers.building.plot_sim import plot_sim
from eagers.building.process.building_warmup import building_warmup
from eagers.building.process.run_building import run_building
from eagers.building.process.run_sim import run_sim
from eagers.building.basic.interpolate_weather import interpolate_weather
from eagers.class_definition.building import Building
from eagers.config.building import BUILD_NAMES, CLIMATE_NAMES
from eagers.config.path_spec import USER_DIR_BUILDINGS, USER_DIR_ENERGYPLUS_WEATHER, USER_DIR_EPLUS
from eagers.read.example_file import load_example_file
from eagers.read.weather_file import read_weather_file
from eagers.read.load_data import load_data
import pickle
...
def validate_building(build = 1,climate = '4A', start_day = 1, duration = 10, horizon = 0, mode = 'build',warmup = 'new'):
    ...
    if warmup == 'new':
        ...
        try:
            weather = read_weather_file(load_example_file(climate + '_TMY2.epw', USER_DIR_ENERGYPLUS_WEATHER),'tmy')
            building = Building.from_file(load_example_file('RefBldg'+build + 'New2004_v1.4_7.2_' + climate, USER_DIR_BUILDINGS))
        except FileNotFoundError:
            weather = read_weather_file(USER_DIR_ENERGYPLUS_WEATHER / (climate + '_TMY2.epw'),'tmy')
            building = Building.from_file(USER_DIR_BUILDINGS / ('RefBldg'+build + 'New2004_v1.4_7.2_' + climate))
    ...
    if mode == 'build':
        ...
        try:
            plot_sim(facility,T_zone,Q_transfer,building,date[1:], load_example_file('RefBldg'+build + 'New2004_v1.4_7.2_' + climate+'.csv', USER_DIR_EPLUS))
        except FileNotFoundError:
