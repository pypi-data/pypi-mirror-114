...
from eagers.config.path_spec import USER_DIR_BUILDINGS, USER_DIR_ENERGYPLUS_WEATHER, USER_DIR_EPLUS, DEMO_DIR
...
def validate_building(build = 1,climate = '4A', start_day = 1, duration = 10, horizon = 0, mode = 'build',warmup = 'new'):
    ...
    if warmup == 'new':
        ...
        try:
            weather = read_weather_file(DEMO_DIR / (climate + '_TMY2.epw'),'tmy')
            building = Building.from_file(DEMO_DIR / ('RefBldg'+build + 'New2004_v1.4_7.2_' + climate))
        except:
    ...
    if mode == 'build':
        ...
        try:
            plot_sim(facility,T_zone,Q_transfer,building,date[1:],DEMO_DIR / ('RefBldg'+build + 'New2004_v1.4_7.2_' + climate+'.csv'))
        except:
