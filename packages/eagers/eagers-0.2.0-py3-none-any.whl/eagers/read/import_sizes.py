...
from eagers.config.building import EIO_FIELDS
...
def import_sizes(filename,building):
    ...
    for t in range(len(component['type'])):
        ...
        if len(i_loc)==2:
            if i_loc[1] in getattr(building,i_loc[0]):
                i = getattr(building,i_loc[0])[i_loc[1]].index(component['name'][t])
            else:
                i = component['name'][t]
        elif len(i_loc)==3:
            i = getattr(building,i_loc[0])[i_loc[1]][i_loc[2]].index(component['name'][t])        
        
        try:
            par = EIO_FIELDS[component['type'][t]][component['field'][t]][0]
            if len(i_loc)==1:
                getattr(building,i_loc[0])[component['name'][t]][par] = component['value'][t]
            elif len(i_loc)==2:
                if i_loc[1]=='name' and 'name' not in getattr(building,i_loc[0]):
                    getattr(building,i_loc[0])[i][par] = component['value'][t]
                else:
                    getattr(building,i_loc[0])[par][i] = component['value'][t]
            elif len(i_loc)==3:
                getattr(building,i_loc[0])[i_loc[1]][par][i] = component['value'][t]
        except KeyError:
