...
from eagers.config.path_spec import USER_DIR_BUILDINGS, USER_DIR_ENERGYPLUS_WEATHER, USER_DIR_EPLUS, DATA_CACHE_PATH
from eagers.read.weather_file import read_weather_file
from eagers.read.load_data import load_data
from eagers.read.online_data import load_dataset
import pickle
...
def validate_building(build = 1,climate = '4A', start_day = 1, duration = 10, horizon = 0, mode = 'build',warmup = 'new'):
    ...
    if warmup == 'new':
        ...
        try:
            weather = read_weather_file(load_dataset(climate + '_TMY2.epw'),'tmy')
            building = Building.from_file(load_dataset('RefBldg'+build + 'New2004_v1.4_7.2_' + climate))
        except:
    ...
    if mode == 'build':
        ...
        try:
            plot_sim(facility,T_zone,Q_transfer,building,date[1:], load_dataset('RefBldg'+build + 'New2004_v1.4_7.2_' + climate+'.csv'))
        except:
