...
class BaseTemplateReader(BaseTemplateHandler):
    ...
    def read(self, filename=None):
        ...
        if self.filename == self._example_filename:
            wb = load_workbook(
                load_example_file(self._example_filename, self._user_dirpath),
                read_only=True,
                data_only=True,
            )
        else:
...
class ProjectTemplateReader(ProjectTemplateHandler, BaseTemplateReader):
    ...
    _user_dirpath = USER_DIR_PROJECTS
    ...
    def _read_buildings_sheet(cls, wb, ws):
        ...
        for row in values:
            filename = row[0]
            try:
                build = Building.from_file(
                    load_example_file(filename, USER_DIR_BUILDINGS)
                )
...
class TestDataTemplateReader(TestDataTemplateHandler, BaseTemplateReader):
    ...
    _user_dirpath = USER_DIR_TESTDATA
        ...
        if filename:
            try:
                weather_data = read_weather_file(
                    ensure_suffix(
                        load_example_file(filename, USER_DIR_ENERGYPLUS_WEATHER),
                        EPW_SUFFIX,
                    ),
                    designation,
                )
