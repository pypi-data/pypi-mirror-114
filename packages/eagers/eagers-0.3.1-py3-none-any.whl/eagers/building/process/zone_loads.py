...
from eagers.building.basic.eval_curve import eval_curve
from eagers.building.components.water_equipment import water_equipment
from eagers.building.basic.window_solar_transmittance import window_solar_transmittance
from eagers.building.basic.solar_calc import solar_calc
...
def zone_loads(building,date,profile,weather,T_zone,m_v_zone,dt,frost):
    ...
    for j in prop:
        ...
        for i in range(len(getattr(building,j)['name'])):
            z_i = getattr(building,j)['zone'][i]
            sched_i = getattr(building,j)['schedule'][i]
            loads[j][z_i] += profile[sched_i]*getattr(building,j)['nominal'][i]*building.zones['floor_area'][z_i] 
            loads['internal_gain'][z_i] += loads[j][z_i]
            for c in cat:
                if c in getattr(building,j):
                    loads[c][z_i] += loads[j][z_i]*getattr(building,j)[c][i]
    ## Solar
