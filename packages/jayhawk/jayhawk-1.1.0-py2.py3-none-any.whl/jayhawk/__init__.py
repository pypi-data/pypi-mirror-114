...
def serve_multiple(handlers,server_address=("0.0.0.0",300),server_cls=ThreadingTCPServer,timeout_interval=0.5,ret=False):
	"""Convenience function. Makes a subclass of VHostSpartanHandler with the HOSTS attribute set to handlers, and passes it to serve."""
	handler = type("VHostServer",(VHostSpartanHandler,),{"HOSTS":handlers})
	return serve(handler,server_address,server_cls,timeout_interval,ret)
