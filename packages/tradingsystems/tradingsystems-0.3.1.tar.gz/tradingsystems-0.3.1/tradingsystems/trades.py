...
class Positions():
    ...
    def _position_values(
        ...
        initial_position_value = np.array([0]*len(df))
        current_position_value = np.array([0]*len(df))
        max_trade_position_value = np.array([0]*len(df))
        max_trade_close_position_value = np.array([0]*len(df))
        min_trade_position_value = np.array([0]*len(df))
        min_trade_close_position_value = np.array([0]*len(df))
            ...
            if trade_number[row] != 0:
                ...
                if (end_of_day_position[row] == 0) and (
                    ...
                    current_position_value[row] = (
                        df['Open'][row] * end_of_day_position[row-1])
                    ...
                    # multiplied by the end of day position
                    current_position_value[row] = (
                        df['Close'][row] * end_of_day_position[row])
    ...
    @staticmethod
    def _position_size(df, benchmark, equity):
        """
