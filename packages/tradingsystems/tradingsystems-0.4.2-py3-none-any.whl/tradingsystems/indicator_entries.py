...
class IndicatorEntry():
    ...
    def entry_channel_breakout(prices, time_period):
        ...
        for row in range(start, len(rolling_high_close)):
            ...
            if prices['Close'][row] >= rolling_high_close[row]:
            ...
            elif prices['Close'][row] <= rolling_low_close[row]:
    ...
    def entry_momentum(prices, time_period, threshold):
        ...
        for row in range(start, len(n_day_close)):
            ...
            # If the price rises to equal or above the n-day high close
            if prices['Close'][row] >= n_day_close[row] + threshold:
            ...
            # If the price falls to equal or below the n-day low close
            elif prices['Close'][row] <= n_day_close[row] - threshold:
    ...
    def entry_volatility(prices, time_period, threshold):
        ...
        for row in range(start, len(atr)):
            ...
            elif ((prices['Close'][row-1] - prices['Close'][row])
                  > (atr[row] * threshold)):
