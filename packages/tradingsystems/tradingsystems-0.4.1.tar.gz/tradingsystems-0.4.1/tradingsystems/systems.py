...
class Data():
    """
    Class to calculate the trading system performance.
    ...
    def __init__(self):
        ...
        self.entry_signal_labels = self.default_dict['df_entry_signal_labels']
        self.exit_signal_labels = self.default_dict['df_exit_signal_labels']
        self.stop_signal_labels = self.default_dict['df_stop_signal_labels']

        # Entry, Exit and Stop dictionaries of defaults
        self.entry_signal_dict = self.default_dict['df_entry_signal_dict']
        self.exit_signal_dict = self.default_dict['df_exit_signal_dict']
        self.stop_signal_dict = self.default_dict['df_stop_signal_dict']

        # Entry signal indicator column names
        self.entry_signal_indicators = self.default_dict[
        ...
        **kwargs : Various
        ...
        for key, value in kwargs.items():
            ...
            if value is None:
                ...
                value = self.default_dict['df_'+str(key)]
    ...
    def _refresh_signals_default(
            self, entry_type, exit_type, stop_type, **kwargs):
        """
        ...
        **kwargs : Various
        ...
        for key, value in kwargs.items():
            ...
            if value is None:
                ...
                if str(key) in self.entry_signal_dict[str(entry_type)].keys():

                    # Extract these from the df_combo_dict
                    value = self.entry_signal_dict[str(
                        entry_type)][str(key)]
                ...
                elif str(key) in self.exit_signal_dict[str(exit_type)].keys():

                    # Extract these from the exit signal dict
                    value = self.exit_signal_dict[str(
                        exit_type)][str(key)]
                ...
                # Check if the key is in the stop signal dict
                elif str(key) in self.stop_signal_dict[str(stop_type)].keys():

                    # Extract these from the df_combo_dict
                    value = self.stop_signal_dict[str(
                        stop_type)][str(key)]
    ...
    def test_strategy(
            ...
            commission=None, riskfree=None, entry_type=None,
         ...
         exit_amount, stop_amount) = itemgetter(
             ...
             'exit_acceleration_factor', 'sip_price', 'exit_amount', \
                 'stop_amount')(self._refresh_signals_default(
                     entry_type=entry_type, exit_type=exit_type,
                     stop_type=stop_type,ma1=ma1, ma2=ma2, ma3=ma3, ma4=ma4,
                     entry_period=entry_period, exit_period=exit_period,
                     stop_period=stop_period, entry_oversold=entry_oversold,
        ...
        self.start_date, self.end_date = Dates.date_set(
        ...
        prices = Markets.create_base_data(
        ...
        self.entry_label, self.exit_label, \
            self.stop_label = Labels.strategy_labels(
            prices=prices, ma1=self.ma1, ma2=self.ma2, ma3=self.ma3, ma4=self.ma4,
            ...
            sip_price=self.sip_price, default_dict=self.default_dict)

        # Generate initial trade data
        prices, self.start, self.position_size, \
            self.benchmark_position_size = self._raw_entry_signals(
                prices=prices, entry_type=self.entry_type, ma1=self.ma1, ma2=self.ma2,
        ...
        prices = self._exit_targets(prices=prices, exit_amount=self.exit_amount,
                                position_size=self.position_size)

        prices = self._stop_targets(prices=prices, stop_amount=self.stop_amount,
                                position_size=self.position_size)

        # Create exit and stop signals
        prices = Exit.exit_and_stop_signals(
            prices=prices, exit_type=self.exit_type,
            stop_type=self.stop_type, exit_period=self.exit_period,
            stop_period=self.stop_period, exit_threshold=self.exit_threshold,
            exit_oversold=self.exit_oversold,
            exit_overbought=self.exit_overbought,
            exit_acceleration_factor=self.exit_acceleration_factor,
            sip_price=self.sip_price, trade_number=prices['raw_td_number'],
            end_of_day_position=prices['raw_eod_position'],
            exit_trailing_close=prices['exit_trailing_close'],
            exit_profit_target=prices['exit_profit_target'],
            stop_initial_dollar_loss=prices['stop_initial_dollar_loss'],
            stop_profit_target=prices['stop_profit_target'],
            stop_trailing_close=prices['stop_trailing_close'],
            stop_trailing_high_low=prices['stop_trailing_high_low'])

        # Combine signals
        prices['combined_signal'] = Positions.signal_combine(
            prices=prices, start=self.start, raw_trade_signal=prices['raw_td_signal'],
            end_of_day_position=prices['raw_eod_position'],
            trade_number=prices['raw_td_number'],
            exit_signal=prices['exit_signal'],
            stop_signal=prices['stop_signal'])

        # Create trade and position data
        prices['start_of_day_position'], prices['trade_signal'], \
            prices['position'] = Positions.positions_and_trade_actions(
                prices=prices, signal=prices['combined_signal'], start=self.start,
                position_size=self.position_size)

        prices['trade_number'] = Positions.trade_numbers(
            prices=prices, end_of_day_position=prices['position'],
            start=self.start)

        # Calculate the trades and pnl for the strategy
        self.prices = Profit.profit_data(
            prices=prices, position_size=self.position_size, slippage=self.slippage,
            commission=self.commission, equity=self.equity)

        # Create monthly summary data
        self.monthly_data = Profit.create_monthly_data(
            prices=self.prices, equity=self.equity)

        # Create dictionary of performance data
        self.perf_dict = PerfReport.performance_data(
            prices=self.prices, monthly_data=self.monthly_data,
    ...
    def _raw_entry_signals(
            prices, entry_type, ma1, ma2, ma3, ma4, entry_period, entry_oversold,
        ...
        prices, start, prices['raw_td_signal'] = Entry.entry_signal(
            prices=prices, entry_type=entry_type, ma1=ma1, ma2=ma2, ma3=ma3, ma4=ma4,
        ...
        position_size, benchmark_position_size = Positions.position_size(
                prices=prices, benchmark=benchmark, equity=equity)

        # Calculate initial position info
        prices['raw_start_of_day_position'], prices['raw_td_action'], \
            prices['raw_eod_position'] = Positions.positions_and_trade_actions(
                prices=prices, signal=prices['raw_td_signal'], start=start,
                position_size=position_size)

        # Generate trade numbers
        prices['raw_td_number'] = Positions.trade_numbers(
            prices=prices, end_of_day_position=prices['raw_eod_position'],
            start=start)

        # Generate initial trade prices
        prices['raw_td_entry_price'], prices['raw_td_exit_price'], \
            prices['raw_td_high_price'], prices['raw_td_low_price'], \
                prices['raw_td_close_high_price'], \
                    prices['raw_td_close_low_price'] = Positions.trade_prices(
                        prices=prices, trade_number=prices['raw_td_number'])

        return prices, start, position_size, benchmark_position_size


    @classmethod
    def _exit_targets(cls, prices, exit_amount, position_size):
        """
        ...
        prices['exit_profit_target'], prices['exit_initial_dollar_loss'], \
            prices['exit_trailing_close'], \
                prices['exit_trailing_high_low'] = Positions.pnl_targets(
                    ...
                    trade_number=prices['raw_td_number'],
                    end_of_day_position=prices['raw_eod_position'],
                    trade_entry_price=prices['raw_td_entry_price'],
                    trade_high_price=prices['raw_td_high_price'],
                    trade_close_high_price=prices['raw_td_close_high_price'],
                    trade_low_price=prices['raw_td_low_price'],
                    trade_close_low_price=prices['raw_td_close_low_price'])

        return prices


    @classmethod
    def _stop_targets(cls, prices, stop_amount, position_size):
        """
    ...
    def performance_graph(self, signals=None):
        ...
        if self.entry_type in ['2ma', '3ma', '4ma', 'sar', 'channel_breakout']:
            graph = perfgraph.two_panel_graph(
                signals=signals, prices=self.prices, entry_type=self.entry_type,
