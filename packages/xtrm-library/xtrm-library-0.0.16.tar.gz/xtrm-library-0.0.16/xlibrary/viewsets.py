...
class ModelViewset(DynamicModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.action=='report':
            ...
            if user.has_perm(strarray[0] + ".change_" + strarray[1]):
                if not (len(self.rowActions) and self.rowActions[0]['type'].lower()=='edit'):
                    self.rowActions.insert(0,{'label': 'Edit', 'type': 'edit'})
            if user.has_perm(strarray[0] + ".delete_" + strarray[1]):
                if not (len(self.rowActions) and self.rowActions[len(self.rowActions)-1]['type'].lower() == 'delete'):
                    self.rowActions.append({'label': 'Delete', 'type': 'delete'})
