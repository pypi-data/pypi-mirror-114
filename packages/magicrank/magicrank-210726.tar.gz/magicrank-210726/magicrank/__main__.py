...
def portfolio(args):
    ...
    prev_names = set([s['name'] for s in prev['data']
                      if s['rank'] < args.hold*len(prev['data'])])
    stock_names = set([s['name'] for s in stock_list
                      if s['rank'] < args.hold*len(stock_list)])
...
def main():
    ...
    parser.add_argument('--hold', dest='hold', type=float, default=0.2)
