    ...
    def handle_flowable(self, flowables):
        ...
        if isinstance(f, PageBreak):
            ...
            if isinstance(f, Image):
                ...
                if f._width > frame._aW:
                    factor = frame._aW / f._width
                    f = self._scaleApply(f, factor)
                    if self.debug:
                        print("Image exceeds available width on frame:",
                              frame.id, frame._aW / cm, frame._aH / cm,
                              f._width / cm, f._height / cm,
                              "rescaling to fit frame geometry.")

                #resizing image if drawHeight is exceeding the available frame Width _aH
                if f._height > frame._aH:

                    # print(f"Image height exceeds height of available space on frame:",
                    #      frame.id,
                    #      "rescaling to fit frame geometry..." )

                    factor = frame._aH / f._height
                    f = self._scaleApply(f, factor)
