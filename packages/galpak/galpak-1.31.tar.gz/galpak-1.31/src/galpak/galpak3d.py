...
acceptance_rate = %s
    ...
    def _set_psf(self):
        ...
        if psf_type == 'moffat':
            ...
            if tmp_alpha is None and tmp_fwhm is None:
                self.logger.error("CONFIG: PSF: alpha and fwhm are not specified.Need at least one for Moffat PSF")
            else:
                psf = MoffatPointSpreadFunction(
                    fwhm=tmp_fwhm,  # psf_fwhm,
                    alpha=tmp_alpha,
