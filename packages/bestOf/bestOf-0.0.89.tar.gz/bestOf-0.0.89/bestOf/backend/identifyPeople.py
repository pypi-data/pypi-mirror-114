...
def crop_subjects_from_segmented_image(image, png=False, n=8):
        ...
        for j in range(n):
            ...
            for idx, sub in enumerate(subs):
                for existing_sub in all_subjects:
                    sub = (
                        sub * 255).astype(np.uint8)[:, :, :3] if png else sub
                    existing_sub = (
                        existing_sub * 255).astype(np.uint8)[:, :, :3] if png else existing_sub
