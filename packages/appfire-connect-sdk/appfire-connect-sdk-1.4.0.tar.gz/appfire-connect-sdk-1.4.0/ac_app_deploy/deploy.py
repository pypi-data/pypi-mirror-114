...
def process(verbose, command, profile, env, stage, stack, app_suffix, build, api_domain, api_stage, app_api_domain, require_approval, tag, bitbucket_username, bitbucket_password, bitbucket_branch, bitbucket_tag_value):
    """
    ...
    if command.lower() == 'version':
        print("Appfire Connect SDK version " + version('appfire-connect-sdk'))
        return
    ...
    if command.lower() == 'bootstrap':
        ...
        if stack == 'core':
            deploy_core(logger,app_name,api_stage,app_domain,brand,env,app_suffix,aws_profile,registry_url,tag)
            ...
            deploy_service_module(logger,stack,True,app_name,api_stage,app_domain,brand,env,app_suffix,aws_profile,registry_url,tag)
        elif stack == 'ac-automation':
            deploy_service_module(logger,stack,True,app_name,api_stage,app_domain,brand,env,app_suffix,aws_profile,registry_url,tag)
        elif stack == 'ac-core':
            deploy_service_module(logger,stack,False,app_name,api_stage,app_domain,brand,env,app_suffix,aws_profile,registry_url,tag)
        elif stack == 'ac-data-engine-api':
            deploy_service_module(logger,"ac-data-engine-api",False,app_name,api_stage,app_domain,brand,env,app_suffix,aws_profile,registry_url,tag)
        elif stack == 'ac-datastore':
            deploy_service_module(logger,"ac-datastore",False,app_name,api_stage,app_domain,brand,env,app_suffix,aws_profile,registry_url,tag)
    ...
    if deploy_biz_services(logger,app_name,stage,app_domain,brand,env,app_suffix,aws_profile) :
        if deploy_app_services(logger,app_domain,brand,env,aws_profile) :
            if deploy_service_module(logger,"ac-core",False,app_name,stage,app_domain,brand,env,app_suffix,aws_profile,registry_url,tag):
                if deploy_service_module(logger,"ac-datastore",False,app_name,api_stage,app_domain,brand,env,app_suffix,aws_profile,registry_url,tag):
...
def list_tags(logger, profile, stack):
    shell(None,"aws ecr describe-images --output yaml --repository-name " + stack + " --profile " + profile
