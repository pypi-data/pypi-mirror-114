...
def _validate_inputs(
    ...
    if is_bulk:
        if prediction_labels is not None:
            _validate_bulk_prediction_inputs(
                prediction_ids,
                prediction_labels,
                features,
                features_name_overwrite,
        ...
        if prediction_labels is not None:
            _validate_prediction_inputs(
                prediction_ids, prediction_labels, features, timestamp_overwrite
            )
        else:
...
def _validate_bulk_prediction_inputs(
    prediction_ids,
    prediction_labels,
    features,
    features_name_overwrite,
    ...
    if features is not None and features.shape[0] != prediction_ids.shape[0]:
        ...
        raise ValueError(msg)
    if (
        features_name_overwrite is not None
        and features is not None
        and len(features.columns) != len(features_name_overwrite)
    ):
    ...
    if (
        ...
        and isinstance(features.columns, pd.core.indexes.range.RangeIndex)
    ...
    if features is not None and features.columns is not None:
        for feature in features.columns:
            if not isinstance(feature, str):
                msg = (
                    f"features.column {feature} is type {type(feature)}, but expect str"
                )
                raise TypeError(msg)
...
def _validate_prediction_inputs(
    ...
    if not isinstance(_convert_element(prediction_labels), (str, bool, float, int)):
    ...
    if features is not None and bool(features):
        for k, v in features.items():
            if not isinstance(_convert_element(v), (str, bool, float, int)):
                ...
                raise TypeError(msg)
    if timestamp_overwrite is not None and not isinstance(timestamp_overwrite, int):
