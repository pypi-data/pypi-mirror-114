...
from yqn_project_pro.utils.sha512_aes_256 import decrypt_pbe_with_hmac_sha512_aes_256
...
class ApolloClient:
    ...
    def get_value(
        ...
        try:
            ...
            # return default_val
            if namespace in self._cache:
                val = self._cache[namespace].get(key, default_val)
            else:
                val = default_val
            spilt_val = re.match("ENC\((.*?)\)", val.strip())
            if self.apollo_secret and spilt_val:
                val = decrypt_pbe_with_hmac_sha512_aes_256(spilt_val.group(1), self.apollo_secret)
            return val
