...
from .__tool__ import get_value_from_dict_by_multi_name
...
class QAStage(Stage):
    ...
    def __init__(self, data):
        ...
        self.disable_refuse_question = get_value_from_dict_by_multi_name(d=data, names=__DISABLE_REFUSE_LABEL__, default=False)
...
class LibSwitchStage(Stage):
    ...
    def find_new_stages(self, kwargs):
        ...
        for filter_tuple in self.stages_filter:
            ...
            #
            assert not isinstance(label, list) or isinstance(label, type(text)), "切換路線之條件與限定結果的類型不一致，必須保持兩者都是`str` 或是兩者都是`list`"
            ...
            for l, t in zip(label, text):
                if self.get_default_var(kwargs, l) != t:
                    pass_token = False
            if pass_token:
                return stages_label
