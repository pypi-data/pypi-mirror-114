...
def retrieve_container(
    path: Path, isel: Union[str, dict, int] = "all", lazy: bool = False
):
    if Path(path).dirname().ext == ".zarr":
        return ZarrContainer.retrieve(path, isel, lazy)
    else:
        return NetCDFContainer.retrieve(path, isel, lazy)
...
class Container:
    def __init__(
        ...
        *,
        ...
        self.background_write = background_write
        self.write = self._bg_write if background_write else self._write
    ...
    def _expand_fields(self, t, fields):
        fields = fields.assign_coords(t=float(t)).expand_dims("t")
    ...
    def connect(self, stream):
        ...
        if self.save == "all":
            self._collector.map(self._concat_fields).sink(self.write)
        else:
            self._collector.map(get_last).sink(self.write)
...
class MemoryContainer(Container):
    ...
    def data(self):
        try:
            return self._concat_fields(self._cached_data).sortby("t")
        except (OSError, AttributeError):
            return


class NetCDFContainer(Container):
    def __init__(
        self,
        path,
        mode="a",
    ...
    def retrieve(path: Path, isel: Union[str, dict, int] = "all", lazy: bool = False):
        ...
        if isel == "last":
            ...
            if lazy:
                data = _safe_open_mfdataset(
                    path / "data*.nc", combine="by_coords"
                ).sortby("t")
            else:
                data = concat(
    ...
    def merge_datafiles(path, override=False):
        ...
        split_data = _safe_open_mfdataset(
            path / "data*.nc", combine="by_coords"
        ).sortby("t")
        split_data.to_netcdf(path / "merged_data.nc")
        merged_data = _safe_open_dataset(
            path / "merged_data.nc",
            chunks=split_data.chunks if split_data.chunks else None,
        )
    ...
    def data(self):
        try:
            self.flush()
            return _safe_open_mfdataset(
                self.path / "data*.nc", combine="by_coords"
            ).sortby("t")
        except (OSError, AttributeError):
            return
    ...
    def merge(self, override=True):
        if self.path:
            return self.merge_datafiles(self.path, override=override)


class ZarrContainer(Container):
    def __init__(
        self,
        path,
        mode="a",
    ...
    def _write(self, concatenated_fields):
        if concatenated_fields is not None:
            concatenated_fields.to_zarr(self.path, append_dim="t", mode="a")
