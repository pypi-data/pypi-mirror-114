...
from sympy import Symbol, lambdify, cse
...
def np_Min(args):
    min_ = np.fmin.reduce(np.broadcast_arrays(*args))
    return min_


def np_Max(args):
    max_ = np.fmax.reduce(np.broadcast_arrays(*args))
    return max_


def np_Heaviside(a):
    return np.where((a < 0) | np.isnan(a), 0, 1)


class CseLambdify:
    def __init__(self, args, expr, optimizations=None, order="none", modules="numpy"):
        self.args = args
        self.expr = expr
        try:
            subs, reduced = cse(expr, order=order, optimizations=optimizations)
        except AttributeError:
            subs = {}
            reduced = self.expr

        self.subs = subs
        self.reduced_expr = reduced[0]

        self.lambda_subs = {}
        for key, sub in self.subs:
            self.lambda_subs[str(key)] = lambdify(
                [*self.args, *self.lambda_subs.keys()], sub, modules=modules
            )
        self.lambda_expr = lambdify(
            [*self.args, *self.lambda_subs.keys()], self.reduced_expr, modules=modules
        )

    def __call__(self, *args):
        evaluated = dict(zip(map(str, self.args), args))
        for key, func in self.lambda_subs.items():
            evaluated[key] = func(**evaluated)
        evaluated_expr = self.lambda_expr(**evaluated)
...
class NumpyBackend(Backend):
    name = "numpy"
    cse = attr.ib(type=bool, default=True)
    optimizations = attr.ib(type=str, default="basic")

    def __attrs_post_init__(self):
        customs = {key: func.numpy_eval for key, func in self.custom_functions.items()}
        modules = [
        ...
        if self.cse:
            self.lambdify = partial(
                CseLambdify, optimizations=self.optimizations, modules=modules
            )
        else:
            self.lambdify = partial(lambdify, modules=modules)
    ...
    def _build_evolution_equations(self):
        self.evolution_equations = [
            self.lambdify(self.inputs, expr.n())
            for expr in self.system.piecewise_system
        ]
    ...
    def _build_jacobian(self):
        ...
        self.jacobian_routines = [
            [self.lambdify(self.inputs, diff) for diff in jac_row]
