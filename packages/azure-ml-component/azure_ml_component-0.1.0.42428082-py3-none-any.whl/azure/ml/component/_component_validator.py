...
class ComponentValidator(object):
    ...
    def _validate_local_run_component_type(component, process_error):
        ...
        elif component._definition.type == ComponentType.PipelineComponent:
            if component._is_registered:
                from azure.ml.component import Component
                pipeline_component_graph = component._definition.get_registered_pipeline_graph()
                pipeline_nodes = [Component.load(workspace=component.workspace, id=node.module_id)() for node in
                                  pipeline_component_graph.module_nodes + pipeline_component_graph.sub_graph_nodes]
            else:
                pipeline_nodes = component._definition._components.values()
            for node in pipeline_nodes:
                ComponentValidator._validate_local_run_component_type(node, process_error)
            return
        else:
