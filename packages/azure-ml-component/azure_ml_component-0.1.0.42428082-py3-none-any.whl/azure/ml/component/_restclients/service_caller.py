...
class DesignerServiceCaller(WorkspaceTelemetryMixin, RequestTelemetryMixin):
    ...
    @track(_get_logger)
    def get_pipeline_run_status(self, pipeline_run_id, experiment_name=None, experiment_id=None, raw=False):
    ...
    def get_all_layer_child_runs(self, root_run_id, raw=False):
        ...
        custome_headers = self._get_custom_headers()
        custome_headers.update({'x-ms-jsonformat-enum-to-string': 'true'})
        result = self._caller.pipeline_runs_v2.get_all_layer_child_runs(
            run_id=root_run_id,
            subscription_id=self._subscription_id,
            resource_group_name=self._resource_group_name,
            workspace_name=self._workspace_name,
            custom_headers=custome_headers,
            raw=raw
        )
    ...
    @track(_get_logger)
    def _get_pipeline_component_graph(self, graph_id, fetch_nested_graphs=False, skip_dataset_load=True, raw=False):
