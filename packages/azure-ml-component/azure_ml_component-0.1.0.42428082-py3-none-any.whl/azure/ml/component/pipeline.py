...
from ._util._utils import _get_short_path_name, _is_prod_workspace, trans_to_valid_file_name, \
    ...
    find_output_owner_in_current_pipeline
...
class Pipeline(Component, Visible, WorkspaceTelemetryMixin):
    ...
    def submit(self, experiment_name=None, default_compute_target=None, description=None, pipeline_parameters=None,
        ...
        if not skip_validation:
            ...
            self._validate(
                pipeline_steps=context.step_nodes, raise_error=True, pipeline_parameters=pipeline_parameters)
    ...
    @timer()
    def _validate(self, pipeline_steps, raise_error=False, is_local=False, pipeline_parameters=None):
        errors = []
        ...
        if enabled_subpipeline_registration():
            ...
            errors += self._validate_pipeline_component(is_local=is_local, pipeline_parameters=pipeline_parameters)
        else:
    ...
    def _validate_pipeline_component(self, is_local, pipeline_parameters=None):
        ...
        def validate_datastore(pipeline: Pipeline, default_datastore):
            # Since pipeline output and node output may share the same output, the validation happens on node level
            # to avoid duplicate error.
            for node in pipeline.nodes:
                if isinstance(node, Pipeline):
                    validate_datastore(node, pipeline._resolve_default_datastore())
                else:
                    ComponentValidator._validate_datastore(
                        ...
                        process_error=functools.partial(node.process_error, raise_error=False, errors=errors),
        ...
        validation_info = self._validate_component(
            ...
            skip_datastore_validation=True
        )
        # 3. validate datastore
        validate_datastore(self, self._resolve_default_datastore())
    ...
    def _run(self, experiment_name=None, working_dir=None, mode=RunMode.Docker.value, track_run_history=True,
        ...
        if not skip_validation:
            self._validate(visualization_context.step_nodes, raise_error=True, is_local=True)
