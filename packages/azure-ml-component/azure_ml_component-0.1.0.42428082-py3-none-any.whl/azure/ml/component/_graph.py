...
class _GraphEntityBuilder(object):
    ...
    def build_graph_entity(self, for_registration: bool = False, enable_subpipeline_registration: bool = False,
        ...
        for module in self._modules:
            ...
            self._update_module_node_params(graph_entity, module_node, module, enable_subpipeline_registration)
    ...
    def _produce_module_runsettings(
            self, graph_entity: GraphDraftEntity, module: Component,
            module_node: GraphModuleNode, pipeline_parameters, enable_subpipeline_registration):
    ...
    def _update_module_node_params(
        self, graph_entity: GraphDraftEntity, module_node: GraphModuleNode,
        module: Component, enable_subpipeline_registration
    ):
        """Add module node parameters and update it with context.pipeline_parameters."""
        param_name_matched_dict = _GraphEntityBuilder._get_flattened_pipeline_param_name_matched_dict(
            module, enable_subpipeline_registration)
        ...
        for param_name, param_value in user_provided_params.items():
            ...
            if isinstance(param_value, PipelineParameter):
                ...
                component_param_name = param_value._full_name
                pipeline_param_name = param_name_matched_dict.get(component_param_name, component_param_name)
    ...
    def _resolve_parameter_value_and_type(
            ...
            param_name_matched_dict=None):
        """Return the parameter value, value_type and assignment if there has one."""
        value = parameter_value
        value_type = _GraphEntityBuilder.LITERAL
        assignments = None
        pipeline_parameters = {} if pipeline_parameters is None else pipeline_parameters
        param_name_matched_dict = {} if param_name_matched_dict is None else param_name_matched_dict
        ...
        if isinstance(value, _ParameterAssignment):
            ...
            component_param_name = value._full_name
            pipeline_param_name = param_name_matched_dict.get(component_param_name, component_param_name)
            if pipeline_param_name in pipeline_parameters:
    ...
    @staticmethod
    def _get_flattened_pipeline_param_name_matched_dict(module: Component, enable_subpipeline_registration=False):
        """Get the parameter name matched dict between module and top pipeline."""
        # Calculate the path from root pipeline to module, add component index in definition
        top_pipeline = module
        if module._parent is None:
            return {}
        while top_pipeline._parent is not None:
            top_pipeline = top_pipeline._parent
            # If enable sub pipeline registration, just get the direct parent.
            if enable_subpipeline_registration:
                break
        # If module is not a pipeline, change to its direct parent
        module = module if module._is_pipeline else module._parent
        if top_pipeline == module:
            return {_k: _k for _k in top_pipeline._definition._pipeline_parameters}
        # Get the top pipeline build parameters
        definition_pipeline_parameters = top_pipeline._definition._pipeline_parameters
        component_parameters = _GraphEntityBuilder._parse_and_flatten_pipeline_parameter_for_component(
            module, definition_pipeline_parameters, enable_subpipeline_registration)
        pipeline_parameters = top_pipeline._definition._flatten_pipeline_parameters(definition_pipeline_parameters)
        # Calculate the param matched dict between component and pipeline
        param_name_matched_dict = {
            comp_k: pipeline_k for comp_k, comp_v in component_parameters.items()
            for pipeline_k, pipeline_v in pipeline_parameters.items() if pipeline_v == comp_v}
        return param_name_matched_dict

    @staticmethod
    def _parse_and_flatten_pipeline_parameter_for_component(
            module: Component, pipeline_parameters, enable_subpipeline_registration=False):
        # Calculate the path from root pipeline to module, add component index in definition
        module_parents = []
        top_pipeline = module
        while top_pipeline._parent is not None:
            parent = top_pipeline._parent
            module_parents.append((parent, parent.nodes.index(top_pipeline)))
            top_pipeline = parent
            # If enable sub pipeline registration, just get the direct parent.
            if enable_subpipeline_registration:
                break
        ...
        module = module if module._is_pipeline else module._parent
        if top_pipeline != module:
            # Parse pipeline parameter for module by parents
            while module_parents:
                pipeline, idx = module_parents.pop()
                pipeline_parameters = pipeline._definition._convert_component_kwargs(
                    pipeline_parameters, component_idx=idx, _for_build=False)
        return top_pipeline._definition._flatten_pipeline_parameters(pipeline_parameters)
    ...
    def _resolve_runsetting_parameter_assignment(
        ...
        param_name_matched_dict = _GraphEntityBuilder._get_flattened_pipeline_param_name_matched_dict(
            component, enable_subpipeline_registration)
        is_compute_target = runsetting_def.is_compute_target
        resolved_value, value_type, assignments = \
            _GraphEntityBuilder._resolve_parameter_value_and_type(
                runsetting_value, pipeline_parameters, param_name_matched_dict)
