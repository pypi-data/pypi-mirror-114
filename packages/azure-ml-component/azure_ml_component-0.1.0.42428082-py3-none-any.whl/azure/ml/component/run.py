...
SUBGRAPH_PORTAL_FLIGHT = '&flight=pipelineSubGraph'
...
def _format_run_url_for_subgraph_scenario(url):
    return url.replace('ml.azure', 'master.ml.azure') + SUBGRAPH_PORTAL_FLIGHT
...
class Run(Visible, WorkspaceTelemetryMixin):
    ...
    def __init__(self, experiment, run_id=None):
        ...
        if enabled_subpipeline_registration():
            self._run_details_url = _format_run_url_for_subgraph_scenario(self._run_details_url)
    ...
    def _get_portal_url(self):
        ...
        # append ui flight pipelineSubGraph to have the new sub graph UI experience
        if enabled_subpipeline_registration():
            return _format_run_url_for_subgraph_scenario(self._core_run.get_portal_url())
        else:
