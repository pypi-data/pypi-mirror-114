...
class Component(WorkspaceTelemetryMixin):
    ...
    * `Define a component using component specs <https://aka.ms/azureml-component-specs>`_
    ...
    def process_error(self, e: Exception, error_type, variable_name, variable_type, raise_error=False, errors=None):
        """
    ...
    def _validate_component(
            ...
            inputs=None, parameters=None, skip_datastore_validation=False
    ) -> List:
        """
        ...
        :type parameters: dict
        :param skip_datastore_validation: Datastore might be incorrect when validating definition but overwrite to
                            correct one before submission. So we skip this check when validating definition and
                            validate it before submission.
        :type skip_datastore_validation: bool
        ...
        if for_definition_validate and definition_validator:
            ...
            process_error = functools.partial(self.process_error, raise_error=raise_error, errors=errors)
        ...
        if for_definition_validate:
            # skip datastore validation for definition because it can be updated after the pipeline definition built
        ...
        if not is_local:
            ...
            if not skip_datastore_validation:
