...
def get_dynamic_param_parameter(self: ComponentDefinition):
    ...
    return [KwParameter(name=name, annotation=str(param.type), default=param.default,
                        _type=str(param.type), _optional=param.optional)
...
def _get_single_parameter_docstring_lines(param: KwParameter):
    hint_suffix = ''
    if param._optional:
        hint_suffix += 'optional'
    if param.default:
        hint_suffix += f"{', ' if param._optional else ''}default value is {param.default!r}"
    hint_suffix = f'({hint_suffix})' if hint_suffix else ''
    single_param_docstring = f":param {param.name}: {param.annotation} {hint_suffix}".strip()
    return [single_param_docstring, ":type {}: {}".format(param.name, param._type)]
