...
class PipelineRunOrchestrator:
    ...
    def __init__(self, workspace, input_dataset, pipeline_node_dict, component_execution_dict,
        ...
        self.component_execution_dict[PARENT_NODE_ID] = ComponentExecutionInfo(node_id=PARENT_NODE_ID)
    ...
    def generate_orchestrator_by_pipeline_component(cls, pipeline_component, pipeline_parameters=None,
        ...
        def set_param_value(component_node, module_parameters):
            ...
            for param_name in component_node._definition.parameters:
                parameter_assignment = next(filter(lambda item: item.name == param_name, module_parameters))
                if parameter_assignment.value and parameter_assignment.value_type == '0':
                    params[param_name] = parameter_assignment.value
            component_node.set_inputs(**params)

        def generate_component_by_graph(graph_info):
            """Generate component in pipeline component by pipeine graph info.

            :param graph_info: Pipeline graph info.
            :type graph_info:
            :return: pipeline_node_dict: Dict of node id and component.
                     component_execution_dict: Dict of component id and execution info.
            :rtype: dict[str, azure.ml.component.Component], dict[azure.ml.component.Component, ComponentExecutionInfo]
            """
            from azure.ml.component import Component
            pipeline_node_dict, component_execution_dict = {}, {}
            for node in graph_info.module_nodes + graph_info.sub_graph_nodes:
                # Generate component in pipeline component.
                component_node = Component.load(workspace=pipeline_component.workspace, id=node.module_id)()
                ...
                # Node name is used to create component working dir.
                component_execution_dict[component_node._instance_id] = ComponentExecutionInfo(node_id=node.id,
                                                                                               graph_path=node.name)
                pipeline_node_dict[node.id] = component_node
            return pipeline_node_dict, component_execution_dict

        def update_components_input_by_graph(pipeline_node_dict, graph_info):
            ...
            :type graph_info:
            """
            # Connect the components in pipeline component and get pipeline component edge info.
            edge_info, dataset_nodes = {}, {node.id: node for node in graph_info.dataset_nodes}
            for edge in graph_info.edges:
                if edge.source_output_port.node_id in dataset_nodes:
                    # When dataset in the pipeline component, it will generate dataset by id in the dataset definition.
                    dataset_definition_value = dataset_nodes[edge.source_output_port.node_id].data_set_definition.value
                    if dataset_definition_value.data_set_reference:
                        from azureml.data.file_dataset import FileDataset
                        input_dataset = FileDataset.get_by_id(workspace=pipeline_component.workspace,
                                                              id=dataset_definition_value.data_set_reference.id)
                    elif dataset_definition_value.literal_value:
                        datastore = Datastore.get(pipeline_component.workspace,
                                                  dataset_definition_value.literal_value.data_store_name)
                        input_dataset = DataReference(
                            datastore=datastore,
                            path_on_datastore=dataset_definition_value.literal_value.relative_path)

                    # Add dataset to list of preparing to download.
                    pipeline_input_dataset.append(input_dataset)
                    ...
                    pipeline_node_dict[edge.destination_input_port.node_id].set_inputs(**{
                        edge.destination_input_port.port_name:
                            pipeline_component.inputs[edge.source_output_port.graph_port_name]})
                elif edge.destination_input_port.node_id and edge.source_output_port.node_id:
                    # Record component edge.
                    if edge.source_output_port.node_id not in edge_info:
                        edge_info[edge.source_output_port.node_id] = [edge.destination_input_port.node_id, ]
                    else:
                        edge_info[edge.source_output_port.node_id].append(edge.destination_input_port.node_id)
        ...
        pipeline_input_dataset = [item._dset for item in pipeline_component._input_ports.values()]

        # Generate component in pipeline component by pipeline graph info.
        graph_info = pipeline_component._definition.get_registered_pipeline_graph()
        pipeline_node_dict, component_execution_dict = generate_component_by_graph(graph_info)
        update_components_input_by_graph(pipeline_node_dict, graph_info)
    ...
    def generate_orchestrator_by_pipeline(cls, pipeline, input_dataset, visualization_context=None,
        ...
        pipeline_node_dict = {node._id: node for node in pipeline._expand_pipeline_nodes()}
        # Mapping of component id and component execution information.
        component_execution_dict = {key: ComponentExecutionInfo(node_id=component_to_node_dict[key])
                                    for key in pipeline_node_dict.keys()}
    ...
    def _find_next_execute_node(self, possible_execution_nodes_id, executed_nodes_id):
        ...
        for node_id in possible_execution_nodes_id:
            ...
            if all([input._owner._id in executed_nodes_id for input in node_inputs if isinstance(input, Output)]):
    ...
    def _execute_nodes(self, execution_nodes, pipeline_run_info, execution_file):
        ...
        for node_id, node in execution_nodes.items():
            ...
            graph_path = self.component_execution_dict[node_id].graph_path or node._graph_path
            component_folder = os.path.join(*[trans_to_valid_file_name(val) for val in graph_path.split('.')])
            ...
            if node.type != ComponentType.PipelineComponent.value:
                ...
                run_future = self.component_executor.submit(self._execute_pipeline_component_node, node,
    ...
    def _handle_done_execution_nodes(self, done_execution_futures, execution_future_dict,
        ...
        for future in done_execution_futures:
            ...
            if node_id in self.edge_info:
                # Get out-of-degree nodes of executed node.
