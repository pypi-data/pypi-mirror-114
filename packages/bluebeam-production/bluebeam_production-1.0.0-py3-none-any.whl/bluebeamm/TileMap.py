...
from bluebeamm.objects import Block
...
class TileMap:
    def __init__(self, globals, level, filename="TestMap.tmx"):
        ...
        self.map_dir = os.path.join(self.sourceFileDir, "bluebeamm/tilemap_resources/")
        #print("hey", self.sourceFileDir)
        #self.tiled_map = load_pygame("tilemap_resources/" + filename)
        self.tiled_map = load_pygame(os.path.join(self.map_dir, filename))

        self.width  = self.tiled_map.width
        self.height = self.tiled_map.height

        self.images = {-500:None}
        ...
        for layer in self.tiled_map.layers:
            if isinstance(layer, pytmx.TiledTileLayer):
                for x, y, gid in layer:
                    if (gid):
                        self.tiles[x][y] = gid
                        if (gid not in self.images):
                            self._load_image(gid)

    # Load tile images used in the tilemap
    def _load_image(self, gid):
        print("loading tile w/ gid #",gid)
        ...
        for i in range(xmin, xmax):
            for j in range(ymin, ymax):
                ...
                try:
                    ...
                    print("ERROR: BROKE ON GID: " + str(gid))
