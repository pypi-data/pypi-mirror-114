...
def ReadPDBintoDataFrame(PDBfileNameString):
    DataFrame = pandas.DataFrame(PandasPdb().read_pdb(PDBfileNameString).DataFrame["ATOM"])
    ...
    return DataFrame
def RetreivePDBatomPosition(PDBDataFrame, segmentIDstring, residueNumber, atomNameString):
    DataFrame = PDBDataFrame.loc[PDBDataFrame["segment_id"]== segmentIDstring]
    DataFrame= DataFrame.loc[PDBDataFrame["residue_number"]==residueNumber]
    DataFrame = DataFrame.loc[PDBDataFrame["atom_name"]==atomNameString]
...
def GetHydrogenBondDistances(PDBDataFrame, resstart, resstop):
    ...
    for i in range(segstart, segstop):
        for j in residues2check:
            HN = RetreivePDBatomPosition(PDBDataFrame, segments[i+1],j+1,'HN')
            CO = RetreivePDBatomPosition(PDBDataFrame, segments[i],j,'O')
