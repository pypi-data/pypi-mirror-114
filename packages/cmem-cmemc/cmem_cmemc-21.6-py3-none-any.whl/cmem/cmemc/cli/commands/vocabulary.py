...
def _get_vocabulary_metadata_from_file(file):
    ...
    try:
        ...
        raise ValueError(
            "File {} could not be parsed as turtle.".format(file)
        ) from error
    ...
    if len(ontology_iris) > 1:
        raise ValueError(
            ...
            "in the turtle file: {}".format(
                [str(iri[0]) for iri in ontology_iris]
            )
        )
    metadata["iri"] = str(next(iter(ontology_iris))[0])
    vann_data = graph.query(GET_PREFIX_DECLARATION.format(metadata["iri"]))
    if len(vann_data) == 0:
        raise ValueError(
            "There is no vann namespace defined "
            "for the ontology: {}".format(metadata["iri"])
        )
    if len(vann_data) > 1:
        raise ValueError(
...
def import_command(app, file, replace):
    ...
    if iri in [_["iri"] for _ in graph_api.get_graphs_list()]:
        if replace:
            ...
            raise ValueError(
                "Proposed graph {} does already exist.".format(iri)
            )
    app.echo_info(
        "Import {} as vocabulary to {} ... ".format(file, iri),
        nl=False
    )
    # upload graph
