...
_linode_cli()
{
    ...
    case "${prev}" in
        linode-cli)
            ...
            ;;
        account)
            COMPREPLY=( $(compgen -W "view update cancel update-card invoices-list invoice-view invoice-items logins-list login-view maintenance-list notifications-list clients-list client-create client-view client-update client-delete client-reset-secret payment-methods-list payment-method-view payment-method-make-default payments-list payment-create payment-view paypal-start paypal-execute promo-add settings settings-update enable-managed transfer --help" -- ${cur}) )
            return 0
            ;;
        default)
        ...
        stackscripts)
            ...
            ;;
        lke)
            COMPREPLY=( $(compgen -W "clusters-list cluster-create cluster-view cluster-update cluster-delete pools-list pool-create cluster-nodes-recycle pool-view pool-update pool-delete pool-recycle node-view node-delete node-recycle api-endpoints-list kubeconfig-view versions-list version-view --help" -- ${cur}) )
            return 0
            ;;
        longview)
