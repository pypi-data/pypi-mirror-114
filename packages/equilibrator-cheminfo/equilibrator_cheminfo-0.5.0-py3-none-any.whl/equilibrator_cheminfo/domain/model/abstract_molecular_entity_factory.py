...
class AbstractMolecularEntityFactory(ABC):
    ...
    with open_binary(data, "manual_exceptions.pkl") as handle:
        _manual_exceptions: Dict[str, Optional[MolecularEntity]] = pickle.load(handle)
    # Create further exception for protons.
    _manual_exceptions["GPRLSGONYQIRFK-UHFFFAOYSA-N"] = None
    ...
    @classmethod
    def _create_manual_exception(
        cls, structure: Structure
    ) -> Optional[MolecularEntity]:
        """Create a molecular entity for structures whose predictions are nonsense."""
        return cls._manual_exceptions[structure.identifier.inchikey]
