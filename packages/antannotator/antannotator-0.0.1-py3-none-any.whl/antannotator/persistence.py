...
from typing import Callable, Dict, List, Any, Optional, Sequence, Set, Iterable, NamedTuple
from antannotator.utils import generate_random_filename, distinct_by, JsonSerializableEncoder, JsonSerilaizable
...
class FileEventStorage(EventStorage):
    ...
    def __get_session_file(self):
        if self.__session_file is None:
            session_file = os.path.join(self.directory, generate_random_filename())
...
class FileAnnotationTaskStorage(AnnotationTaskStorage):
    ...
    def _persist_batch(self, samples: List[AnnotationSample]):
        batch_file = os.path.join(self.directory, generate_random_filename())
        os.makedirs(self.directory, exist_ok=True)
