...
class Vehicle(object):
    ...
    def __init__(self, username, password):
        ...
        if (self.accessTokenExpiresAt):
            if isinstance(self.accessTokenExpiresAt, float):
                self.accessTokenExpiresAt = time.gmtime(accessTokenExpiresAt)
    ...
    def auth(self):
        ...
        if response.status_code == 200:
            ...
            self.accessTokenExpiresAt = time.gmtime((time.time() - 100) + result["AuthenticationResult"]["ExpiresIn"])
            ...
            result["expiry_date"] = time.gmtime((time.time() - 100) + result["AuthenticationResult"]["ExpiresIn"])
    ...
    def __acquireToken(self):
        ...
        if self.accessTokenExpiresAt:
            if time.time() >= calendar.timegm(self.accessTokenExpiresAt):
    ...
    def writeToken(self, token):
        ...
        lock = filelock.FileLock(TOKEN_FILE_LOCK_LOCATION)
