...
class Vehicle(object):
    ...
    def auth(self):
        ...
        if response.status_code == 200:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                executor.submit(self.writeToken, result)
            return True
        else:
    ...
    def __acquireToken(self):
        ...
        if os.path.isfile(self.token_location):
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(self.readToken)
                data = future.result()
        else:
            data = dict()
    ...
    def writeToken(self, token):
        ...
        threadId = concurrent.futures.getName()
        with open(self.token_location, "w") as outfile:
            _LOGGER.debug(f"Thread {threadId} has acquired lock to WRITE Token.")
            json.dump(token, outfile)
            _LOGGER.debug(f"Thread {threadId} has finished Writing token and should exit.")
    ...
    def readToken(self):
        ...
        with open(self.token_location) as token_file:
            _LOGGER.debug(f"Thread {threadId} has acquired lock to READ Token.")
