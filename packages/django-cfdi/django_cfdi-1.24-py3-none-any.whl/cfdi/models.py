...
class DescargaCfdi(models.Model):
    ...
    paquetes = ArrayField(models.CharField(max_length=255), blank=True, null=True)
    ...
    def solicitar_descarga(self):
        ...
        assert self.path_fiel_cer
        assert self.path_fiel_key
    ...
    def solicitar_status_descarga(self):
        ...
        """
            SI EL ESTADO QUE REGRESÓ LA SOLICITUD ESTÁ ENTRE 1 Y 2,
        ...
        if status in [1,2]:
            ...
            self.status = 3
            self.respuesta_consulta = verificacion
            ...
            for paq in self.consulta_object["paquetes"]:
                ...
                if descarga.get("cod_estatus") == "5000":
                    ...
                    filepath = default_storage.save(
                        f'{self.path_paquetes}/{paq}.zip', 
                        ContentFile(
                            base64.b64decode(descarga['paquete_b64'])
                        )
                    )
                    self.paquetes.append(filepath)
                else:
