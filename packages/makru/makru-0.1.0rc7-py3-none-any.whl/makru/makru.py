...
def file_exists(path: Union[str, Path], go_panic=False, msg="could not found {it}."):
    """
...
class Sandbox(object):
    def __init__(self, confpath, **globalconf):
        ...
        self.config_name = Path(confpath)
        ...
        self._info_store: Store = None # FIXME: use Optional
        self._build_store: Store = None
    
    @deprecated(
        deprecated_in="0.1.0-rc.6",
        removed_in="0.2.0-rc.0",
        current_version=__version__,
        details="Use str(Sandbox.config_name) instead.")
    def confpath(self) -> str:
        return str(self.config_name)
    confpath = property(confpath)
    
    @property
    def root(self) -> str:
        return str(self.config_name.parent)
    
    @property
    def root_name(self) -> Path:
        return self.config_name.parent
    ...
    @staticmethod
    def read_config(confpath: Union[str, Path]):
        return yaml_load(Path(confpath).read_text())
    ...
    @deprecated(
        ...
        removed_in="0.2.0-rc.0",
        current_version=__version__,
        details="Use .read_config instead.",
    )
    def prepare_config(cls, confpath):
        return cls.read_config(confpath)
    ...
    def run_action(self, name, arg=None):
        ...
        if target_action_s:
            ...
            return self.run_action_function(
                actionf, self.build_desc(action_default_arg=default_arg)
            )
        elif name in self.gconfig["bulitin_actions"]:
    ...
    @deprecated(
        ...
        removed_in="0.2.0-rc.0",
        current_version=__version__,
        details="Use .expand_path_name instead.",
    )
    def expand_path(self, s):
        """
        Expand the path `s` as if the `self.root` is the current working directory.
        """
        return str(self.expand_path_name(s))
    
    def expand_path_name(self, s: Union[str, Path]) -> Path:
        """
        Expand the path `s` as if the `self.root` is the current working directory.
        """
        if isinstance(s, Path):
            if s.is_relative_to('.'):
                return Path(self.root_name, *s.parts)
            else:
                return s
        else:
            return self.expand_path_name(Path(s))
...
class Makru(object):
    def __init__(self, arguments=None):
        ...
        if not root_config_path:
            root_config_path = Path("./makru.yaml")
        self.globalconfig = {
            "plugin_paths": [],
            "root_config_path": str(root_config_path),
            "root_config_name": root_config_path,
            "bulitin_actions": {
    ...
    @classmethod
    def search_config_path(cls, arguments: List[str]) -> Optional[Path]:
    ...
    def list_actions(self, desc: CompilingDesc):
    ...
    @deprecated(
        ...
        removed_in="0.2.0-rc.0",
        current_version=__version__,
        details="Use .correct_config_name instead.",
    )
    def correct_config_path(cls, path: Union[str, Path]) -> str:
        return str(cls.correct_config_name(path))
    
    @staticmethod
    def correct_config_name(name: Union[str, Path]) -> Path:
        path = Path(name)
        if path.is_dir():
            return path / "makru.yaml"
        else:
            return path
