...
@LastEditTime: 2021-07-26 15:02:56
            ...
            if len(user_asset_list) > 0:
                for user_asset in user_asset_list:
                    try:
                        ...
                        logger_error.error(f"用户资产负数预警队列{mod_value}异常,json串:{user_asset.decode()},ex:{ex.decode()}")
                        continue
            else:
                time.sleep(60)
            ...
            if len(asset_inventory_list) > 0:
                for asset_inventory in asset_inventory_list:
                    try:
                        ...
                        logger_error.error(f"资产每日进销存预警队列{mod_value}异常,json串:{asset_inventory.decode()},ex:{ex.decode()}")
                        continue
            else:
                time.sleep(60*60)
            ...
            if len(stat_queue_list) > 0:
                ...
                for stat_queue in stat_queue_list:
                    try:
                        ...
                        if not stat_orm:
                            stat_orm = stat_orm_model.get_entity("act_id=%s and module_id=%s and key_name=%s", params=[stat_queue.act_id,stat_queue.module_id, stat_queue.key_name])
                            if stat_orm:
                                redis_init.set(redis_stat_orm_key, SevenHelper.json_dumps(stat_orm), ex=config.get_value("model_expire", 60 * 1))
                        if not stat_orm:
                        ...
                        logger_error.error(f"统计队列{mod_value}异常,json串:{stat_queue.decode()},ex:{ex.decode()}")
                        db_transaction.rollback_transaction()
                        stat_queue.process_count +=1
                        stat_queue.process_result = f"出现异常,json串:{stat_queue.decode()},ex:{ex.decode()}"
                        minute = 1 if stat_queue.process_count<=5 else 5
                        stat_queue.process_date = TimeHelper.add_minutes_by_format_time(minute=minute)
                        stat_queue_model.update_entity(stat_queue, "process_count,process_result,process_date")
                        continue
