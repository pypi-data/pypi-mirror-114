...
@LastEditTime: 2021-07-28 14:55:59
...
class FrameBaseHandler(BaseApiHandler):
    ...
    def get_app_key_secret(self):
        ...
        if request_app_key not in (0, app_key):
            redis_key = f"middler_product:requestappkey_{request_app_key}"
            redis_init = SevenHelper.redis_init()
            ...
            if not middler_product_dict:
                ...
                if middler_product_dict:
                    redis_init.set(redis_key, SevenHelper.json_dumps(middler_product_dict), ex=config.get_value("cache_expire", 60 * 10))
            else:
