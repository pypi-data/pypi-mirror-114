...
@LastEditTime: 2021-07-28 15:38:03
...
class UserBaseModel():
    ...
    def _get_user_info_cache_key(self, act_id, id, open_id=""):
        """
    ...
    def _delete_user_info_cache(self,user_info_dict):
        """
        :description: 添加用户信息缓存
        :param user_info_dict：用户信息字典
        :return: 
        :last_editors: HuangJianYi
        """
        redis_key = self._get_user_info_cache_key(user_info_dict["act_id"],user_info_dict["id"])
        redis_init = SevenHelper.redis_init()
        redis_init.delete(redis_key)

    def get_user_info_dict(self,app_id, act_id, user_id,open_id=""):
        """
    ...
    def get_user_info_dict_cache(self,app_id, act_id, user_id,open_id=""):
        """
        :description: 获取用户信息单条记录
        :param app_id：应用标识
        :param act_id：活动标识
        ...
        """
        id = self._get_user_info_id(act_id,user_id)
        redis_key = self._get_user_info_cache_key(act_id,id,open_id)
        redis_init = SevenHelper.redis_init()
        ...
        if user_info_dict:
            ...
            if user_info_dict:
                redis_init.set(redis_key, SevenHelper.json_dumps(user_info_dict), ex=config.get_value("cache_expire", 60 * 1))
        return user_info_dict
    ...
    def save_user_by_openid(self, app_id, act_id, open_id, user_nick, avatar, union_id=""):
        ...
        try:
            ...
            if not user_info:
                ...
                self._delete_user_info_cache(user_info.__dict__)
    ...
    def update_user_info(self, app_id, act_id, user_id, open_id, user_nick, avatar):
        ...
        try:
            ...
            if not user_info_dict:
                ...
                self._delete_user_info_cache(user_info_dict)
    ...
    def get_black_info_dict(self, app_id, act_id, user_id):
        ...
        if not user_black_dict:
            ...
            if not user_black_dict:
                return user_black_dict
            redis_init.set(redis_key, SevenHelper.json_dumps(user_black_dict), ex=config.get_value("cache_expire", 60 * 1))
        else:
