...
@LastEditTime: 2021-07-26 14:39:45
...
from seven_cloudapp_frame.models.taobao_model import *
...
from seven_cloudapp_frame.models.db_models.role.role_power_model import *
from seven_cloudapp_frame.models.db_models.role.role_user_model import *
...
class FrameBaseHandler(BaseApiHandler):
    ...
    def check_continue_request(self, handler_name, app_id, object_id, expire=1):
        """
        :description: 一个用户同一handler频繁请求校验
        :param handler_name: handler名称
        :param app_id: 应用标识
        :param object_id: object_id
        :param expire: 间隔时间，单位秒
        ...
        if object_id and handler_name and app_id:
            if SevenHelper.is_continue_request(f"continue_request:{handler_name}_{app_id}_{object_id}", expire):
                result = True, f"频繁请求,请{expire}秒后再试"
        return result
    ...
    def add_request_user(self, app_id, object_id):
        ...
        redis_key = f"request_user_list:{app_id}_{str(SevenHelper.get_now_int(fmt='%Y%m%d%H%M'))}"
        redis_init.sadd(redis_key, object_id)
        redis_init.expire(redis_key, 3600)
    ...
    def check_request_user(self, app_id, current_limit_count, current_limit_minute=1):
        ...
        if current_limit_minute == 1:
            ...
            for i in  current_limit_minute:
                ...
                key_list.append(key)
            redis_key = f"request_user_list:{app_id}"
            redis_init.sinterstore(redis_key, key_list)
            redis_init.expire(redis_key, 3600)

        if int(redis_init.scard(redis_key)) >= current_limit_count:
            return True
        else:
            return False
    ...
    def get_app_info(self,app_id):
        ...
        :last_editors: HuangJianYi
        """
        redis_key = f"app_info:{app_id}"
        redis_init = SevenHelper.redis_init()
        app_info = redis_init.get(redis_key)
        if not app_info:
            app_info = AppInfoModel(context=self).get_dict("app_id=%s", params=[app_id])
            if app_info:
                redis_init.set(redis_key, SevenHelper.json_dumps(app_info), ex=config.get_value("model_expire", 60 * 1))
        if isinstance(app_info,AppInfo) == False:
            app_info = json.loads(app_info)
        return app_info

    def get_power_menu_list(self, app_id):
        """
        :description: (高级功能)获取权限菜单列表
        :param app_id：应用标识
        :return 返回权限列表
        :last_editors: HuangJianYi
        """
        ...
        redis_key = f"power_menu_list:{app_id}"
        redis_init = SevenHelper.redis_init()
        data = redis_init.get(redis_key)
        if data:
        ...
        if len(role_id_list) <= 0:
            return data
        data = role_power_model.get_dict_list(SevenHelper.get_condition_by_int_list("role_id", role_id_list))
        if len(data) > 0:
            ...
            data = list(set(data))
            redis_init.set(redis_key, SevenHelper.json_dumps(data), ex = config.get_value("model_expire", 60 * 1))
        return data
