...
@LastEditTime: 2021-07-28 18:00:35
...
class ActBaseModel():
    ...
    def get_act_info_dict_cache(self,act_id):
        ...
        :last_editors: HuangJianYi
        """
        cache_key = self._get_act_info_cache_key(act_id)
        redis_init = SevenHelper.redis_init()
        act_info_dict = redis_init.get(cache_key)
        if act_info_dict:
            act_info_dict = json.loads(act_info_dict)
        else:
            act_info_dict = self.get_act_info_dict(act_id)
            if act_info_dict:
                redis_init.set(cache_key, SevenHelper.json_dumps(act_info_dict), ex=config.get_value("cache_expire", 60 * 1))
        return act_info_dict
