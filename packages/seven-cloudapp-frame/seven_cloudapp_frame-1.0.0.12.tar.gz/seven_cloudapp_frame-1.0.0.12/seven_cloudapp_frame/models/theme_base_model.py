...
class ThemeBaseModel():
    ...
    def get_theme_info_cache(self,theme_id, ver_no):
        ...
        if theme_info_dict:
            ...
            if theme_info_dict:
                redis_init.set(cache_key, SevenHelper.json_dumps(theme_info_dict), ex=config.get_value("cache_expire", 60 * 1))
        return theme_info_dict
    ...
    def get_theme_list_cache(self,app_id):
        ...
        :last_editors: HuangJianYi
        """
        cache_key = f"theme_list:appid_{app_id}"
        redis_init = SevenHelper.redis_init()
        theme_list = redis_init.get(cache_key)
        if theme_list:
            theme_list = json.loads(theme_list)
        else:
            theme_list = self.get_theme_list(app_id)
            if theme_list:
                redis_init.set(cache_key, SevenHelper.json_dumps(theme_list), ex=config.get_value("cache_expire", 60 * 1))
        return theme_list
    ...
    def get_skin_list_cache(self, theme_id=0, theme_out_id=""):
        ...
        :last_editors: HuangJianYi
        """
        cache_key = f"skin_list:themeid_{theme_id}_themeoutid_{theme_out_id}"
        redis_init = SevenHelper.redis_init()
        theme_list = redis_init.get(cache_key)
        if theme_list:
            theme_list = json.loads(theme_list)
