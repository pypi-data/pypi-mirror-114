...
@LastEditTime: 2021-07-23 12:22:42
...
class SevenHelper:
    ...
    @classmethod
    def is_continue_request(self, redis_key, expire=2):
        """
        :description: 请求太频繁校验
        :param redis_key：自定义redis_key
        :param expire：过期时间，单位秒
        :return: bool true-代表跳过限制，false-代表连续请求进行限制
        :last_editors: HuangJianYi
        """
        redis_init = self.redis_init()
        post_value = redis_init.get(redis_key)
        if post_value == None:
            redis_init.set(redis_key, 10, ex=expire)
            return False
        return True
