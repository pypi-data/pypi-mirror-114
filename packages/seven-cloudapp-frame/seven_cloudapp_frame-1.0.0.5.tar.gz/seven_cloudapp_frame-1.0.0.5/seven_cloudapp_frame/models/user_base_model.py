...
@LastEditTime: 2021-07-26 18:43:43
...
class UserBaseModel():
    ...
    def get_user_info(self,app_id, act_id, user_id):
        """
        ...
        user_info = user_info_model.get_dict_by_id(id)
        if user_info and user_info["app_id"]!=app_id:
            user_info = None
        return user_info
    ...
    def _get_or_add_user_by_openid_before(self, app_id, act_id, open_id, user_nick, avatar, union_id=""):
        """
        :description: 获取或更新用户信息（主要用于登录）执行前事件
        :param app_id：应用标识
        :param act_id：活动标识
        :param open_id：open_id
        :param user_nick：昵称
    ...
    def _get_or_add_user_by_openid_after(self,user_info):
        """
        :description: 获取或更新用户信息（主要用于登录）执行后事件，主要用于统计上报
    ...
    def get_or_add_user_by_openid(self, app_id, act_id, open_id, user_nick, avatar, union_id=""):
        ...
        invoke_result_data = self._get_or_add_user_by_openid_before(app_id, act_id, open_id, user_nick, avatar, union_id)
        ...
        self._get_or_add_user_by_openid_after(user_info)
    ...
    def apply_black_unbind(self,app_id,act_id,user_id,reason="误封号,申请解封"):
        ...
        """
        invoke_result_data = InvokeResultData()
        user_info_dict = self.get_user_info(act_id,user_id)
    ...
    def get_black_info(self, app_id, act_id, user_id):
        """
        ...
        if not user_id:
            return user_black
        user_info_dict = self.get_user_info(app_id, act_id, user_id)
        if not user_info_dict:
            return user_black
        condition = "act_id=%s and user_id=%s"
        params = [act_id, user_id]
        user_black = UserBlackModel(context=self.context).get_dict(condition, params=params)
        return user_black
