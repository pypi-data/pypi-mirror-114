...
@LastEditTime: 2021-07-27 16:56:10
...
class FrameBaseHandler(BaseApiHandler):
    ...
    def get_app_key_secret(self):
        ...
        if request_app_key not in (0, app_key):
            redis_key = f"middler_product:{request_app_key}"
            redis_init = SevenHelper.redis_init()
            middler_product_dict = redis_init.get(redis_key)
            if not middler_product_dict:
                ...
                if not middler_product_dict:
                    return app_key, app_secret
                redis_init.set(redis_key, SevenHelper.json_dumps(middler_product_dict), ex=config.get_value("model_expire", 60 * 1))
            else:
    ...
    def get_app_info_dict(self,app_id):
        ...
        if not app_info_dict:
            app_info_dict = AppInfoModel(context=self).get_dict("app_id=%s", params=[app_id])
            if app_info_dict:
                redis_init.set(redis_key, SevenHelper.json_dumps(app_info_dict), ex=config.get_value("model_expire", 60 * 1))
        if app_info_dict and isinstance(app_info_dict, dict) == False:
