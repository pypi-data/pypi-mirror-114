...
@LastEditTime: 2021-07-28 18:01:49
...
class AppBaseModel():
    ...
    def get_app_info_dict_cache(self,app_id):
        ...
        :last_editors: HuangJianYi
        """
        cache_key = self._get_app_info_cache_key(app_id)
        redis_init = SevenHelper.redis_init()
        app_info_dict = redis_init.get(cache_key)
        if app_info_dict:
            app_info_dict = json.loads(app_info_dict)
        else:
            app_info_dict = self.get_app_info_dict(app_id)
            if app_info_dict:
                redis_init.set(cache_key, SevenHelper.json_dumps(app_info_dict), ex=config.get_value("cache_expire", 60 * 1))
