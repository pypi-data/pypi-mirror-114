...
@LastEditTime: 2021-07-28 18:02:49
...
class UserBaseModel():
    ...
    def _delete_user_info_cache(self,user_info_dict):
        ...
        :last_editors: HuangJianYi
        """
        cache_key = self._get_user_info_cache_key(user_info_dict["act_id"],user_info_dict["id"])
    ...
    def get_user_info_dict_cache(self,app_id, act_id, user_id,open_id=""):
        ...
        """
        id = self._get_user_info_id(act_id,user_id)
        cache_key = self._get_user_info_cache_key(act_id,id,open_id)
        redis_init = SevenHelper.redis_init()
        user_info_dict = redis_init.get(cache_key)
        if user_info_dict:
            user_info_dict = json.loads(user_info_dict)
        else:
            user_info_dict = self.get_user_info_dict(app_id,act_id, user_id,open_id)
            if user_info_dict:
                redis_init.set(cache_key, SevenHelper.json_dumps(user_info_dict), ex=config.get_value("cache_expire", 60 * 1))
        return user_info_dict
    ...
    def get_black_info_dict(self, app_id, act_id, user_id):
        ...
        if not user_info_dict:
            return user_black_dict
        cache_key = f"user_black:{act_id}_{user_id}"
        redis_init = SevenHelper.redis_init()
        user_black_dict = redis_init.get(cache_key)
        if not user_black_dict:
            ...
            if not user_black_dict:
                return user_black_dict
            redis_init.set(cache_key, SevenHelper.json_dumps(user_black_dict), ex=config.get_value("cache_expire", 60 * 1))
        else:
