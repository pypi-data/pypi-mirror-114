...
@LastEditTime: 2021-07-26 16:28:29
...
class TopBaseModel():
    ...
    def instantiate_new(self, is_log=False):
        """
        :description: 实例化
        ...
        if not base_info:
            return self.reponse_json_error("error", "基础信息未配置")
        ...
        if ":" in user_nick:
            return self.reponse_json_error("account_error", "对不起，初次创建活动包含实例化，请使用主账号进行创建。")
                ...
                for content in content_list:
                    if "submsg=" in content:
                        if "名称已经存在" in content:
                            return self.reponse_json_error("create_error", content[len("submsg="):], {"icon": icon, "app_name": app_name})
                        if "应用名称不合法" in content:
                            return self.reponse_json_error("create_error", content[len("submsg="):], {"icon": icon, "app_name": app_name})
                        return self.reponse_json_error("create_error", content[len("submsg="):], {"icon": icon, "app_name": app_name})

    def app_update(self, app_id, client_template_id, client_ver, access_token, app_info, is_log=False):
        """
        :description: app更新
        :param app_id:app_id
        ...
        try:
            ...
            if resp and ("miniapp_template_updateapp_response" in resp.keys()):
                ...
                if "miniapp_template_onlineapp_response" in online_app_info.keys():
                    ...
                    app_info.modify_date = self.get_now_datetime()
                    AppInfoModel(context=self).update_entity(app_info, "app_ver,template_ver,modify_date")
