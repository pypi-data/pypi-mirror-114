...
class LaunchBaseModel():
    ...
    def add_launch_goods(self, app_id, act_id, goods_id, old_goods_id,source_types):
        ...
        if goods_id != old_goods_id:
            ...
            for item in source_types:
                if int(item) == 1:
                    act_prize_total = ActPrizeModel(context=self).get_total("act_id=%s and goods_id=%s", params=[act_id, old_goods_id])
                    if act_prize_total > 0 :
                        is_update = False
                elif int(item) == 2:
                    price_gear_total = PriceGearModel(context=self).get_total("act_id=%s and goods_id=%s", params=[act_id, old_goods_id])
    ...
    def update_launch_goods(self, act_id, goods_id, source_types):
        ...
        for item in source_types:
            if int(item) == 1:
                act_prize_total = ActPrizeModel(context=self).get_total("act_id=%s and goods_id=%s", params=[act_id, goods_id])
                if act_prize_total > 0 :
                    is_update = False
            elif int(item) == 2:
                price_gear_total = PriceGearModel(context=self).get_total("act_id=%s and goods_id=%s", params=[act_id, goods_id])
                if price_gear_total > 0 :
                    is_update = False
        if is_update == True:
