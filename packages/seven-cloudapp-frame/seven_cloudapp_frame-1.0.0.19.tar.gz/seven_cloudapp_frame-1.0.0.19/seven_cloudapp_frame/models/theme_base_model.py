...
@LastEditTime: 2021-07-30 10:15:38
...
class ThemeBaseModel():
    ...
    def get_skin_list(self, theme_id=0, theme_out_id="",is_cache=True):
        ...
        if is_cache:
            dict_list = SkinInfoModel(context=self).get_cache_dict_list(where=where, params=params)
        else:
            dict_list = SkinInfoModel(context=self).get_dict_list(where=where, params=params)
        for dict_info in dict_list:
            dict_info["client_json"] = self.json_loads(dict_info["client_json"]) if dict_info["client_json"] else {}
            dict_info["server_json"] = self.json_loads(dict_info["server_json"]) if dict_info["server_json"] else {}
        return dict_list
