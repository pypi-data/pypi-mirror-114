...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--spread",
            action="store_true",
            help="Schedule into random slots, not in contiguous ones",
        )
        parser.add_argument(
            "--yes",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if options["talk_type"]:
            talks = talks.filter(talk_type__name__in=options["talk_type"])
        ...
        if options["venue"]:
            venues = venues.filter(name__in=options["venue"])
