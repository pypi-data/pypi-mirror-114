...
For additional light sources, get_local_dirs() can be used with the buffers returned from render_scene() and a light
position.
...
def set_tight_cam(vertices, cam: dict = None, use_bbox: bool = True, visualize: bool = False):
    """automatically set camera parameters to enclose a scene (either all vertices, or their axis aligned bounding box)
     in the view frustum"""
    if use_bbox:
        corners = get_bbox_corners(get_bbox(vertices))
    else:
        corners = vertices

    # set up camera if not provided: lookat: scene center
    scene_center = np.mean(corners, axis=0)
    if cam is None:
    ...
    if not hasattr(cam, 'position'):
        cam.position = scene_center + np.r_[5., 0., -10.]
    ...
    corners_cam = (np.linalg.inv(cam.extrinsics) @ np.r_[corners.T, np.ones((corners.shape[0], 1)).T]).T[:, :3]
    ...
    cam.sensor_width = 2 * np.maximum(np.abs(bbox_projected[0, 0]), bbox_projected[1, 0])
    cam.sensor_height = 2 * np.maximum(np.abs(bbox_projected[0, 1]), bbox_projected[1, 1])

    # adjust sensor dimension with its actual aspect ratio
    if cam.res_x > cam.res_y:
        cam.sensor_width = np.maximum(cam.sensor_width, cam.sensor_height * cam.res_x / cam.res_y)
    else:
        cam.sensor_height = np.maximum(cam.sensor_height, cam.sensor_width * cam.res_y / cam.res_x)
    ...
    if visualize:
        ...
        if use_bbox:
            for ci in range(8):
                plot3(np.r_[corners[ci:ci+1, :], corners_projected_world[ci:ci+1, :], cam.position[None]].T, axes=ax, color=0.85 * np.r_[1, 1, 1])
            scatter3(corners, axes=ax, color=np.r_[0.1, 0.4, 0.85])
            plot3(corners[np.r_[np.r_[:4], 0, np.r_[4:8], 4, 5, 1, 2, 6, 7, 3]].T, axes=ax, color=np.r_[0.1, 0.4, 0.85])
            ...
            scatter3(corners[::123, :], marker='.', axes=ax, color=np.r_[0.1, 0.4, 0.85])
...
def embree_create_scene(meshes,
    ...
    for mesh in meshes:
        ...
        if auto_cam and not auto_cam_bbox:
            all_vertices = mesh.vertices if all_vertices is None else np.concatenate((all_vertices, mesh.vertices), axis=0)
