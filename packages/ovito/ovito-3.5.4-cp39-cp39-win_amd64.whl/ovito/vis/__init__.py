...
def _Viewport_create_widget(self, parent = None):
    ...
    It may be used in a Python script to display a simulation dataset on screen and build graphical user interfaces. 
    The method create an interactive window accepting mouse inputs from the user similar to the viewport windows 
    ...
    **Note:** This method has experimental status and is currently available only in the OVITO package for Anaconda.
    ...
    vpwin_ptr = ovito.plugins.PyScriptGui.OpenGLViewportWindow._create(self, 0 if parent is None else shiboken.getCppPointer(parent))
