...
def checkin(path=''):
    ...
    config_path = None
    config_path_list=["/ql/config/check.json","check.json", "../check.json", "./config/check.json", "../config/check.json"]
    if path=='':
        pass
    else:
        config_path_list=config_path_list.append(path)
    for one_path in config_path_list:
