...
class OptionIVDiffStrategy(Strategy):
    ...
    def processStrategy(self, ticker, nextExpiryCount = 1):
        ...
        strike_low_max_profit = (strike_low_put_price - strike_low_put_nxt_price)*lot_size
        strike_low_max_loss = (strike_low_nxt - strike_low)*lot_size + strike_low_max_profit
        strike_high_max_profit = (strike_high_call_price - strike_high_call_nxt_price)*lot_size
        strike_high_max_loss = (strike_high_nxt - strike_high)*lot_size + strike_high_max_profit
        ...
        data['strategy_action'], data['strategy_max_profit'], data['strategy_max_loss'] = self._getAction(data)
    ...
    def screener(self,nextExpiryCount = 1):
        ...
        if len(pending_tickers) != 0:
            ...
            for ticker in sample_tickers:
                try:
                    data = self.processStrategy(ticker, nextExpiryCount)
                    result.append(data)
                except:
                    pass
    ...
    def _getAction(self, x, threshold = 10):
