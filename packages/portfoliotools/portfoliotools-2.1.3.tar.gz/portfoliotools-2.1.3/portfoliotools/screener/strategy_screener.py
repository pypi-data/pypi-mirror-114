...
# Option Strategies
class OptionIVDiffStrategy:
    
    def sortStrikes(self, up, x):
        return abs(up-x)

    def getClosestStrikes(self, strikes, value, count = 3):
        strikes.sort(key = lambda x: self.sortStrikes(value, x))
        filtered_strikes = strikes[:count]
        return filtered_strikes

    def getAction(self, x, threshold = 10):
        action = ''
        if abs(x['iv_diff']) >= threshold:
            if x['iv_diff'] > 0:
                action = 'Sell ' + x['ticker'] + ' PUT@' + str(x['strike_low']) + ' for ' + str(x['strike_low_pe_price'])
                action +=  ' and Buy ' + x['ticker'] + ' PUT@' + str(x['strike_low_nxt']) + ' for ' + str(x['strike_low_pe_nxt_price'])
            else:
                action = 'Sell ' + x['ticker'] + ' CALL@' + str(x['strike_high']) + ' for ' + str(x['strike_high_ce_price'])
                action +=  ' and Buy ' + x['ticker'] + ' CALL@' + str(x['strike_high_nxt']) + ' for ' + str(x['strike_high_ce_nxt_price'])
        return action
    
    def screener(self,nextExpiryCount = 1):
        data = []
        failed = []
        helper = OptionHelper()
        for ticker in tqdm(helper.fnoList()[:5]):
            try:
                df = helper.formatedOptionChain(ticker, False)
                ...
                iv_ce = round(np.mean(df[df['strikePrice'].isin(self.getClosestStrikes(callStrikes, up))].iv_ce),2)
                iv_pe = round(np.mean(df[df['strikePrice'].isin(self.getClosestStrikes(putStrikes, up))].iv_pe),2)
                ...
                strike_low = self.getClosestStrikes(putStrikes, low,1)[0]
                strike_low_nxt = strikes[strikes.index(strike_low) - 1]
                strike_high = self.getClosestStrikes(callStrikes, high,1)[0]
                ...
                data.append({
                    ...
                    'strike_low_nxt' : strike_low_nxt,
                    'strike_high_nxt' : strike_high_nxt,
        ...
        data['Action'] = data.apply(lambda x: self.getAction(x), axis = 1)
        return data
