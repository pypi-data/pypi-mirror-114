...
def pdf_of_pages(n):
    inp = 'c:\\temp\\1.png'
    out = 'c:\\temp\\out.pdf'
    f = PIL.Image.open(inp)

    images = []
    for i in range(n-1):
        im = PIL.Image.open('c:\\temp\\2.png')
        im.load()
        images.append(im)

    f.save(out, title='sdasd', append_images=images, save_all=True)
    #for i in range(n-1):
    #    f.save(out, "PDF", resolution=100.0, append=True, title='מסמך תולעת המשפט', author='מחבר')


def change_title():
    from PyPDF2 import PdfFileReader, PdfFileWriter
    from PyPDF2.pdf import DocumentInformation, TextStringObject
    out = 'c:\\temp\\out.pdf'
    out2 = 'c:\\temp\\out-2.pdf'
    pdf = PdfFileReader(out)
    metadata = pdf.getDocumentInfo()
    print(type(metadata))
    metadata[TextStringObject('/Title')] = TextStringObject('modified')
    inf = DocumentInformation()
    writer = PdfFileWriter()
    writer.addMetadata(metadata)
    writer.appendPagesFromReader(pdf)

    writer.write(open(out2, 'wb'))
