...
def get_mapper_xml(database_info: DataBaseInfo, table_name: str) -> str:
    ...
    param = {
        "data_base_name": database_info["name"],
        "table": database_info["table_map"][table_name]
    }

    # Find primary key information
    key = ""
    for item in param["table"]["indexs"]:
    ...
    # Load supported param types
    param["int_types"] = ["bit", "tinyint", "smallint", "mediumint", "int", "integer", "bigint"]
    param["float_types"] = ["real", "double", "float", "decimal", "numeric"]
    param["date_types"] = ["date", "time", "year", "timestamp", "datetime"]
    param["string_types"] = ["char", "varchar", "text", "mediumtext", "longtext"]
