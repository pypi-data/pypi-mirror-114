...
class ErrorNoSgsPlay(QMainWindow):
    def __init__(self):
    ...
    def mood_window(self):
        ...
        Person.setLabel(self, "Oops! There are no songs in", True, self.sw * 130, self.sh * 200, self.sw * 740, self.sh * 50, self.sw * 20, "#ffe0ff", True, 'Segoe UI')
        Person.setLabel(self, "the playlist you selected! (try adding some!)", True, self.sw*150, self.sh*300, self.sw*740, self.sh*50, self.sw*20, "#ffe0ff", True, 'Segoe UI')

        self.tryAgainBtn = QPushButton("Select a Different Playlist", self)
