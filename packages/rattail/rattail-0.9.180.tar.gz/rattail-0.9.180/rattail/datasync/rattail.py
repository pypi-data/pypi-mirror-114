...
class FromRattailToRattailExportConsumer(NewDataSyncImportConsumer):
    ...
    def setup(self):
        ...
        self.handle_price_xref = self.config.getbool(
    ...
    def process_changes(self, session, changes):
        ...
        # if we had any Product changes, we may also want to process
        # the "price uuid" fields for each, via a custom importer just
        # for that purpose.  note that we do this *after* all other
        # changes have been processed...in the hopes that ProductPrice
        # records are already in place, and nothing will break!
        if self.handle_price_xref:
            product_changes = [c for c in changes
                               if c.payload_type == 'Product' and not c.deletion]
            if product_changes:
                price_xref_importer = self.handler.get_importer(
                    'ProductPriceAssociation',
                    host_session=session, session=target_session)
                for change in product_changes:
                    self.process_change(session, price_xref_importer, change)
