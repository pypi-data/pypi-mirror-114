...
"""Tests for detection_tools.utils.detection_tools_evaluation."""
...
from detection_tools import eval_util
from detection_tools.core import standard_fields
from detection_tools.utils import detection_tools_evaluation
from detection_tools.utils import tf_version
...
class OpenImagesV2EvaluationTest(tf.test.TestCase):
  ...
  def test_returns_correct_metric_values(self):
    ...
    oiv2_evaluator = detection_tools_evaluation.OpenImagesDetectionEvaluator(
        categories)
    image_key1 = 'img1'
...
class OpenImagesChallengeEvaluatorTest(tf.test.TestCase):
  ...
  def test_returns_correct_detection_metric_values(self):
    ...
    oivchallenge_evaluator = (
        detection_tools_evaluation.OpenImagesChallengeEvaluator(
...
class PascalEvaluationTest(tf.test.TestCase):
  ...
  def test_returns_correct_metric_values_on_boxes(self):
    categories = [{'id': 1, 'name': 'cat'},
                  ...
                  {'id': 3, 'name': 'elephant'}]
    #  Add groundtruth
    pascal_evaluator = detection_tools_evaluation.PascalDetectionEvaluator(
        categories)
    image_key1 = 'img1'
  ...
  def test_returns_correct_metric_values_on_masks(self):
    ...
    #  Add groundtruth
    pascal_evaluator = (
        detection_tools_evaluation.PascalInstanceSegmentationEvaluator(
            categories))
    image_key1 = 'img1'
    groundtruth_boxes1 = np.array([[0, 0, 1, 1], [0, 0, 2, 2], [0, 0, 3, 3]],
                                  dtype=float)
...
class WeightedPascalEvaluationTest(tf.test.TestCase):
  ...
  def create_and_add_common_ground_truth(self):
    #  Add groundtruth
    self.wp_eval = (
        detection_tools_evaluation.WeightedPascalDetectionEvaluator(
            self.categories))
...
class PrecisionAtRecallEvaluationTest(tf.test.TestCase):
  ...
  def create_and_add_common_ground_truth(self):
    #  Add groundtruth
    self.wp_eval = (
        detection_tools_evaluation.PrecisionAtRecallDetectionEvaluator(
...
class ObjectDetectionEvaluationTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self.od_eval = detection_tools_evaluation.ObjectDetectionEvaluation(
  ...
  def test_value_error_on_zero_classes(self):
    with self.assertRaises(ValueError):
      detection_tools_evaluation.ObjectDetectionEvaluation(
  ...
  def test_merge_internal_state(self):
    ...
    copy_od_eval = detection_tools_evaluation.ObjectDetectionEvaluation(
...
class ObjectDetectionEvaluatorTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def setUp(self):
    ...
    self.od_eval = detection_tools_evaluation.ObjectDetectionEvaluator(
