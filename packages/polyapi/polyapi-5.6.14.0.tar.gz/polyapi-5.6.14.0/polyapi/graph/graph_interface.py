...
# перечисление типов уже реализованных графиков
# нужно для того, чтобы пользователь не смог вызвать ещё не существующие графики
# подлежит удалению, когда все типы графиков будут реализованы (ID-3190)
IMPLEMENTED_GRAPHS = ['lines', 'cylinders', 'cumulative_cylinders', 'point', 'point_series', 'chord', 'sankey',
        'radar', 'circles', 'circles_series', 'balls', 'pools', '3d_pools', 'pies', 'areas', 'cumulative_areas']

class Graph():
    """Класс, реализующий методы для взаимодействия пользователя с графиками"""
    def __init__(self, base_bl: 'BusinessLogic', graph_type: Union[int, str], settings: str, grid: int,
