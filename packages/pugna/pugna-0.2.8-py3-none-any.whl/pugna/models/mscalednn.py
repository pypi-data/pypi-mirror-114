...
def build_model(input_dim, output_dim, nlayers, units, nscales, activations, dropouts, batch_norms, scale_names, bias_initializer="random_normal"):
    """
    Args:
        ...
        bias_initializer: default "random_normal" can use something liek
            bias_initializer=tf.keras.initializers.RandomUniform(minval=-10, maxval=10, seed=None)
    ...
    for n in range(nlayers):
        model.add(pugna.layers.Mscale(units[n], nscales[n], scale_names[n], bias_initializer=bias_initializer))
        if batch_norms[n]:
