...
class DataUploadClient:
    ...
    def create_session(self, files_lists: list, type_of_files: str, auth_info: AuthInfo):
        ...
        response = requests.post(config.BASE_IMG_UPLD_URL + 'session-create', json=session_camel_case, headers=headers)
    ...
    def start_session(self, session_id: str, auth_info: AuthInfo):
        ...
        response = requests.put(config.BASE_IMG_UPLD_URL + 'session-start', json=session_camel_case, headers=headers)
    ...
    def resume_session(self, session_id: str, auth_info: AuthInfo):
        ...
        response = requests.put(config.BASE_IMG_UPLD_URL + 'session-resume', json=session_camel_case, headers=headers)
        return {'status_code': response.status_code}
    ...
    def cancel_session(self, session_id: str, auth_info: AuthInfo):
        ...
        response = requests.put(config.BASE_IMG_UPLD_URL + 'session-cancel', json=session_camel_case, headers=headers)
        return {'status_code': response.status_code}
    ...
    def end_session(self, session_id: str, auth_info: AuthInfo):
        ...
        response = requests.put(config.BASE_IMG_UPLD_URL + 'session-end', json=session_camel_case, headers=headers)
        return {'status_code': response.status_code}
        ...
        for file_id, file_info in session['fileMap'].items():
            if (file_info['transmittedFlag']):
                ...
                for i in range (file_info['totalChunks']):
                    ...
                    response = requests.post(config.BASE_IMG_UPLD_URL + session['sessionId'], data=form, headers=formDataHeaders)
