...
class StorageManager(ABC):
    ...
    def update(self, state: StorageItem, values: Dict) -> None:
        ...
        if is_updated:
            self.__redis_client.set(state.get_id().__str__(), json.dumps(raw_data))

    # -----------------------------------------------------------------------------

    def delete(self, state: StorageItem):
        if self.__redis_client.get(state.get_id().__str__()) is not None:
            self.__redis_client.delete(state.get_id().__str__())
