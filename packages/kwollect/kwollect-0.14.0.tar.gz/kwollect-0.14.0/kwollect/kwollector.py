    ...
    for device, metrics in metrics_per_device.items():
        ...
        asyncio.create_task(
            schedule_every(
                ...
                process_method,
                (device, metrics),
                task_name=f"{device.protocol}{device.port if device.port else ''}@{device.hostname}",
            )
        )
...
async def process_prometheus_host(device, metrics):
    ...
    for metric in metrics:
        ...
        # Metric to be fetched may be limited to what's in path
        ...
        for prom_metric in resp_txt.split("\n"):
            ...
            if prom_metric == "" or prom_metric.startswith("#"):
                continue
            ...
            # copy is more effient than dataclass' replace
            fetched_metric = copy.copy(metric)
            fetched_metric.name = "prom_" + prom_metric_name
    ...
    # We might get duplicates if several metrics relate to the same exporter
    if len(metrics) > 1:
        fetched_metrics_nodup_dict = {
            (values[idx][0], metric.device_id, metric.name, str(metric.labels)): metric
            for idx, metric in enumerate(fetched_metrics)
        }
        fetched_metrics = fetched_metrics_nodup_dict.values()
...
async def filter_optional_metrics(metrics):
    ...
    if cur_time - promoted_lastupdate > 0:
        ...
        promoted_metrics_by_devices = {
            ...
            for promoted_device, promoted_metric in await sql(
