...
su postgres -s /bin/sh -c "kwollect-setup-db --chunk_interval_hour 1 --kwollect_password changeme --chunk_compress_hour $((24*365*10)) --request_timeout_min $((5*60))"
...
su postgres -c "echo \"WITH chunks AS (SELECT chunk_table FROM chunk_relation_size_pretty('metrics') ORDER BY ranges ASC LIMIT (SELECT COUNT(*) from chunk_relation_size_pretty('metrics'))/2) SELECT compress_chunk(chunk_table, if_not_compressed=>true) from chunks;\" | psql kwdb"
...
echo "CREATE OR REPLACE VIEW nodetime_by_job AS
  ...
  SELECT 2 AS job_id, '2020-12-31 22:00:00+00'::timestamp with time zone as start_time, NULL::timestamp with time zone as stop_time, 'device-9' as node;" | su postgres -c "psql kwdb"


su postgres -s /bin/sh -c "echo \"CALL refresh_continuous_aggregate('metrics_summary', NULL, NULL);\" | | psql kwdb"
