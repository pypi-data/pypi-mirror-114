...
async def async_main():
    ...
    log.info(
        "Scheduling %s SQL workers every seconds", sql_worker_count
    )
...
async def process_snmp_host(device, metrics):
    ...
    await batch_metrics_values(metrics, values)
...
async def process_ipmisensor_host(device, metrics):
    ...
    command += ["--sdr-cache-recreate", "-D", "LAN_2_0", "-h", device.hostname]
    if device.username:
...
async def process_prometheus_host(device, metrics):
    ...
    await batch_metrics_values(fetched_metrics, values)
...
async def batch_metrics_values(metrics, values):
    """Insert metrics and associated values into DB"""

    log.debug(
        "Preparing SQL INSERT for task: %s",
        asyncio.current_task().task_name,
    )
...
async def insert_metrics_values():
    ...
    if len(sql_values_batch) > 100 or cur_time - sql_values_lastinsert > 1:
        ...
        try:
            ...
            log.error("DB connection error on SQL INSERT: %s", ex)
            log.debug(traceback.format_exc())
        except Exception as ex:
            log.error("Error when performing SQL INSERT command, abording")
            log.error(traceback.format_exc())
        log.debug(
            "Done SQL INSERT %d lines in %s (pool size : %d/%d) for task: %s",
            len(sql_values),
            time.time() - qtime,
            psql_pool._queue.qsize(),
            psql_pool._maxsize,
        ...
        log.debug(
            "Batching SQL INSERT for task: %s",
            asyncio.current_task().task_name,
        )
