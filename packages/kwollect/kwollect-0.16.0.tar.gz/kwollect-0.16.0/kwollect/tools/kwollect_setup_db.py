...
ALTER TABLE metrics SET (timescaledb.compress, timescaledb.compress_segmentby = 'device_id, metric_id, labels', timescaledb.compress_orderby = 'timestamp DESC');
...
CREATE MATERIALIZED VIEW IF NOT EXISTS metrics_summary
  ...
  SELECT time_bucket(INTERVAL '15 minute', timestamp) AS timestamp,
    device_id,
    metric_id,
    AVG(value) AS value,
    labels
  FROM metrics
  GROUP BY time_bucket(INTERVAL '15 minute', timestamp), device_id, metric_id, labels
  WITH NO DATA;
...
cond = []
if job_id:
    req = "SELECT timestamp, device_id, metric_id, value, labels FROM metrics_by_job WHERE "
    cond.append("job_id = %d" % job_id)
else:
    req = "SELECT * FROM metrics_by_device WHERE "
