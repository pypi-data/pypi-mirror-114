...
class DrumRawClassifier:
    ...
    def add_bar_class(self, midi_obj, bar_resolution=1920, drums=None):
        drum_maps = get_drum_maps(midi_obj, bar_resolution=bar_resolution, drums=drums)

        for bar, drum_map in enumerate(drum_maps.reshape(drum_maps.shape[0], len(drums) * drum_maps.shape[2])):
