...
class IntraValidationDriver(Driver):
    ...
    async def extract_job_information(self, body: Dict[str, Any]) -> Dict[str, Any]:
        credentials = self.config.credentials["intra"]

        try:
            # TODO : Replace with AsyncEtnaSession
            session = EtnaSession(
    ...
    async def _download_from_intra(self, moulinette_name: str, info: Dict[str, Any]):
        credentials = self.config.credentials["intra"]
        # TODO : replace with AsyncEtnaSession
        session = EtnaSession(
            username=credentials.username,
    ...
    async def retrieve_moulinette(self, info: Dict[str, Any]) -> str:
        ...
        try:
            ...
            try:
                await self._download_from_intra(moulinette_name, info)
            except EtnaAPIError as e:
                raise RetryableError(e)
        except Exception:
