...
class VectorOperator(object):
    ...
    def __init__(self, **kwargs):
        ...
        if "coords" in kwargs:
            ...
            self.ndims = self.__get_dimension(self.coords)
            self.componentsec = [FinDiff((k, self.coords[k], 1), **kwargs) for k in range(self.ndims)]
            self.componentsec2 = [FinDiff((k, self.coords[k], 2), **kwargs) for k in range(self.ndims)]
            self.componentcross = [FinDiff((i, self.coords[i]),(j, self.coords[j])) for i in range(self.ndims) for j in range(self.ndims)]
...
class Hessian(VectorOperator):
    ...
    def __call__(self, f):
        ...
        # if len(f.shape) != self.ndims + 1 and f.shape[0] != self.ndims:
        #     raise ValueError("Hessian can only be applied to vector functions of the same dimension")
        ...
        for k in range(self.ndims**2):
            result.append(self.componentcross[k](f, acc=self.acc))
