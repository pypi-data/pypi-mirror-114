...
class Shift(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if not np.isfinite(np.asarray(shift)).all():
            warnings.warn(
                "The defined shift contains infinite values. This may lead to numerical issues."
            )
...
class Scale(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if not np.isfinite(np.asarray(scale)).all():
            warnings.warn(
                "The defined scale contains infinite values. This may lead to numerical issues."
            )
