...
def validate_metadata(metadata: ModelMetadata, model_input_shape: List[int], model_output_shape: List[int]) -> None:
    ...
    """
    if len(model_input_shape) != len(metadata.input_shape) or \
            not all(model_dim in (None, 0, meta_dim) for meta_dim, model_dim in zip(
                metadata.input_shape, model_input_shape
            )):
        raise ValueError(
    ...
    if len(model_output_shape) != len(metadata.output_shape) or \
            not all(model_dim in (None, 0, meta_dim) for meta_dim, model_dim in zip(
                metadata.output_shape, model_output_shape
            )):
        raise ValueError(
