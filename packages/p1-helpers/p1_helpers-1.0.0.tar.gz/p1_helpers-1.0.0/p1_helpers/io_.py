...
import p1_helpers.system_interaction as hsyste
...
def create_dir(
    ...
    """
    p1_dbg.dassert_is_not(dir_name, None)
    p1_dbg.dassert(
        os.path.normpath(dir_name) != ".", msg="Can't create the current dir"
    )
    if abort_if_exists:
        p1_dbg.dassert_not_exists(dir_name)
...
def create_enclosing_dir(file_name: str, incremental: bool = False) -> str:
    ...
    """
    p1_dbg.dassert_is_not(file_name, None)
    # p1_dbg.dassert_isinstance(file_name, str)
...
def to_file(
    ...
    if mode is None:
        if use_gzip:
            ...
            mode = "w"
    # p1_dbg.dassert_in(type(file_name), (str, unicode))
...
def from_file(
    ...
    p1_dbg.dassert_exists(file_name)
    if use_gzip:
        # Check if user provided correct file name.
        if not file_name.endswith(("gz", "gzip")):
            _LOG.warning("The provided file extension is not for a gzip file.")
    ...
    try:
        ...
        f.close()
    p1_dbg.dassert_isinstance(data, str)
    return data
