...
class Dataset:
    ...
    def split(self, split: Union[str, List[str]]) -> Dataset:
    ...
    def __repr__(self) -> str:
        return f"<Sarus Dataset id={self.id} name={self.name}>"

    def as_tensorflow(self) -> SarusTensorflowDataset:
        """
        Returns the corresponding `SarusTensorflowDataset` object.
    ...
    def _synthetic_to_html(
        ...
        """
        synthetic: pd.DataFrame = self._fetch_synthetic(
    ...
    def _synthetic_as_tf_dataset(
        ...
        output_types = dict()
        output_shapes = dict()
        for c in self._fetch_synthetic(
            force_refresh=True, original=False
        ).columns:
            val = self._fetch_synthetic()[c].iloc[0]
            if self._fetch_synthetic()[c].dtype == "O":
        ...
        def generator():
            for _, row in self._fetch_synthetic(
...
class Client:
    ...
    def list_datasets(self) -> List[Dataset]:
        ...
        return list(map(lambda x: self.dataset(name=x), dataset_names))

    def dataset(self, name: str = None, id: int = None) -> Dataset:
        ...
        """
        if name:
            return self._fetch_dataset_by_name(name)
        else:
        ...
        **kwargs,
        ...
        Args:
            ...
            wait_for_completion (bool): if True waits until the model training
                is finished to return (and displays progress info while
                waiting)
        ...
        # Change loss reduction to NONE for DP-SGD
        if model.loss.reduction != tf.keras.losses.Reduction.NONE:
            model.loss.reduction = tf.keras.losses.Reduction.NONE
            if verbose:
                print("Changing loss reduction to NONE.")
