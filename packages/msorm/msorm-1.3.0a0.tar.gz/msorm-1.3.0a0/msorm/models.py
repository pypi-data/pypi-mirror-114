...
class Model:
    ...
    def where(cls, *args, **kwargs):
        ...
        kwargs = " AND ".join([f"{mssql_fields.field.n_find_filter(key, value)}" for key, value in kwargs.items()])
        args = " ".join([str(arg) for arg in args])
    ...
    def n_where(cls, *args, **kwargs):
        ...
        names = " AND ".join([f"{mssql_fields.field.find_filter(key, value)}" for key, value in kwargs.items()])
        args = " ".join([str(arg) for arg in args])
        text = 'SELECT {fields} FROM {table} WHERE ({kwargs} {args})'.format(
            fields=str(f'{", ".join(fields)}' if fields else "*"),
            table="dbo." + cls.__table_name__,
