...
def start():
  ...
  state = OperationState(utils.rand_str(), operation.get_id())

  print(f"Operation[{operation.get_title() or operation.get_id()}]")
  ...
  while simulator.stage_at(stage_ind) is not None:
    ...
    print(f"  Stage [{stage.get_title()}]:")
    if step_id is None:
      step_id = stage.steps[0].get_id() if len(stage.steps) > 0 else 'done'
    while not step_id == 'done':
      step = stage.step(step_id)
      print(f"    Step [{step.get_title()}]:")
