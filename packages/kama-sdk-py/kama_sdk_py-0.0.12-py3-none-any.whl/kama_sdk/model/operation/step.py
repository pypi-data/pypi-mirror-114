...
class Step(Model):
  ...
  def get_summary_descriptor(self, inputs: Dict, op_state) -> Optional[Dict]:
    self.patch(gen_downstream_patches(op_state, inputs))
    try:
      return self.resolve_prop(SUMMARY_DESCRIPTOR, lookback=False)
    except:
      perr(self, f"{self.id()} compute summary desc failed", True)
      return None
...
def action2config(action: Action) -> Dict:
  if isinstance(action, MultiAction):
    ...
    return {
      ...
      model.TITLE_KEY: action.title,
      model.INFO_KEY: action.info,
