...
def ensure_top_level_to_child_refs(descriptor: Dict) -> Optional[Violation]:
  ...
  for ref in refs:
    ...
    if ref.startswith(ID_REFERENCE_PREFIX):
      referee = model.find_any_config_by_id(un_prefixed_ref)
    elif ref.startswith(KIND_REFERENCE_PREFIX):
      referee = model.find_any_class_by_name(un_prefixed_ref)
    else:
      can_handle = False
...
def ensure_kind_class_exists(descriptor: Dict) -> Optional[Violation]:
  if kind := descriptor.get('kind'):
    if not model.find_any_class_by_name(kind):
      return Violation(
        severity='error',
