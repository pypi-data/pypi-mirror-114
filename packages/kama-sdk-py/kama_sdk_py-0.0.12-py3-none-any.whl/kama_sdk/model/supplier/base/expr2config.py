...
def tokens2tree(parts: List[str]) -> Dict:
  if len(parts) == 1:
    ...
    pwar(__name__, f"malformed expr {parts}")
    return {}
...
def case_of_more_than_three(parts):
  ...
  return {
    model.KIND_KEY: Supplier.__name__,
...
def polymorphic_expr_to_config(expr: str) -> Dict:
  ...
  if expr == self_key:
    return {model.KIND_KEY: SelfSupplier.__name__}
  elif expr.startswith(DELAYED_REF_PREFIX):
    un_ref_expr = expr[len(DELAYED_REF_PREFIX):]
...
def delayed_expr_to_config(expr: str) -> Dict:
  if expr.startswith(model.ID_REFERENCE_PREFIX):
    actual = expr[len(model.ID_REFERENCE_PREFIX):]
    return delayed_inherit_expr_to_config(actual)
  elif expr.startswith(model.KIND_REFERENCE_PREFIX):
    actual = expr[len(model.KIND_REFERENCE_PREFIX):]
...
def delayed_kind_expr_to_config(kind_expr: str) -> Dict:
  ...
  return {
    model.KIND_KEY: DelayedInflateSupplier.__name__,
    supplier.SRC_DATA_KEY: {
      model.KIND_KEY: kind_expr
    },
  }
...
def direct_inherit_expr_to_config(id_expr: str):
  ...
  return {
    model.KIND_KEY: Supplier.__name__,
    model.INHERIT_KEY: id_expr
  }


def direct_kind_expr_2_config(kind_name: str):
  return {
    model.KIND_KEY: kind_name
  }
