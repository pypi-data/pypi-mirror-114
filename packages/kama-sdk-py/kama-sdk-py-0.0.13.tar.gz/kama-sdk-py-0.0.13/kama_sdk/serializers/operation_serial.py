...
def ser_standard(operation: Operation):
  ...
  return dict(
    id=operation.get_id(),
    title=operation.get_title(),
    info=operation.get_info(),
    tags=operation.get_tags(),
    synopsis=operation.get_synopsis(),
    stages=list(map(ser_embedded_stage, operation.get_stages())),
    space=operation.get_space()
  )
