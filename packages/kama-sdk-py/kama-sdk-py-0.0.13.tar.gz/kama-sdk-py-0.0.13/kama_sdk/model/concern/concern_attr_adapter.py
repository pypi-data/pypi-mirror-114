...
class ConcernAttrAdapter(Model):
  ...
  @cached_property
  def label(self) -> str:
    return self.get_attr(LABEL_KEY) or self.get_id()

  @cached_property
  def value_info(self) -> str:
    return self.get_attr(VALUE_INFO_KEY)
  ...
  def compute_value(self) -> Optional[Any]:
    ...
    if VALUE_KEY in self.config.keys():
      return self.get_attr(VALUE_KEY, depth=100)
    elif LABEL_KEY in self.config.keys():
      synth_name = f"get::props {CONCERN_KEY}=>{self.label}"
      return self.get_attr_value(synth_name, depth=100)
    else:
