...
class Stage(Model):
  ...
  def first_step_id(self) -> Optional[str]:
    ...
    """
    return self.steps[0].get_id() if len(self.steps) > 0 else None
  ...
  def next_step_id(self, crt_step: Step, op_state: OperationState) -> str:
    ...
    if override_value:
      ...
      index = step_index(stage_steps, crt_step.get_id())
      is_not_last = index < len(stage_steps) - 1
      return stage_steps[index + 1].get_id() if is_not_last else 'done'
