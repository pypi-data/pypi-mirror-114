...
class Model:
  ...
  def __init__(self, config: Dict):
    self.config = config
    self._id = config.get(ID_KEY)
    self.parent = None
    self.cached_results = {}
    self.config['kind'] = self.__class__.__name__
    self._attr_helper_map = self.gen_attr_helper_map()

  def gen_attr_helper_map(self) -> Dict[str, Callable]:
    result = {}
    for method_name, _ in inspect.getmembers(self.__class__):
      candidate_method = getattr(self.__class__, method_name)
      if hasattr(candidate_method, ATTR_KEY_KEY):
        setattr(candidate_method, 'cache', self.cached_results)
  ...
  def synopsis(self) -> str:
    return self.resolve_prop(SYNOPSIS_KEY, lookback=0)
  ...
  def labels(self) -> Dict:
    return self.resolve_prop(LABELS_KEY, lookback=0) or {}
  ...
  @cached_property
  def space(self):
    return self.get_prop(SPACE_KEY)

  @cached_property
  def config_space(self) -> str:
    # ideally safe to return None though
    if explicit := self.get_prop(CONFIG_SPACE_KEY):
      value = explicit
    else:
      value = self.space
  ...
  def selectable_by(self, selector: Dict) -> bool:
    return utils.selects_labels(selector, self.labels)
  ...
  def resolve_list_like_prop_value(self, value: List, depth=0):
    ...
    for sub_value in value:
      if type(sub_value) == str and sub_value.startswith(SPLATTER_STR):
        ...
        resolved_sub_value = self.resolve_prop_value(resolvable_part, depth)
        ...
        resolved_sub_value = self.resolve_prop_value(sub_value, depth)
  ...
  def resolve_child_ref_kod(self, **kwargs) -> Union[List[KoD], KoD]:
    if explicit_kod := kwargs.get('kod'):
      ...
      if kwargs.get(RESOLVE_CHILD_KOD_FLAG, True):
        return self.resolve_prop(prop_name, lookback=0)
      else:
        # return self.resolve_prop(prop_name, lookback=0, depth=0)
