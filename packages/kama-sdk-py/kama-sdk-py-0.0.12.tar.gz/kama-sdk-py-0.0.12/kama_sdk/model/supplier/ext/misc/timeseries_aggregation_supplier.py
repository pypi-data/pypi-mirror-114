...
class TimeseriesAggregationSupplier(Supplier):
  ...
  def source_data(self) -> Optional[List[Dict]]:
    return super(TimeseriesAggregationSupplier, self).source_data()

  def outer_aggregator(self) -> str:
    return self.get_prop(INNER_AGG_FUNC_KEY, 'sum')

  def inner_aggregator(self) -> str:
    return self.get_prop(OUTER_AGG_FUNC_KEY, 'sum')

  def t_point(self) -> str:
    return self.resolve_prop(T_POINT_KEY, backup='n-1', lookback=0)

  def _compute(self):
    if timeseries := self.source_data():
      subset = outer(timeseries, self.t_point())
      scalars = nd_to_1d(subset, self.inner_aggregator())
      return _1d_to_1(scalars, self.outer_aggregator())
