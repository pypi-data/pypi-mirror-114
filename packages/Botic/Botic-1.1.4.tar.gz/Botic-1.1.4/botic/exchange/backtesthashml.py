...
class BacktestHashML(BaseExchange):
    """Backtest"""
    ...
    def __init__(self, config: configparser.ConfigParser) -> None:
        ...
        while 1:
            try:
                ...
                print('DONE;', err)
                break
            if self._data_pos % 1000 == 0:
                elapsed = time.time() - start_time
                pdone = self._data_pos / data_len
                sys.stdout.write('{}/{} {:.2f}m {:.2f}%                 \r'.format(
                    self._data_pos, data_len, elapsed/60,
                    pdone*100
                ))
            if self._data_pos % 101 == 0:
                if len(self._hdq) == 60:
                    X = []
                    for i in range(30):
                        a = self._hdq[i]
                        ...
                        p = round((a - b) / a, 5)
                        X.append(str(p))
                    y = round((self._hdq[29] - self._hdq[-1]) / self._hdq[29], 5)
                    self._hml.fit(X, str(y))
        # REWIND buffer
        self._data_pos = 1
        self._data_buf = []
        self._data_buf_idx = 0
    ...
    def get_hold_value(self) -> Decimal:
        return self._coins * self._last_price
        total = Decimal('0.0')
