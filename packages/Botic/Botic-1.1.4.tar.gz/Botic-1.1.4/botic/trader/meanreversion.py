...
class Meanreversion(BaseTrader):
    ...
    def _maybe_sell(self) -> None:
        # Now check all buy orders without an associated sell order and see if they can sell
        for buy_order_id, info in self.data.items():
            if self.data[buy_order_id]['completed'] or info['sell_order']:
                continue
            bought_price = Decimal(info['last_status']['funds'])/Decimal(info['last_status']['filled_size'])
            can_sell = self._check_if_can_sell(bought_price)
            #print('debug-can-sell: {} --> {}'.format(bought_price, can_sell))
            if not can_sell:
                continue
            try:
                price_target = round(self.current_price * Decimal('0.001') + self.current_price, 2)
                response = self.exchange.sell_limit(
                    price_target,
                    info['last_status']['filled_size']
                )
                self.logit('SELL-RESPONSE: {}'.format(response),
                    custom_datetime=self._time2datetime())
                msg = 'SELL-PLACED: size:{} price:{}'.format(
                    info['last_status']['filled_size'], price_target)
                for i in msg.split('\n'):
