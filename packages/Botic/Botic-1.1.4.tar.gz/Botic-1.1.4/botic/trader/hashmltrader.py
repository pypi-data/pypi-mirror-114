...
from ..hashml import HashML

class HashMLTrader(BaseTrader):
    """HashMLTrader trader"""
    ...
    def __init__(self, config) -> None:
        ...
        self._hdq = deque(maxlen=30)
        self._hml_sell_target = None
        self._hml_last_check = None
    ...
    def _check_if_can_buy(self) -> bool:
        ...
        if len(self._hdq) != 30:
            return False
        if not self. _hml_last_check:
            self._hml_last_check = self.exchange.get_time()
        if self.exchange.get_time() - self._hml_last_check < 60:
            return False
        self._hml_last_check = self.exchange.get_time()
        X = []
        for i in range(len(self._hdq)-1):
            a = self._hdq[i]
            b = self._hdq[i+1]
            p = round((a - b) / a, 5)
            X.append(str(p))
        prediction = Decimal(self.exchange._hml.predict(X))
        #print('prediction:', str(prediction))
        if prediction*100 < 4:
            can = False
        self._hml_sell_target = self.current_price + self.current_price * prediction
        self.current_price_target = self._hml_sell_target
