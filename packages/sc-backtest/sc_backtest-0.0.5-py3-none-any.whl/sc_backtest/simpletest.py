...
def get_report(indicator, ret):
    """
    :param indicator: pd.Series with DatatimeIndex
    :param ret: pd.Series with DatatimeIndex
    :return: report: pd.DataFrame
    """
    if len(indicator) != len(ret):
        print(F'mismatched length of indicator, pnl, len(indicator) == {len(indicator)} len(ret) == {len(ret)}')
        return

    if not (isinstance(indicator, pd.Series)) or not (isinstance(ret, pd.Series)):
        print('indicator and pnl must be pd.Series')
        return

    if not (isinstance(indicator.index, pd.core.indexes.datetimes.DatetimeIndex)) \
    ...
    report_sub = indicator.resample('Y').apply(lambda x: _get_report(x, ret.reindex(x.index))).unstack()
    report_sub.index = report_sub.index.to_series().apply(lambda x: x.year)
...
class simpletest():
    ...
    def simple_pnl(self, x, y,
                   ax=None, plot=True, data_return=False):
        """
        x: pd.Series
        y: pd.Series
        ...
        if data_return:
            return {'pnl_sign': sum_pnl1, 'pnl_med': sum_pnl2, 'delta_sign': indicator1, 'delta_med': indicator2}

        return

    def threshold_sign_pnl(self, x, y, t_list, ax=None, plot=True, data_return=False):
        """
        x: pd.Series
        y: pd.Series
        ...
        if len(t_list) == 2:
            indicator.loc[factor > t_list[1]] = 1 * np.sign(corr)
            indicator.loc[factor < t_list[0]] = -1 * np.sign(corr)
            indicator.loc[factor >= t_list[0] & factor <= t_list[1]] = 0
        else:
            indicator.loc[factor >= t_list[0]] = 1 * np.sign(corr)
            indicator.loc[factor < t_list[0]] = -1 * np.sign(corr)
        ...
        if self.is_day:
            ...
            day_pnl = sum_pnl[self.freq - 1::self.freq].diff()
    ...
    def plot_composite(self, x, y, markout_periods=30, cdf_period2=5, fig=None, figsize=(20, 20), fig_return=False):
        """
