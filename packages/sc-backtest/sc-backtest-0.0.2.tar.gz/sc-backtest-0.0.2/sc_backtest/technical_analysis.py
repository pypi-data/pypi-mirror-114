...
def gma(x, window):
    """
    The Geometric moving average calculates the geometric mean of the previous N bars of a time series x.
    The geometric mean is calculated by multiplying the time series' N previous values and then taking the
    N'th root product of the last result : $\left( \prod_{i=1}^{N} x_i \right)^\frac{1}{N}$
    :param x:
    :param window:
    :return:
    """
...
def qma(x, window):
    """
    The quadratic moving average filter is calculated as follows :
    QMA$_N$ =  $\Large \sqrt{\frac{\sum_{i=1}^{N}{x_i^2}}{N}}$
    where $N$ is the lookback period and x the price time series.
    """
