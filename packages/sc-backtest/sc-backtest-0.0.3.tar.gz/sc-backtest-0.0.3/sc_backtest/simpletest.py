...
class simpletest():
    ...
    def simple_pnl(self, x, y,
        ...
        if data_return:
            return {'pnl_sign': sum_pnl1, 'pnl_value': sum_pnl2, 'delta_sign': indicator1, 'delta_value': indicator2}
    ...
    def threshold_sign_pnl(self, x, y, t_list, ax=None, plot=True, data_return=False):
        ...
        if not 1 <= len(t_list) <= 2:
            print(F'mismatched length of t_list, len(t_list) == {len(t_list)} (t_list length should be 1 pr 2)')
            return
    ...
    def plot_cdf(self, x, y,
        ...
        ax.text(peak_idx, peak_cdf, 'factor val: {:.2f}'.format(peak))
