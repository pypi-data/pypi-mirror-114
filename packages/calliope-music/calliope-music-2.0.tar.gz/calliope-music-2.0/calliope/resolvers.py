...
from calliope.utils import normalize_artist_title
...
def _annotate_similarity(item: Item, candidate: Item) -> Item:
    ...
    artist_i, title_i = normalize_artist_title(item.get("creator"), item.get("title"))
    artist_c, title_c = normalize_artist_title(
        candidate.get("creator"), candidate.get("title")
    )
    ...
    isrcs_i = _get_isrcs(item)
    isrcs_c = _get_isrcs(candidate)
