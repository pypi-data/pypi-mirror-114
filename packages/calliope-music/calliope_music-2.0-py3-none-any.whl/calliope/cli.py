...
@click.option(
    ...
    default=False,
    help='Promt for user input if no perfect matches are found',
)
@click.pass_context
...
@click.argument('playlist', type=click.File(mode='r'))
@click.pass_context
def cmd_spotify_resolve(context, playlist):
    '''Add Spotify-specific information to tracks in a playlist.'''
    context.obj.spotify.authenticate()
    api = context.obj.spotify.api
    for track in calliope.spotify.resolve(api, calliope.playlist.read(playlist)):
