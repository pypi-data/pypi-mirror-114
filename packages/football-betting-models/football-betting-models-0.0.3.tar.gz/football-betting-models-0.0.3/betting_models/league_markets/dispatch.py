...
class ModelDispatch:
    ...
    def generate_seeds(
        ...
        #batches = sm.batchify_seeds(seeds, n_batch_splits)
        json = ModelDispatch.json_encode(matchids, leagueid, seeds)   # batches['batch_1']
        fig = ModelDispatch.print_status(prob_list, seeds)
        return json, fig  #, plot_img_data


def main(n_seeds, tol, prob_list, n_batch_splits, matchids, leagueid):
