...
class SeedingModel:
    ...
    def get_seeds(self, prob_list, bias=0.0):
        'Return near-optimal league seeds within tolerance in tensor format'
        target_alloc = 1
        for T in prob_list:
            target_alloc = np.tensordot(target_alloc, T, axes=0)
        result = self._optimise(target_alloc)
        return result
    
    def get_tailed_seeds(self, prob_list, lb_cutoff=10, ub_cutoff=5000, random_seed=False):
        ...
        result = self._optimise(adjusted_target)
        
        return result        

    def get_tailed_seeds_pct(self, prob_list, lb_cutoff_proba=0.20, 
